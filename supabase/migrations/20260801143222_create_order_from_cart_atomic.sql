-- ============================================================================
-- Migración — create_order_from_cart(...): reserva de stock + orden + items
-- + receta + cupón en UNA sola transacción atómica.
-- ============================================================================
-- Hallazgo #8, audit 2026-08-01: `lib/checkout/orders.ts` hacía reserve_stock
-- (RPC) + INSERT prescriptions + INSERT orders + INSERT order_items + INSERT
-- coupon_redemptions en pasos JS separados (cada uno su propio round-trip /
-- transacción). Si el proceso Node moría entre pasos (crash, timeout de
-- Vercel, deploy a mitad de request), quedaba stock decrementado SIN orden
-- asociada — inventario fantasma, sin forma de auto-corregirse. La
-- compensación manual (`revertStock`) solo corre si el código Node sigue
-- vivo para ejecutarla.
--
-- Esta función mueve TODO a un solo statement PL/pgSQL. Una función completa
-- corre dentro de la transacción del caller: si CUALQUIER paso falla (stock
-- insuficiente, INSERT inválido, lo que sea), Postgres revierte TODO lo hecho
-- hasta ese punto — no hace falta compensación manual, no puede quedar un
-- estado a mitad de camino.
--
-- Idempotencia: `p_idempotency_key` (nuevo, `orders.idempotency_key` UNIQUE).
-- Si ya existe una orden con esa key, la función la devuelve tal cual (sin
-- reservar stock de nuevo, sin duplicar la orden) — protege contra
-- doble-submit del form (doble click) o un retry del cliente después de un
-- timeout de red donde el request original sí había llegado a commitear.
--
-- SECURITY INVOKER (no DEFINER): el único caller es `createAdminClient()`
-- (service_role), que ya bypassa RLS y tiene privilegios completos sobre
-- estas tablas — igual criterio que `reserve_stock` (migración 00006).
-- DEFINER escalaría privilegios sin necesidad real (anti-patrón).
-- ============================================================================
...
COMMENT ON COLUMN public.orders.idempotency_key IS
  'UUID generado por el cliente al montar /checkout, reenviado en cada submit del form. create_order_from_cart() lo usa para no duplicar la orden ante un doble-submit o retry. NULL en órdenes creadas antes de esta migración.';
...
-- Mismo criterio de exposición que reserve_stock: solo service_role, nunca
-- expuesta al browser (aunque anon/authenticated pudieran llamarla, las
-- tablas que toca no les dan permiso de escritura vía RLS — defensa doble).
REVOKE ALL ON FUNCTION public.create_order_from_cart(uuid, text, jsonb, jsonb, jsonb, jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.create_order_from_cart(uuid, text, jsonb, jsonb, jsonb, jsonb) FROM anon;
REVOKE ALL ON FUNCTION public.create_order_from_cart(uuid, text, jsonb, jsonb, jsonb, jsonb) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.create_order_from_cart(uuid, text, jsonb, jsonb, jsonb, jsonb) TO service_role;
...
-- ============================================================================
-- BEGIN;
-- DROP FUNCTION IF EXISTS public.create_order_from_cart(uuid, text, jsonb, jsonb, jsonb, jsonb);
-- DROP INDEX IF EXISTS public.orders_idempotency_key_key;
-- ALTER TABLE public.orders DROP COLUMN IF EXISTS idempotency_key;
-- COMMIT;
-- Nota: `lib/checkout/orders.ts` tiene que volver a la versión multi-paso
-- (git revert del commit de este fix) ANTES de aplicar este rollback, o el
-- checkout queda roto (llamaría a una función que ya no existe).
