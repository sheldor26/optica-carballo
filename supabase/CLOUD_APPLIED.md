...
| `seeds/19_vulk_yamain_frame_shape_cat_eye.sql` | ⏳ pendiente | UPDATE attributes.frame_shape de Vulk Yamain de `oval` → `cat_eye`. Solo en mi DB local, NO sync a ML (ML sigue como "Ovalado"). Aplica product-level a las 3 variantes. Impacto: filtros del catálogo, comparador, ficha técnica, recomendador IA. |
