...
| `20260528180000_newsletter_subscribers.sql` | ⏳ pendiente | Tabla `newsletter_subscribers` para captura de leads (single opt-in iter 1). Email UNIQUE + CHECK formato + CHECK lowercase. RLS estricto: sin policies para anon/authenticated → solo `service_role` accede (todas las ops pasan por `createAdminClient()` desde server actions). Trigger `updated_at`. **Aplicar antes del primer signup en producción** — sin esta tabla `subscribeNewsletterAction` devuelve error. |
