...
| `seeds/18_fix_vulk_brand_image_path.sql` | ⏳ pendiente | UPDATE correctivo: `brands.includes_image_path = 'vulk-estuche-franela.jpg'` (solo filename, sin prefijo). Frontend ahora construye URL apuntando al bucket `brands-shared` (no `products`). Imagen verificada en producción vía curl HTTP 200. Aplicar tras commit del fix de código que cambió `buildGalleryImages()` para apuntar al bucket correcto. |
