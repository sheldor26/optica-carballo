...
| `seeds/17_vulk_brand_includes_image.sql` | ⚠️ aplicado pero IMAGEN NO ENCONTRADA en bucket (2026-05-30) | UPDATE brand Vulk con `includes_image_path='brands-shared/vulk-estuche-franela.jpg'`. SQL aplicado correctamente. PERO: la imagen no está en bucket con esa path (curl devuelve HTTP 400). Founder confirmó "todo subido" pero la URL pública del bucket no resuelve. Posibilidades: (1) subida a otra path, (2) carpeta `brands-shared/` no permite reads públicos, (3) nombre distinto. Pendiente confirmación path real del founder para hacer UPDATE correctivo del path en brands. |
