...
| `seeds/15_rusty_yau_mblue_revo_green_pol.sql` | ⏳ pendiente | Variante nueva Rusty Yau SKU 126082 (MBLUE/R. GREEN POL - YELLOW: armazón Azul Mate + lentes Verde Espejada Polarizada + par yellow). Datos auto-extraídos vía `/api/admin/ml-import-preview/MLA2707007110` (price 103902 ARS, stock 3, mlitem confirmado). + UPDATE al producto Rusty Yau para reflejar el **adaptador interno extraíble para lentes graduadas** (característica del modelo común a TODAS las variantes, omitida en seed 10 original): agrega `attributes.prescription_adapter=true` + nuevo include `adaptador-interno-lentes-graduadas` + nuevo callout "¿Usás receta?" + párrafo dedicado en description larga. **Requiere founder subir 2 fotos al bucket ANTES de aplicar**: `rusty-yau/06-revo-green-lateral.jpg` y `rusty-yau/07-revo-green-frontal.jpg`. |
| `seeds/16_vulk_yamain.sql` | ⏳ pendiente | NUEVO producto Vulk Yamain (sol, ovalado, mujer). 3 variantes a cargar (founder NO sube por ahora 2 variantes marrones por problemas color fabricante): SKU 127100 CRY/CSV01 (transparente, gris degradé, NO POL, $79.832,39, stock 6), SKU 127101 MBLK/G3237 (negro mate, gris oscuro, NO POL, $79.832,39, stock 3), SKU 127104 SBLK/SG91 POL (negro brillo, gris degradé POLARIZADA, $86.228, stock 1 — viene de listing ML SEPARADO MLA2026217358). Producto-level: G-Flex + Flexo, policarbonato UV400, oval, female, 30.9g, talle Large, medidas 146/58/55/16/145mm, prescription_adapter=false, has_polarized_variant=true. Variante 127104 sobrescribe lens_treatment con ['polarized','uv400']. Datos auto-extraídos vía `/api/admin/ml-import-preview` para ambos listings. **Requiere founder subir 7 fotos al bucket ANTES de aplicar** en `vulk-yamain-sol/`: 01-cry-lateral.jpg, 02-cry-frontal.jpg, 03-mblk-lateral.jpg, 04-mblk-frontal.jpg, 05-sblk-lateral.jpg, 06-sblk-frontal.jpg, 07-medidas.jpg. |
| `seeds/17_vulk_brand_includes_image.sql` | ⏳ pendiente | UPDATE brand Vulk con `includes_image_path='brands-shared/vulk-estuche-franela.jpg'` + `includes_image_alt`. Asocia la imagen brand-wide del kit Vulk (estuche+franela+stickers) que se renderiza automáticamente al final de la galería de PDP de TODOS los productos Vulk. **PREREQUISITOS**: (1) migration `20260530200000_brands_includes_image.sql` aplicada, (2) founder subió `brands-shared/vulk-estuche-franela.jpg` al bucket `products`. |
