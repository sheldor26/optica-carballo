/**
 * Operaciones sobre un pedido: cambiar estado y dar de alta el envío.
 *
 * Vive acá y no adentro de `app/admin/pedidos/actions.ts` porque ahora hay dos
 * puertas que dan a lo mismo: el panel admin de la tienda y el sistema de
 * gestión de la óptica (OptiSys), que las llama por `/api/optisys/*`.
 *
 * NO ES UN ARCHIVO `'use server'`, y eso es a propósito. En un archivo así todo
 * lo que se exporta queda accesible desde afuera como endpoint; estas funciones
 * no verifican quién llama, así que exportarlas de ahí sería dejar el alta de
 * envíos abierta. La verificación la hace cada puerta: `requireAdmin` en el
 * panel, la clave compartida en las direcciones.
 *
 * Tampoco revalida rutas ni sabe de Next: de eso se encarga quien llama.
 */
 ...
 *
 * Flujo: UPDATE orders.status (el trigger DB registra el evento en el timeline
 * + setea paid_at/shipped_at/delivered_at) → (opcional) adjunta la nota al
 * evento recién creado → manda email al cliente si el estado es notificable.
 * El email es best-effort: si falla, el cambio de estado YA quedó persistido y
 * no se revierte.
 */
export async function cambiarEstadoDePedido(input: {
  orderId: string;
  ...
  if (
    ...
    const result = await sendOrderStatusUpdateToCustomer({
      ...
      data: {
        ...
        status: newStatus as 'preparing' | 'reviewed' | 'shipped' | 'delivered',
 ...
 * lo tratamos como éxito (`alreadyImported`). Por eso REINTENTAR un alta que
 * falló funciona, pero pedir un envío NUEVO para el mismo pedido no: Correo
 * devuelve el que ya tiene.
 *
 ...
 */
export async function generarEnvioDePedido(
  orderId: string,
