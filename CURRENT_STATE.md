...
🔴 **SEED ESCRITO, PENDIENTE APLICAR: Rusty Peating RECETA (2026-07-14, seed 84)**. `supabase/seeds/84_rusty_peating_receta.sql` completo + commiteado, pero **NO aplicado al cloud** — el MCP Supabase se desconectó a mitad de la carga y siguió caído todo el turno. Flow de norma cumplido (catalog-loader molde Zinz 78 + seo-strategist). name **"Rusty Peating Carey"** (sin "Receta" — la categoría ya la renderiza en el title). Cuadrado UNISEX G-Flex carey liviano 18,9g, bisagras metálicas (hinge_system="metalica"), 2 var: SDEMI carey brillo SKU 129251 MLA3604846462 (primary) / MDEMI carey mate SKU 129254 MLA3604846464, ambos $79.790, stock 2 c/u. Medidas 143/52×49/19/133. **NO bluecut**. **SEO**: primaria CAREY `anteojos carey` 390 (carril libre — Zinz/Spell/Katleen/Strewn ya ocupan forma/género/transparente); cross-link Peating↔Zinz↔Spell; bloque en SEO_STRATEGY. Fotos ya en bucket `rusty-peating-receta/` (900×442, naming inconsistente respetado), scale 1.1/1.0 en `image-scale-overrides.ts` (código commiteado, sin efecto hasta deploy). Peating de SOL existe en ML pero no cargado → sin cross-link modalidad hoy. Docs: seed 84, CLOUD_APPLIED (marcado 🔴 no aplicado), SEO_STRATEGY. typecheck OK. **⚠️ SIN pushear tampoco** (código listo, esperando aplicar antes de commitear+push juntos, o se pueden separar). **⬜ PRÓXIMO PASO EXACTO**: founder reconecta el conector MCP de Supabase (o aplica el SQL del seed 84 manualmente) → avisar para aplicar vía `execute_sql` → verificar (2 variantes, stock 4, 5 imágenes, 1 primaria) → HTTP 200 fotos → PDP live → scale en grid → actualizar CLOUD_APPLIED a ✅ + PRODUCTS_INVENTORY → commit+push.
