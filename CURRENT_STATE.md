...
🟡 **Carga Rusty Misty RECETA (armazón óptico, redondo unisex, talle chico) — SEED PREPARADO, NO aplicado (faltan fotos)** (2026-06-02). Seed `48_rusty_misty_receta.sql` escrito (producto + 3 variantes), **NO ejecutado**. Versión óptica del Misty sol (mismo frame). MLA1388546107 multi-variante:
- MBLK 125734 (negro mate) var 183827659731 stock 7 (primary); 373K 125731 (marrón) var 183827659729 stock 4; 0292 125730 (caramelo) var 179932980765 stock 3. Todas $82.745,69.
- Receta → convención: `lens_compatibility ["monofocal","bifocal","progresivo","multifocal"]` + `hinge_system "flex"` (NO lens_material/treatment/polarized). `size_fit "chico"` (talle pequeño), gender unisex, 17,8g, medidas 132/44×42/22/145.
- **⛔ BLOQUEANTE — fotos**: `rusty-misty-receta/` vacío, el founder no pasó nombres. El seed tiene un TODO para el INSERT de imágenes. Apenas suba: completar imágenes + aplicar MCP + scale + docs.
- **⚠️ A confirmar**: `frame_shape="redondo"` (igual que Misty sol, mismo frame) pero el título ML dice "ovalados".
