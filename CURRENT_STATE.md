...
🟡 **Incident Password Reset "Auth session missing" — FIX PUSHEADO, FOUNDER PENDIENTE VERIFICAR** (2026-05-29). Founder probó reset password tras config Vercel+Supabase, recibió email con URL correcta (dominio prod), llenó form de nueva contraseña → error "El link expiró o ya fue usado". Causa: `passwordResetForEmail.redirectTo` apuntaba directo a `/recuperar-clave/restablecer` sin pasar por `/auth/callback`. La página renderizaba el form pero nunca hacía `exchangeCodeForSession(code)`. Cookie `code_verifier` quedaba sin usar. Form submit fallaba con "Auth session missing!" porque `updateUser({password})` no tenía sesión válida. NO era Safe Links de Hotmail (PKCE flow protege contra prefetch). Fix: `redirectTo` ahora va a `/auth/callback?next=/recuperar-clave/restablecer`. Founder pendiente: tras Vercel deploy auto, pedir reset password de nuevo y verificar que ahora funciona.
