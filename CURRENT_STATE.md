...
🔴 **4 SEEDS ESCRITOS, PENDIENTES APLICAR: Rusty Peating RECETA+SOL + Vulk Dieven RECETA+SOL (2026-07-14, seeds 84+85+86+87)**. El MCP de Supabase sigue sin aparecer disponible en esta sesión pese a que el founder lo reconectó (probable que haga falta reiniciar la sesión de Claude Code, no solo el conector). Los 4 seeds están completos, con flow de norma cumplido, typecheck OK y commiteados — listos para aplicar todos juntos apenas el MCP responda.

- **Rusty Peating** (receta `84` + sol `85`): ver detalle abajo, sin cambios desde el registro anterior.
- **Vulk Dieven receta** (`86_vulk_dieven_receta.sql`, slug `vulk-dieven-receta`): rectangular bordes anchos unisex G-Flex, 28,5g. name "Vulk Dieven Unisex". 3 var (MBLK/SBLK/L.ROSE), $75.590 c/u. **Fix este turno**: founder renombró la carpeta del bucket (ahora `vulk-dieven-receta/`) → storage_path y scale overrides corregidos.
- **Vulk Dieven sol** (`87_vulk_dieven_sol.sql`, slug `vulk-dieven` sin sufijo): rectangular unisex, mismo armazón físico que el receta (medidas idénticas). name "Vulk Dieven" (sin "Unisex" — patrón limpio del catálogo sol). 3 var: MBLK/S10 GREY POL negro mate SKU 116085 $76.156,15 stock 2 (primary, polarizada); SBLK/SG91 POL negro brillo SKU 116089 $76.156,15 stock 7 (polarizada); ROSE/BROWN-GREEN rosa translúcido SKU 116087 $69.810,63 stock 8 (⚠️ polarización NO confirmada — `polarized:false`, sin afirmarlo en copy, criterio Bruk 2/3). **Discrepancia resuelta**: título ML decía "Cuadrado" pero se usó `frame_shape="rectangular"` (mensaje explícito del founder + medidas = al receta = mismo armazón). Cross-link sol↔receta automático por slug — engancha apenas ambos estén `is_active=true`. **Bug de infraestructura detectado**: el filtro `/anteojos-de-sol/[brand]/polarizados` NO deriva de variantes en runtime (el comentario del seed 79 Zinz decía que sí, es incorrecto) — Zinz (2/2 pol) probablemente tampoco aparece ahí hoy. Anotado en BACKLOG con causa raíz y 2 opciones de fix.

Docs: seeds 84-87, CLOUD_APPLIED (4 filas 🔴), SEO_STRATEGY (4 bloques), BACKLOG (bug polarizados + gap /cuadrados + cross-link Peating pendiente). **⚠️ SIN pushear**. **⬜ PRÓXIMO PASO EXACTO**: founder reintenta reconectar/reiniciar sesión del MCP de Supabase → avisar cuando las tools aparezcan → aplicar 84+85+86+87 juntos vía `execute_sql` → verificar los 4 (variantes/imgs/primarias) → HTTP 200 fotos → PDP live de los 4 + ambos cross-links visibles (Peating y Dieven sol↔receta) → scale en grids receta y sol → actualizar CLOUD_APPLIED a ✅ + PRODUCTS_INVENTORY → commit+push.

🔴 **Detalle Rusty Peating (referencia, sin cambios este turno) — seeds 84+85**. Founder reconectó el conector de Supabase pero las tools del MCP **todavía no aparecen disponibles** en esta sesión (ToolSearch sin resultados en 3 intentos) — probable que haga falta reiniciar la sesión del conector (mismo patrón de runs previos: reconectar solo no alcanza, a veces hace falta un ciclo completo de desconectar/reconectar o reiniciar Claude Code).

- **Vulk Dieven receta** (`86_vulk_dieven_receta.sql`, slug `vulk-dieven-receta`): rectangular bordes anchos unisex G-Flex, 28,5g, medium, bisagras plásticas reforzadas. **PRIMER rectangular Vulk en receta** (los otros 3 son Rusty). 3 var: MBLK negro mate SKU 116080 MLA1895280107 (primary) / SBLK negro brillo SKU 116083 MLA1895280109 / L.ROSE rosa pálido SKU 116090 MLA3604894350, todos $75.590, stock 2 c/u. SKUs del founder. name **"Vulk Dieven Unisex"** (SEO: presupuesto de title ~20 char + "rectangular" ya es primaria de R-CY02; primaria real = `anteojos recetados` 720). Fotos ya en `vulk-dieven/` (900×442, HTTP 200, naming inconsistente respetado). Existe Dieven SOL en ML pero no cargado → sin cross-link hoy (BACKLOG).
- **Rusty Peating receta+sol**: ver detalle en CLOUD_APPLIED, sin cambios desde el registro anterior.

Los 3 seeds con flow de norma cumplido (catalog-loader + seo-strategist), typecheck OK, commiteados. Docs: seeds 84/85/86, CLOUD_APPLIED (3 filas 🔴), SEO_STRATEGY (3 bloques), BACKLOG (gap /cuadrados + cross-links sol pendientes). **⚠️ SIN pushear**. **⬜ PRÓXIMO PASO EXACTO**: founder reintenta reconectar el MCP de Supabase (posiblemente necesita reiniciar sesión/Claude Code) → avisar cuando las tools aparezcan → aplicar 84+85+86 juntos vía `execute_sql` → verificar los 3 (variantes, imgs, primarias) → HTTP 200 fotos → PDP live de los 3 (+ cross-link Peating sol↔receta visible) → scale en grids → actualizar CLOUD_APPLIED a ✅ + PRODUCTS_INVENTORY → commit+push. Par completo (sol + receta) del mismo armazón físico Rusty Peating, cuadrado unisex G-Flex, 18,9g, medidas 143/52×49/19/133 — **ninguno de los dos aplicado al cloud**: el MCP Supabase se desconectó a mitad de la carga del receta y siguió caído todo el turno (revalidado varias veces, confirmado que sigue caído). Ambos con flow de norma cumplido (catalog-loader + seo-strategist), typecheck OK, código commiteado.

- **RECETA** (`84_rusty_peating_receta.sql`, slug `rusty-peating-receta`): name **"Rusty Peating Carey"**. 2 var carey: SDEMI carey brillo SKU 129251 MLA3604846462 (primary) / MDEMI carey mate SKU 129254 MLA3604846464, ambos $79.790, stock 2 c/u. hinge_system="metalica". **NO bluecut**. SEO primaria CAREY `anteojos carey` 390 (carril libre — Zinz/Spell/Katleen/Strewn ocupan forma/género/transparente). Fotos en `rusty-peating-receta/` (900×442, naming inconsistente respetado).
- **SOL** (`85_rusty_peating_sol.sql`, slug `rusty-peating` SIN sufijo): name **"Rusty Peating"** (sin "cuadrado" — esa forma ya es primaria de Zinz sol). 2 var negras, AMBAS polarizadas: MBLK/S10 negro mate SKU 960100 MLA1504687501 $82.127 stock 2 (primary) / SBLK/DRT03 negro brillo degradé gris SKU 960103 MLA1440111809 $81.798,49 stock 1. SEO primaria = branded + carril COLOR de soporte (`negros cuadrados`) — NO "cuadrados" (Zinz) ni "polarizados" como primaria (Terdey). Fotos en `rusty-peating/` (sin sufijo -sol, 900×442, HTTP 200 verificado).

**Cross-link sol↔receta**: automático por convención de slug (`rusty-peating` ↔ `rusty-peating-receta`, verificado en `lib/catalog/queries.ts`) — sin código nuevo, engancha solo apenas ambos existan `is_active=true` en la DB. Scale 1.1/1.0 en ambos (`image-scale-overrides.ts`, commiteado). **Gap SEO detectado y anotado en BACKLOG**: no existe faceta `/anteojos-de-sol/cuadrados`. Docs: seeds 84+85, CLOUD_APPLIED (ambos 🔴), SEO_STRATEGY (2 bloques), BACKLOG. **⚠️ SIN pushear** (esperando aplicar antes, o separar). **⬜ PRÓXIMO PASO EXACTO**: founder reconecta el conector MCP de Supabase → aplicar 84 y 85 juntos vía `execute_sql` → verificar ambos (2 variantes c/u, 5 imgs c/u, 1 primaria c/u) → HTTP 200 fotos → PDP live de las 2 + cross-link visible en ambas → scale en grid receta y sol → actualizar CLOUD_APPLIED a ✅ + PRODUCTS_INVENTORY → commit+push.
