...
🔵 **PRÓXIMO: retomar carga de productos (2026-06-11)**. Founder quiere seguir subiendo productos nuevos. Último seed aplicado = `58_rusty_esvep`. Playbook aceitado (stock=ML, primaria=perfil, polarizadas→/polarizados, scale comparado con el grid, medidas/peso/forma/género, verificación PDP+grid, update PRODUCTS_INVENTORY + CLOUD_APPLIED). **⚠️ El conector Supabase (MCP) se desconectó** — los seeds se aplican por ahí → reconectar (o el founder aplica) cuando lleguemos al paso de aplicar. Próximo input esperado del founder: link de ML del 1er producto + fotos. Recordatorios activos en memoria: [[stock-siempre-ml]], [[feedback-grid-primaria-perfil]], [[feedback-c2-vrast-no-mencionar]].
