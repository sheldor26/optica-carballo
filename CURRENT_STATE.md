...
**Próximo paso EXACTO**: ninguno urgente. El founder eligió 1, 2 y 6 de la lista priorizada — los 3 ya están implementados y verificados (ver entry de abajo). El resto de la lista (Club Carballo, post-compra que retiene, quiz de texto, alertas sin login, virtual try-on AR) queda disponible para un próximo ciclo del loop cuando el founder quiera dispararlo — no está en BACKLOG.md porque no fue "elegido para hacer", fue "generado como propuesta"; si el founder quiere retomar alguno, está documentado en este mismo archivo, sección de arriba.
...
**Pendiente real: el CÓDIGO todavía no está commiteado ni deployado** — la migración por sí sola no activa nada. Falta: commit + push de los 3 ítems (por ahora solo están en el working tree local), y una vez deployado en Vercel, la entrada nueva de `check-prescription-expiry` en `vercel.json` recién ahí queda registrada como cron real. `CRON_SECRET` ya existe en Vercel (lo usan los otros 3 crons), no hace falta uno nuevo.
