...
~~#1~~ ✅. ~~#2 (APIs admin sin PIN)~~ ✅. ~~#3 (face-shape sin rate limit)~~ ✅. ~~#4 (sitemap/metadata sin filtrar stock)~~ ✅. ~~#5 (placeholders `[A CONFIRMAR]` en FAQ/legales)~~ ✅. ~~#6 (checkout sin receta obligatoria)~~ ✅. ~~#7 (`meta_title` ignorado)~~ ✅ **hecho en esta misma sesión** (ver abajo). Próximo paso real: hallazgo #8 (reserva de stock y creación de orden no son atómicas) — el último 🟠 alto que queda en `AUDIT_2026-08-01.md`.
