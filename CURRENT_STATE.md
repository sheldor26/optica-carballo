...
🟡 **PROBANDO: logos de marca en el mega-nav** (2026-06-02, commit `eb659a7`). Founder decidió probar logos (Opción 2) — argumento válido: los logos son wordmarks, el nombre se lee igual. Implementado: `MegaItem.brandLogo` opcional + `BRAND_LOGOS` map en mega-nav.ts; `desktop-nav.tsx` renderiza el logo (h-5, object-contain) con inversión vía `shouldInvertLogo` (Reef es `-light` → brightness-0 sobre fondo claro). Las 3 columnas de marca (sol/receta/marcas). **Mobile sigue con texto** (ignora brandLogo). Logos CDN 200, home 200, tsc OK. Mi recomendación previa era texto+logo-wall, pero es prueba reversible. **⬜ Esperando veredicto visual founder** (mirar: alineación con anchos dispares, Paula Cahen D'Anvers wordmark largo, Reef invertido, tamaño h-5). Si no convence → revertir a texto. Founder preguntó si reemplazar los nombres de marca (columna "POR MARCA") por logos. Las 5 marcas activas tienen logo SVG (`brand-logos/*`, Reef es light, resto dark). **Mi recomendación: NO en el nav** (inconsistencia visual entre logos de distinto aspect-ratio, peor escaneo, peor SEO/anchor, Reef necesitaría inversión sobre fondo claro). **Contrapropuesta**: logo wall en home o /marcas + header de marca (donde el logo sí luce). **⬜ Esperando decisión founder**: texto+logo-wall vs logos en el menú. Sin código tocado.
