...
🟡 **Carga Rusty Eslav (sol, deportivo envolvente, 2 variantes) — SEED PREPARADO, NO aplicado (bloqueado por fotos)** (2026-06-02). Seed `46_rusty_eslav.sql` escrito y listo, **NO ejecutado todavía**. Data completa levantada de ML + founder:
- MBLK/S10 POL — SKU 126060 — MLA1423923051 — $98.350,02 — stock 14 (activa, primary).
- MBLUE/R.GREEN POL — SKU 126062 — MLA1856270534 — $103.902 — stock 0 (pausada).
- **AMBAS polarizadas** → `lens_treatment ["uv400","polarized"]` (sí entra a /polarizados). G-Flex, bisagras plásticas, policarbonato UV400 cat3, envolvente, unisex, deportiva, base 8. Incluye lentes amarillas intercambiables (`interchangeable_lenses:true`) + adaptador receta (`prescription_adapter:true`). Medidas 138/75×51/12/120.
- **⛔ BLOQUEANTE — fotos**: la carpeta `rusty-eslav/` está VACÍA. Los archivos del screenshot (`MBLK-S10-YELLOW---PERFIL/FRENTE`, `MBLUE-R-GREEN-POL-YELLOW-PERFIL/FRENTE`) NO existen. **CUIDADO**: hay fotos de nombre casi idéntico en `rusty-sotion/` pero son del producto **Sotion existente** (verificado: sus `product_images` las referencian) — NO usarlas para Eslav. El founder debe subir las 4 fotos (+ medidas) a `rusty-eslav/`.
- **Posible duplicación a chequear**: Eslav (126060/126062, envolvente polarizado deportivo con lentes amarillas) se parece MUCHO a Sotion (mismo tipo, variantes con códigos casi iguales MBLK/S10 + MBLUE/R-GREEN). ¿Son modelos distintos o el founder está duplicando Sotion? Confirmar antes de aplicar.
- **Próximo paso**: founder sube fotos a `rusty-eslav/` + confirma que Eslav ≠ Sotion → verificar HTTP 200 → aplicar seed MCP → scale 1.15/1.0 → docs.
