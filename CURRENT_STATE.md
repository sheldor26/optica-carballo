...
### Decisión: loop de mejora continua con Codex + Antigravity (sesión posterior, misma fecha)

El founder pidió trabajar en sinergia constante con Codex y Antigravity: después de cerrar trabajo grande, preguntarse activamente cómo mejorar velocidad, features, conversión al plan pago, retención/"adictividad" y estética — investigando tendencias modernas de apps y benchmarking de ópticas online (Argentina + Europa/EEUU), no solo con conocimiento interno. Guardado como preferencia permanente en memoria (`feedback-sinergia-ia-mejora-continua`).

Decisiones cerradas vía pregunta explícita al founder: (1) el loop **investiga y propone**, no implementa código autónomamente — cualquier cambio pasa por su aprobación antes de tocar el sitio (dado que hay checkout/pagos reales activos); (2) el founder **dispara el loop manualmente** cuando quiere una pasada nueva, no corre en cadencia automática.

Corrección importante: había memoria de la sesión anterior de que "Antigravity no tiene CLI headless". Al re-verificar (WebSearch + test directo) resultó falso — Antigravity CLI (`agy`) reemplazó a Gemini CLI cuando Google cortó el acceso free/Pro/Ultra de este último el 2026-06-18, y ya está instalado y autenticado en la máquina del founder. Memoria corregida (`project-antigravity-cli-agy`, la vieja `project-antigravity-sin-cli` se borró). Codex CLI (`codex`, v0.146.0) también confirmado disponible. `agy` tardó 3 intentos en dar una respuesta útil (necesita el prompt como valor de `--print`, no por stdin; y con `--sandbox`/exploración libre del repo termina auto-denegándose comandos de shell en headless sin avisar) — la vuelta que funcionó fue pegarle el código relevante directo en el prompt en vez de dejarlo explorar. Detalle en la memoria del CLI.

### Primer ciclo del loop de mejora — completado, síntesis consolidada

4 fuentes independientes (3 investigaciones web + Codex CLI grounded en el código real; Antigravity CLI dio una segunda lectura solo-estética después de resolver problemas de invocación). Full detail en el mensaje de cierre al founder de esta sesión — resumen acá:

**Convergencia fuerte entre fuentes independientes** (mayor confianza en estas 3): (1) recordatorios honestos ligados a datos reales (vencimiento de receta, reposición de descartables) como palanca de retención sin dark patterns; (2) mostrar garantía/devolución con números explícitos en la ficha de producto (no enterrado en términos); (3) dirección estética "editorial/warm" en vez de "tech dark premium" — Codex y Antigravity llegaron a la misma conclusión de forma independiente (Antigravity la nombró "Warm Tactile Editorial").

**Ya construido y más fuerte que la competencia** (no tocar, solo explotar mejor): el analizador de forma de cara con Claude Vision (`face-shape-analyzer.tsx`) es más sofisticado que los quizzes de la mayoría de competidores internacionales — falta cerrar el loop hacia el catálogo filtrado por forma.

**Próximo paso EXACTO**: ninguno urgente. El founder eligió 1, 2 y 6 de la lista priorizada — los 3 ya están implementados, deployados y verificados (ver entries de abajo). El resto de la lista del primer ciclo (Club Carballo, post-compra que retiene, quiz de texto, alertas sin login, virtual try-on AR) más las propuestas nuevas del segundo ciclo (ver entry "Segundo ciclo" más abajo) quedan disponibles para cuando el founder dispare el próximo ciclo — no están en BACKLOG.md porque no fueron "elegidas para hacer", fueron "generadas como propuesta".

### Segundo ciclo del loop de mejora — completado, síntesis consolidada (misma fecha, sesión posterior a los 3 ítems)

5 fuentes independientes: 3 investigaciones web (tendencias modernas de apps 2026, competencia óptica online Argentina, competencia óptica online internacional) + Codex CLI (auditoría técnica grounded en el código real, post-3-ítems) + Antigravity CLI/`agy` (segunda opinión estética, con el código de los 2 componentes nuevos pegado directo en el prompt).

**Convergencia fuerte entre fuentes independientes** (mayor confianza):
1. **Cuotas de Mercado Pago visibles en la ficha de producto, no solo en el checkout** — converge en la investigación de tendencias 2026 Y en la de competencia internacional (GlassesUSA/Splitit citan +15% AOV, -10% abandono de carrito con este mismo cambio). Esfuerzo bajo (copy/UI, sin lógica nueva).
2. **Cerrar el loop del recomendador de forma de cara hacia el catálogo filtrado** — converge en 3 fuentes ahora (primer ciclo del loop, Codex, y competencia internacional vía GlassesUSA "Pairfect Match AI", que documenta +21% conversión / -12% devoluciones con un mecanismo equivalente). Ya existe la IA (`face-shape-analyzer.tsx`), falta la conexión al catálogo — no es construir de cero.
3. **Garantía/devolución visible como trust signal** — ya implementado hoy (ítem 1). Confirmado retroactivamente por 2 fuentes independientes: competencia argentina ("nadie lo hace bien, Carballo lo tiene resuelto por dentro pero no lo explotaba por fuera") y competencia internacional (GlassesUSA/Specsavers lo comunican agresivo). Valida que la decisión de hoy fue acertada.

**Corrección importante a un hallazgo de la investigación**: el research de competencia argentina reportó "ningún competidor tiene comparador de productos, sería el primer diferenciador técnico del sector" — pero Óptica Carballo YA TIENE un comparador (`components/compare/compare-bar.tsx`, `compare-table.tsx`, `compare-bar-search.tsx`, wireado en `app/(storefront)/layout.tsx`). Confirmado vivo (no código muerto) antes de escribir esto. Conclusión correcta: no hay que construirlo, hay que visibilizarlo/promocionarlo mejor — es una fortaleza existente sin explotar, no una feature pendiente.

**Bug real encontrado por Codex** (no es una propuesta, es un defecto en lo implementado hoy): `sendPrescriptionReminderEmail` (`lib/emails/send-prescription-reminder-email.ts`) traga cualquier error de Resend en un try/catch silencioso, y el cron (`check-prescription-expiry`) marca `reminder_sent_at` igual, sin verificar que el envío haya tenido éxito — si Resend falla, el recordatorio se pierde sin que nadie se entere y no se reintenta. Mismo patrón que `sendAlertEmail` (preexistente, no es nuevo de hoy), así que es deuda técnica del patrón general, no un error aislado. Anotado en `BACKLOG.md`.

**Lista completa de propuestas nuevas** (esfuerzo/impacto, agrupadas): cuotas MP en PDP y cerrar loop del recomendador (arriba, máxima confianza); reviews de producto con estrellas en el grid; cross-sell "completá tu par" (sol↔receta compatible) extendiendo `RelatedProducts`; "offline wishlist" vía WhatsApp cuando alguien prueba en el local y no compra (Ace & Tate — cero desarrollo, es proceso); segundo par "backup" con descuento (seguro contra pérdida/rotura, no 2x1 genérico); jerarquizar visualmente los 7 trust signals (3 pilares con tarjeta + 4 legales como lista compacta, sugerencia de Antigravity); optimistic UI en favoritos/carrito; View Transitions API grid→PDP; dirección estética "editorial cálido" (colores arena/hueso, serif en títulos) en vez de "SaaS genérico". Full detail con fuentes en el mensaje de cierre al founder de esta sesión.

**Descartado explícitamente por las fuentes** (no perseguir, con razón documentada): Home Try-On físico (Warby Parker lo discontinuó por costo en 2025), AR facial 3D completo (caro, técnicamente exigente, ni Warby Parker lo resolvió bien), fidelidad por puntos/niveles (sin frecuencia de compra que lo sostenga), suscripción tipo "Prime" para armazones (sin lógica de negocio salvo lentes de contacto, que no vendemos aún), BNPL internacional (Klarna/Afterpay no operan en Argentina).

**Próximo paso original**: ninguno urgente — el founder tiene que elegir de esta lista nueva qué shippear (como hizo con 1, 2, 6 del primer ciclo). Superado por lo de abajo: eligió los 2 primeros items de la lista al toque.

### Implementados: cuotas MP en PDP + link "ver todos" en el recomendador (2do ciclo, ítems 1 y 2)

Founder eligió los 2 ítems de mayor confianza de la síntesis del segundo ciclo. El audit obligatorio (regla #14) reveló que **los dos ya estaban ~95% construidos** — ninguno era una feature nueva.

- **Cuotas de Mercado Pago en la PDP + grid**: ya existía `lib/site/installments.ts` (single source of truth, con comentario explícito de compliance: mostrar "cuotas sin interés" sin tener la promo activa y pagada en el panel de MP es publicidad engañosa, Ley 24.240 art. 8 — regla escalada en `MISTAKES.md` 2026-06-01) y ya estaba consumido en `product-price-block.tsx` (PDP) y `product-card.tsx` (grid), apagado por un flag `INSTALLMENTS_ENABLED = false`. No era una tarea de código — era una pregunta de HECHO que solo el founder puede contestar (¿la promo está activa en su panel de MP ahora?). Se le preguntó explícitamente vía `AskUserQuestion`, confirmó que sí está activa → se cambió `INSTALLMENTS_ENABLED` a `true` (1 línea). Verificado en vivo: "3 cuotas sin interés de $X" visible en las 46 cards del grid de `/anteojos-de-sol` y en la PDP de `Vulk Biller`.

- **Link "ver todos los modelos de esta forma" en el recomendador de rostro**: el recomendador (`face-shape-analyzer.tsx` + `recommended-products-grid.tsx`) YA mostraba hasta 5 productos matcheados por forma de cara (`fetchRecommendedProducts`, `DEFAULT_LIMIT = 5`), con fallback a link de catálogo filtrado cuando no había ningún match. El único hueco real: cuando SÍ había productos (el caso común), no había forma de ver los que quedaban afuera del top 5. Se agregó un link "Ver todos los modelos de esta forma →" al final del grid con resultados, mismo patrón/href que ya usaba el `EmptyFallback` (`/anteojos-de-sol?forma=...`). Cambio de 8 líneas, reusa imports ya presentes (`Link`, `ArrowRight`). `pnpm typecheck` limpio; no se verificó en vivo con una foto real (hubiera significado gastar una llamada real a Claude Vision por un cambio JSX aislado que reusa un patrón ya probado en el mismo archivo — desproporcionado).
...
**Hallazgo clave del audit (regla 14)**: `orders.customer_dni` ya existía en el schema (`20260528114114`) y el Facturador YA la lee — solo el checkout nunca la llenaba. Conectar el checkout activa una integración que ya estaba armada del otro lado.

**Implementado**: campo "DNI o CUIT" obligatorio en `components/checkout/checkout-page.tsx` (valida 7-11 dígitos client-side, gate en el botón "Confirmar pedido"), zod schema en `lib/checkout/actions.ts` (misma validación server-side, normaliza guiones/puntos), se prellena con el DNI/CUIT del pedido más reciente del user (`fetchLastUsedDni()` en `lib/orders/queries.ts` — no retipear en compras siguientes), persiste vía `createOrderFromCart` → RPC `create_order_from_cart` (migración `20260804000000_create_order_from_cart_customer_dni.sql`, agrega `customer_dni` al INSERT, no toca schema). Visible en el admin (`app/admin/pedidos/[id]/page.tsx`) — no en el detalle del cliente (mismo criterio que nombre/email/teléfono, ya no se mostraban ahí).

**Verificación**: typecheck + lint limpios. Migración probada en local (Docker + `supabase db reset`, con el workaround ya documentado del INSERT hardcodeado de `facturador_staff_rls.sql` que rompe reset desde cero) + llamada real a la RPC con fixtures mínimos dentro de una transacción con `ROLLBACK` — confirmó que `customer_dni` persiste bien, sin dejar datos de prueba. Aplicada a cloud vía MCP + verificada (`pg_get_functiondef` confirma el campo en la función activa). **No se hizo click-through real en el navegador**: `.env.local` de este proyecto apunta a Supabase de PRODUCCIÓN (no local), así que un test end-to-end real del checkout tocaría stock/órdenes reales — se evitó a propósito.

**Próximo paso EXACTO**: que el founder haga una compra de prueba (o la próxima venta real) y confirme que el campo aparece, valida bien, y el DNI llega correctamente al Facturador de escritorio.
...
- **Ítem 1 — Garantía legal explícita en trust signals de la PDP**: `components/product/product-trust-signals.tsx` suma un 7mo signal "Garantía legal 1 año" (ícono `Scale`, distinto de `ShieldCheck` que ya usa `ProductIncludes` para la garantía del fabricante — son dos regímenes legales distintos, ambos verificados, sin contradicción). Redacción revisada por `argentine-ecom` reusando texto ya verificado de `/politica-de-devolucion` (art. 11 Ley 24.240). Grid pasó a `lg:grid-cols-4` para que 7 ítems cierren parejo. Verificado en vivo.

- **Ítem 2 — Banner de recomendación por WhatsApp con foto**: nuevo `components/catalog/whatsapp-photo-banner.tsx`, copy y composición diseñados con `conversion-optimizer` (dos caminos sin jerarquía implícita: quiz automático vs. WhatsApp humano). **Hallazgo importante durante la implementación**: el plan original apuntaba a `category-index-page.tsx`, pero ese componente es **código muerto** — la ruta real (`/anteojos-de-sol`, `/anteojos-de-receta`) usa `CategoryFilteredPage`/`CategoryCatalogView`, que no tenía ningún banner de descubrimiento. Se revirtió el cambio en el archivo muerto y se implementó en el componente real, condicionado a `!anyFilterActive` (solo en la vista sin filtrar). Ver `LEARNINGS.md` — verificar en el browser antes de confiar en qué componente lee un audit de código.

- **Ítem 6 — Recordatorio de vencimiento de receta**: gate obligatorio con `optical-expert` ANTES de escribir copy (YMYL) — confirmó que "vencimiento" es una inferencia nuestra (emisión + 365 días, no un dato médico), así que el copy final NUNCA afirma "tu receta venció", solo recomienda un control (`lib/emails/send-prescription-reminder-email.ts`). Gate adicional con `argentine-ecom` sobre consentimiento (Ley 25.326) — no hace falta opt-in nuevo (excepción contractual art. 5.2.b), pero sí link de baja propio (reusa el patrón de `product_alerts.unsubscribe_token` con su propio token, no comparte preferencias con las alertas de stock). Infraestructura nueva: migración `20260801190000_prescription_reminder.sql` (`reminder_sent_at` + `reminder_unsubscribe_token` en `prescriptions`), cron `app/api/cron/check-prescription-expiry/route.ts` (mismo esqueleto que `check-alerts`), endpoint `app/api/prescription-reminders/unsubscribe/route.ts`, entrada en `vercel.json`, línea nueva en `/politica-de-privacidad`. **Probado end-to-end contra Postgres local** con 4 fixtures (dentro de ventana → manda; muy reciente → no manda; ya notificada → no re-manda; archivada → no manda) — los 4 casos se comportaron exactamente como se esperaba. Solo cubre anteojos recetados (lentes de contacto no es categoría vendible todavía, ver `BACKLOG.md`).

**Problema durante el testing (sin daño, ver `MISTAKES.md`)**: casi se corrió la prueba del cron contra la base de PRODUCCIÓN — `.env.local` apunta a cloud, no a local, y no fue obvio hasta que la query falló por una columna que no existe en cloud. Se corrigió arrancando el dev server con overrides explícitos de env vars apuntando a `127.0.0.1:54321`.

**Migración de item 6 APLICADA a cloud 2026-08-01** — founder confirmó "aplicala vos por MCP", pero el MCP de Supabase estaba caído (`net::ERR_FAILED` en 5 intentos, distintos endpoints, incluidos los de solo lectura) y la CLI tampoco tenía sesión (`supabase login` pendiente, requiere el founder). Se le pasó el SQL directo y el founder la corrió a mano en el SQL Editor de Supabase Studio — verificado post-aplicación (columnas con tipo/nullable/default exactos). Registrado en `supabase/CLOUD_APPLIED.md`.

**Código commiteado y pusheado** (`9915744`) — los 3 ítems ya están en `main`, Vercel los toma en el próximo deploy automático. `CRON_SECRET` ya existe en Vercel (lo usan los otros 3 crons), no hizo falta uno nuevo.
...
El founder autorizó explícitamente ("Te doy permiso, aplicala vos por MCP") después de que el classifier de auto-mode bloqueara un primer intento automático (acción irreversible sobre producción — correcto que lo frenara). Apliqué `20260801143222_create_order_from_cart_atomic.sql` a cloud vía MCP `apply_migration`. Verificado post-aplicación: columna `idempotency_key` presente, función `prosecdef=false` (INVOKER confirmado, no DEFINER), 6 parámetros, `GRANT EXECUTE` solo a `service_role`. `get_advisors` (security) sin warnings nuevos atribuibles a esta función — los que aparecen son todos preexistentes (coupons sin policy, extensión vector en public, etc.), ninguno de mi cambio. También agregué `orders.idempotency_key` a mano en `types/supabase.ts` (Row/Insert/Update) y registré la migración en `supabase/CLOUD_APPLIED.md` siguiendo el formato que ya usa el proyecto.

### Fix aplicado: hallazgo #8 — atomicidad de stock+orden (código listo, cloud pendiente de confirmación)

Nueva función SQL `create_order_from_cart(...)` (migración `20260801143222`) que hace TODO (reserva de stock + receta + orden + items + cupón) en una sola transacción — si algo falla, Postgres revierte todo, sin compensación manual. Suma `orders.idempotency_key` para que un doble-submit no duplique la orden. `lib/checkout/orders.ts` reescrito para llamar esta RPC una sola vez; se borró la compensación manual (`revertStock`) que ya no hace falta. Probado exhaustivamente LOCAL (Docker + `supabase db reset` + 5 escenarios directo contra Postgres: caso feliz, replay de idempotencia, stock insuficiente con rollback, atomicidad cruzada entre 2 items, cupón+receta juntos — los 5 dieron el resultado esperado). `pnpm typecheck`/`lint` limpios, regresión en vivo contra cloud sin errores.

**No apliqué la migración a cloud** — es un cambio al corazón del flujo de pagos/stock (regla core #7: verificar dos veces) y preferí seguir el mismo patrón que ya usan las migraciones anteriores del proyecto (founder revisa y aplica, o me confirma que la aplique yo vía MCP). Sin urgencia porque `NEXT_PUBLIC_CHECKOUT_ENABLED=false` en prod, pero bloqueante antes de Fase 2.

**Nota operativa de la sesión**: tuve que levantar Docker Desktop (estaba apagado) para poder probar la migración localmente — la regla dura del proyecto es no aplicar migraciones sin probar local primero. De paso encontré que `supabase db reset` falla desde cero porque la migración `20260706000000_facturador_staff_rls.sql` (ya aplicada a cloud) hardcodea un `user_id` real de `auth.users` que no existe en una DB local fresca — la parkeé temporalmente (rename) para poder resetear, y la restauré al terminar. No la modifiqué. Es una deuda de reproducibilidad local pre-existente, no algo que rompí yo — si en algún momento se quiere `db reset` 100% limpio de nuevo, hay que resolver ese INSERT (ej. envolverlo en un `WHERE EXISTS` o mover el alta del facturador a `seed.sql` en vez de a la migración).

### Fix aplicado: hallazgo #7 cerrado — `meta_title` de producto activado en metadata

Fix chico: `lib/catalog/metadata.ts` (`buildProductMetadata`) agrega `meta_title` al select y lo usa como `title` con fallback al patrón genérico si es null. Verificado en vivo: `rusty-peating` pasó de mostrar el título genérico a mostrar el `meta_title` real del seed (research de keywords que estaba muerto en la DB desde julio). Los 68 productos activos ya tienen `meta_title` cargado — cero casos de fallback para probar, pero es un `??` de una línea, riesgo trivial. `pnpm typecheck` limpio.

### Fix aplicado: hallazgo #6 cerrado — receta obligatoria antes de pagar en checkout, con founder eligiendo el flujo

Le pregunté al founder cómo quería resolver esto (3 opciones: obligatoria antes de pagar / opcional hasta despacho / solo copy) — eligió **obligatoria antes de pagar**. Investigando encontré que la infraestructura YA existía sin usarse: `orders.prescription_id`/`prescription_snapshot` (columnas del diseño original, ADR-006) y una cookie firmada (`oc_prescription`) que ya llenan `/cargar-receta` y `/lector-de-receta`, pero el checkout nunca las conectaba. Cerrado con defensa en 3 capas (page-level gate visual, server action, `createOrderFromCart` como última barrera antes de tocar stock). Nuevo archivo `lib/checkout/prescription.ts` con la lógica compartida (`cartRequiresPrescription`, `prescriptionInsertFromCookie`). La receta se persiste como fila reusable en `prescriptions` (ADR-006) Y como snapshot inmutable en la orden (ADR-007). `pnpm typecheck` + `pnpm lint` limpios, test unitario de la lógica de detección, regresión en vivo sin errores. **No se corrió un E2E completo del pago** (crearía una orden real + preferencia de Mercado Pago real) — si el founder quiere probarlo de punta a punta, sugiero hacerlo en preview de Vercel como se hizo con el checkout original (ver entrada 2026-06-08 más abajo en este archivo).

### Fix aplicado: hallazgo #5 cerrado — placeholders de FAQ/legales sincronizados, verificados con argentine-ecom + optical-expert

De los 7 placeholders `[A CONFIRMAR]` en `lib/content/faqs.ts`: 3 se resolvieron con datos que YA estaban confirmados pero desincronizados (dirección del local — estaba en `lib/site/business.ts` desde 2026-06, `BACKLOG.md` no se había actualizado, ver entrada nueva en MISTAKES.md; devolución — `BUSINESS_POLICIES.md` ya tenía la respuesta legal verificada 2026-06-11; graduación elevada — `optical-expert` dio un criterio cualitativo en vez de un número inventado). 2 se corrigieron por contradecir el estado real del sitio (cuotas/medios de pago afirmaban cosas que no aplican con `INSTALLMENTS_ENABLED=false` y checkout apagado — `argentine-ecom` confirmó riesgo de publicidad engañosa, Ley 24.240 art. 8, y dio la redacción). De paso se corrigió una afirmación legal desactualizada sin placeholder (botón de arrepentimiento ya no es "exigido" por norma, la que lo obligaba fue derogada en 2025). Quedan 4 placeholders genuinamente pendientes del founder, registrados en `BACKLOG.md` (horario del local ×2, efectivo, plazos reales de Correo Argentino). `FaqJsonLd` ahora filtra respuestas con `[A CONFIRMAR` antes de publicarlas como structured data. `terminos-y-condiciones`/`politica-de-privacidad` (razón social/CUIT/domicilio fiscal sin confirmar) pasan a `noindex, follow` vía flag nuevo `incomplete` en `buildInfoPageMetadata`. Verificado en vivo: JSON-LD de FAQ pasó de 29 a 25 entidades, ambas legales dan `noindex`, `/politica-de-devolucion` (control) sigue indexable. `pnpm typecheck` limpio.

### Fix aplicado: hallazgo #4 cerrado — sitemap/metadata/`generateStaticParams` ahora filtran por stock real

Helper nuevo `hasAvailableStock()` en `lib/catalog/availability.ts` (mismo criterio `is_active && stock_qty > 0` que ya usaba el resto del catálogo), aplicado en `app/sitemap.ts` (excluye del sitemap junto al filtro de placeholders existente), `lib/catalog/metadata.ts` (`buildProductMetadata` agrega `noindex` sin stock) y `lib/catalog/queries.ts` (`getStaticProductParamsForCategory` deja de prerenderear sin stock — siguen accesibles por URL directa, ninguna page tiene `dynamicParams = false`). No toqué el indexado de páginas de marca/género/filtro con 0 productos (thin pages) — hallazgo relacionado pero separado, fuera de este fix. **Nota de verificación**: el catálogo real hoy no tiene ningún producto sin stock (68/68 activos con stock, confirmado por API), así que el caso negativo se probó con un test unitario sintético de `hasAvailableStock()` (5 casos, todos OK) en vez de contra datos reales; el caso positivo sí se verificó en vivo sin regresión. `pnpm typecheck` limpio.
...
✅ **COMPLETO + LIVE: Rusty Yeah SOL (2026-08-04, seed 92)**. `/anteojos-de-sol/rusty/rusty-yeah`, PDP live HTTP 200 (verificado title, precio $133.965, badge "Polarizado", sin "bluecut"). **Norma: catalog-loader + seo-strategist invocados.** Aviador doble puente unisex, full ACETATO BIO-BASED (frente+patillas, diferenciador físico vs resto del cluster G-Flex), policarbonato 100% UVA/UVB cat 3, 32,9g. **⚠️ Los 2 links ML del founder tenían las etiquetas de color CRUZADAS respecto a la realidad** — desambigüé con `attribute_combinations` reales de la API, no las etiquetas de los links: el link marcado "Multivariante C1 y C2 Pol" (MLA2819024290) era en realidad el item SIMPLE "C3 - Carey" (ACTIVO); el link marcado "C3" (MLA2819011214) era en realidad el MULTI-VARIACIÓN con C1+C2 (PAUSADO en ML, no afecta `is_active` en DB — precedente PRO 30/Vulk Ready). **Esto NO se le había comunicado al founder todavía — pendiente avisarle en la respuesta de este turno** para que sepa que sus links pueden traer las etiquetas de ML desactualizadas. **ESTRUCTURA MIXTA de variantes — primer precedente del catálogo**: C1 SKU 1194370 (POL) + C2 SKU 1194371 (POL) comparten `mercadolibre_item_id=MLA2819011214` con `variation_code` real distinto; C3 SKU 1194372 (NO pol) tiene su propio `mercadolibre_item_id=MLA2819024290` con `variation_code=NULL` — confirmado sin choque de constraint (`UNIQUE(item_id, variation_code) DEFERRABLE`, NULLs distinct). $133.965 c/u, stock 3 c/u (total 9). 2/3 polarizadas (67%) → NO se afirma "Polarizado" en title/H1/name (criterio Play/Patien, no el de Terdey/Zinz/The Take 100%). **SEO**: primaria `anteojos de sol aviador` (110/10, REAL) — distinta de `lentes de sol aviador` (170/12) que ya es carril de Rusty The Take (también aviador Rusty-sol, 1/1 pol); Yeah es el 2º aviador del cluster, corregida la nota vieja de "carril único" en SEO_STRATEGY.md. Cross-link Yeah↔The Take obligatorio. NO linkeado a `/anteojos-de-sol/rusty/polarizados` (bug BACKLOG: esa faceta usa criterio viejo "todas las variantes", Yeah 2/3 no calificaría honestamente ahí). Fotos en `rusty-yeah/` (7, 900×442/1500×1500, HTTP 200 verificado, respetando guion bajo doble en C1 vs simple en C2). Verificación MCP: `forma=aviador, mat=acetate, variantes=3, stock=9, imgs=7, primaria=C1 perfil`. Scale 1.2/1.0 provisional (gemelo The Take) — reverificar contra grid `/anteojos-de-sol/rusty` (regla 15). `pnpm typecheck` limpio. Archivos actualizados: `supabase/seeds/92_rusty_yeah_sol.sql` (nuevo), `lib/catalog/image-scale-overrides.ts`, `supabase/CLOUD_APPLIED.md`, `SEO_STRATEGY.md`. Sin commit todavía (esperando confirmación del founder o próxima instrucción explícita de "push").

🐛 **BUG REAL detectado y corregido: fotos rotas en Vulk Ready? por `?` sin encodear (2026-08-04)**. El founder reportó "las imágenes aparecen rotas en la web". Causa: `getProductImageUrl` (`lib/storage/product-image-url.ts`) concatenaba el `storage_path` crudo en la URL sin encodear — funcionaba con espacios (el navegador los tolera) pero el `?` de "vulk-ready?-receta/Ready?-CRY-PERFIL.jpg" es un carácter reservado de URL (inicia el query string), cortando la ruta ahí. Primer producto con `?` en el nombre → bug latente sin manifestarse en ~90 productos previos. **Fix**: encodear cada segmento del path con `encodeURIComponent` (split `/` → encode → join) antes de concatenar. Encontré un SEGUNDO lugar que duplicaba la misma construcción sin encodear (`lib/catalog/metadata.ts`, el `og:image` de metadata) → corregido para reusar `getProductImageUrl` en vez de reimplementar. Verificado: URL encodeada (`Ready%3F-CRY-PERFIL.jpg`) resuelve HTTP 200 contra el bucket real. `pnpm typecheck` limpio. Entry en MISTAKES.md con la regla preventiva (toda URL desde `storage_path` DEBE pasar por `getProductImageUrl`). **Segunda parte del fix**: el founder renombró los ARCHIVOS del bucket (no la carpeta) sacándoles el "?" (`Ready-CRY-PERFIL.jpg` / `Ready-CRY-FRENTE.jpg`, la carpeta sigue siendo `vulk-ready?-receta/` con el símbolo) → actualicé `storage_path` en la DB (UPDATE por path viejo, verificado HTTP 200) + sincronicé seed 91 + scale overrides con los nombres reales. Con esto la imagen ya funciona incluso ANTES de que se despliegue el fix de código (el nombre de archivo nuevo no tiene caracteres reservados). El fix de `getProductImageUrl` sigue siendo válido y necesario para productos futuros. **⬜ Pendiente**: founder confirma visualmente que las fotos ya cargan bien (ISR revalida cada 5 min, así que puede tardar un rato en verse sin el redeploy).

✅ **COMPLETO + LIVE: Vulk Ready? RECETA (2026-08-04, seed 91)**. `/anteojos-de-receta/vulk/vulk-ready-receta` HTTP 200 (Ready, $77.690, transparente). UNISEX, G-Flex con sistema de bisagras flexo, 18,2g. **1 sola variante** (100% transparente cristal): SKU 956944 MLA1947385195, stock 3, 3 imágenes (1 primaria=perfil). **name = "Vulk Ready?"** con el signo de pregunta literal (nombre real del modelo); **slug técnico = `vulk-ready-receta`** (sin símbolo, URL-safe) — distinto del **nombre de carpeta del bucket, que SÍ lleva el símbolo literal** (`vulk-ready?-receta/`), confirmado que `storage_path` (columna `text`) no tiene restricción de charset. `frame_shape="cuadrado"` (hipótesis, ratio 54×42≈1.29, match cercano a Katleen). Flow de norma cumplido (catalog-loader + seo-strategist). **SEO**: primaria `anteojos transparentes` (720/16) GENÉRICO sin género — único Vulk receta 100% transparente, cierra el cluster junto a Strewn (mujer) y Rusty PRO 30 (hombre). Fotos 900×442, scale 1.1/1.0. Docs: seed 91, CLOUD_APPLIED, SEO_STRATEGY. **⚠️ SIN pushear**. **⬜ Pendiente menor**: founder confirma `frame_shape` real + reverifica scale en grid (regla 15). **Próximo paso**: push, o siguiente producto.

✅ **APLICADO + LIVE: Vulk The Trial Optics receta — +3ª variante CRY-206 (2026-08-01)**. Founder pidió agregar SOLO la variante que faltaba a un producto EXISTENTE (`vulk-the-trial-receta`, product_id f6a6891c…). Datos ML (`MLA1939459323`, catálogo `MLAU4551228282`): **CRY-206 Optics SKU 125723** (frente transparente cristal / patillas azul mate), $83.736→8373600c, stock 3, sort 3. Medidas = a las del resto del modelo (147/50×49/15/150, 19,5g) — el founder confirmó explícito "usar las mismas medidas". Aplicado vía MCP: variante + 2 imágenes (`Thetrial-CRY-206-PERFIL.jpg` / `Thetrial-cry-206-frente.jpg`, sort 8/9, 900×442 verificado con PIL tras un falso positivo de mi parser manual de JPEG; scale 1.03/1.0 igual al resto del modelo; medidas.webp→sort 10). Verificación MCP: `variantes=3 activas, stock_total=6, imagenes=7, orden 125721:1..125723:3`. PDP en vivo confirma CRY-206/azul mate/125723 visibles (el primer fetch pisó cache de aplicación reciente, el segundo lo mostró bien). Docs: seed 72 (comentario de adición), CLOUD_APPLIED, image-scale-overrides. **⚠️ Nota de contexto**: había trabajo sin commitear de otro proceso en paralelo (fixes de seguridad de la auditoría de hoy — admin auth, ML oauth, face-shape rate limit) en el working tree; NO se tocó ninguno de esos archivos, solo se commitearon los 3 propios de esta carga (seed 72, image-scale-overrides.ts, CLOUD_APPLIED.md, este archivo). **⬜ Pendiente menor**: founder chequea el scale visual (1.03/1.0) en el grid de receta.

✅ **COMPLETO + LIVE: Rusty PRO 30 RECETA (2026-08-01, seed 90)**. `/anteojos-de-receta/rusty/rusty-pro-30-receta` HTTP 200 (PRO 30, $95.399, cuadrado, transparente, **cero bluecut/filtro azul/blue block**). Cuadrado hombre, G-Flex + patillas con alma de metal + terminales de goma antideslizantes, 22,9g, garantía 1 año. **1 sola variante**: LIGHT GREY (gris transparente/terminales azules) SKU 126411, MLA1939580597, $95.399, stock 3, 3 imágenes (1 primaria=perfil). name "Rusty PRO 30 Optics" (convención Rusty receta). Flow de norma cumplido (catalog-loader + seo-strategist). ⚠️ Descartada una publicación duplicada en ML (MLA3709808184) que no matcheaba el link del founder. **SEO**: primaria `anteojos transparentes hombre` 260 — carril libre pese a que PRO 30 colisiona en forma+género con Spell (cuadrado) e Invig (hombre branded); resuelto por color+género en vez de pelear esas dos keywords. Fotos en bucket `Rusty-PRO30-receta/` (nombre con mayúsculas, distinto del slug técnico). Scale 1.1/1.0. Docs: seed 90, CLOUD_APPLIED, PRODUCTS_INVENTORY, SEO_STRATEGY. **⚠️ SIN pushear**. **⬜ Pendiente menor**: founder reverifica scale en grid (regla 15). **Próximo paso**: push, o siguiente producto.

✅ **COMPLETO + LIVE: Vulk Be Again RECETA (fotos aplicadas) + Vulk Vartis RECETA nuevo (2026-08-01, seeds 88+89)**.

- **Be Again** (`vulk-be-again-receta`): fotos confirmadas por el founder, aplicadas (7 imgs, 1 primaria=MBLK perfil). Nombres reales distintos a los tentativos originales. Mismo caso que Strewn: la variante CRY son fotos full-res 5365×3577 → scale 1.2/1.05 (vs 1.1/1.0 del resto). PDP live 200, sin bluecut. `frame_shape="cuadrado"` sigue como hipótesis sin confirmar por el founder.
- **Vartis** (`vulk-vartis-receta`, seed 89, nuevo): armazón receta MUJER, frente G-Flex+bisagras metálicas, patillas de acetato, 29,2g. 2 colores: L.PINK rosa transparente/patillas carey 969361 (primary) / MDEMI carey mate/negro brillo 969363, $88.922 c/u, stock 3 c/u. `frame_shape="redondo"` (hipótesis, mejor fundamentada que "cuadrado" por comparación de ratio con Ther/Kirt). name "Vulk Vartis Mujer" — primer Vulk receta que reclama género como keyword primaria (`anteojos vulk mujer` 320). Anti-canibalización con el trío mujer Vulk (Katleen=forma, Strewn=color, Vartis=marca+género). Verificado que "B.CUT" (bluecut) de los nombres de archivo NUNCA aparece en texto visible, solo en URLs de imagen. Fotos de modelo (`modelo-vartis-*`) sin usar — sin precedente en el catálogo, anotado en BACKLOG.

Ambos con flow de norma cumplido (catalog-loader + seo-strategist), aplicados y verificados vía MCP, PDPs live 200. Docs: seeds 88 (comentario actualizado)+89, CLOUD_APPLIED, SEO_STRATEGY (2 bloques), BACKLOG. **⚠️ SIN pushear** (código commiteado en turnos previos para 88; 89 es nuevo). **⬜ Pendiente menor**: founder confirma `frame_shape` real de ambos (cuadrado/redondo Be Again, redondo Vartis) + reverifica scale en grid mujer (regla 15).
...
- **Rusty Peating sol** (`rusty-peating`): "Rusty Peating", cuadrado unisex negro, AMBAS polarizadas. MBLK/S10 960100 (primary) / SBLK/DRT03 960103, $82.127/$81.798.
- **Vulk Dieven receta** (`vulk-dieven-receta`): "Vulk Dieven Unisex", rectangular bordes anchos G-Flex 28,5g, primer rectangular Vulk en receta. MBLK 116080 (primary) / SBLK 116083 / L.ROSE 116090, $75.590 c/u.
- **Vulk Dieven sol** (`vulk-dieven`): "Vulk Dieven", rectangular unisex (forma resuelta a favor del founder + medidas del receta, sobre el "Cuadrado" del título ML). MBLK/S10 GREY POL 116085 (primary, polarizada) / SBLK/SG91 POL 116089 (polarizada) / ROSE/BROWN-GREEN 116087 (polarización no confirmada, `polarized:false` sin afirmarlo en copy).

Docs: CLOUD_APPLIED (4 filas ✅), PRODUCTS_INVENTORY (4 filas), SEO_STRATEGY (4 bloques ya cargados en el turno anterior), BACKLOG (bug `/polarizados` no deriva de variantes en runtime — el comentario del seed 79 Zinz era incorrecto — y gap de faceta `/anteojos-de-sol/cuadrados`, ambos anotados). **⚠️ SIN pushear** (código de los 4 seeds + docs commiteado en turnos previos; falta push). **⬜ Pendiente menor**: founder reverifica scale (1.1/1.0 en los 4) en los grids receta y sol (regla 15). **Próximo paso exacto**: push, o siguiente producto.

✅ **COMPLETO + LIVE: Vulk Kirt RECETA (2026-07-07, seed 83)**. `/anteojos-de-receta/vulk/vulk-kirt-receta` HTTP 200 (Kirt, $74.990, redondo, dorado/cobre, **cero bluecut**). Redondo UNISEX metal liviano 17,5g medium, patillas Monel + bisagras integradas, 2 var (LG dorado 128870 / MDB cobre 128871), stock 4, 5 imágenes (primaria LG perfil). Flow de norma (catalog-loader molde Ther 75 + seo-strategist). name "Vulk Kirt Optics". `temple_material=metal` (Monel→copy), `hinge_system=integrada`. **SEO**: primaria MATERIAL `anteojos de metal` 210 (diferencia de My Crew G-Flex; leads invertidos vs Ther que lidera forma); cross-link Kirt↔Ther; bloque en SEO_STRATEGY. Fotos 900×442, scale 1.1/1.0 (idéntico Ther). Cross-link sol↔receta: Kirt sol pausado en ML sin seed → hoy sin par (engancha solo si se carga `vulk-kirt`). Docs: seed 83, CLOUD_APPLIED, PRODUCTS_INVENTORY, SEO_STRATEGY, image-scale-overrides. **⚠️ SIN commitear/pushear**. **⬜ Pendiente menor**: founder reverifica scale en grid (regla 15). **Próximo paso**: commit + push, o siguiente producto.

✅ **COMPLETO + LIVE: Rusty Invig RECETA (2026-07-07, seed 82)**. `/anteojos-de-receta/rusty/rusty-invig-receta` HTTP 200 (Invig, $72.999, rectangular, **cero menciones de bluecut**). Rectangular HOMBRE metal ultra liviano 14,7g, 3 var (C2 negro/C1 marrón/C3 gris oscuro), stock 6, SKUs founder 840071/840070/840072, 7 imágenes (1 primaria=C2 perfil). Flow de norma (catalog-loader molde Zinz 78 + seo-strategist; primaria `anteojos rusty hombre`). **Colores C1/C3 RESUELTOS mirando las fotos**: C1 = marrón oscuro + reflejo azul del lente en el frente (por eso el archivo decía "BLUE"); C3 = gris oscuro gunmetal (el archivo dice "PLATEADO" pero NO es plateado → se mantiene el nombre de ML). Scale 1.1/1.0 provisional. Docs: seed 82, CLOUD_APPLIED, PRODUCTS_INVENTORY, SEO_STRATEGY, image-scale-overrides. **⚠️ SIN commitear/pushear** (código: seed + scale + docs; DB ya live). **⬜ Pendiente menor**: founder reverifica scale en grid tras deploy (regla 15); opcional confirmar hero C2. **Próximo paso**: commit + push, o siguiente producto.

✅ **APLICADO + LIVE: carga Vulk Strewn RECETA (2026-07-07, seed 81)**. `/anteojos-de-receta/vulk/vulk-strewn-receta` HTTP 200 (Strewn, $80.190, cuadrado, transparente). Verificación MCP: `forma=cuadrado, gender=female, peso 17.8, variantes=3, stock=6, imgs=7, primarias=1`. Flow de norma cumplido (catalog-loader + seo-strategist). **Decisiones/defaults del asistente** (avisados al founder): name "Vulk Strewn Receta", hero MBLK, `temple_material=g-flex` (asumido), `hinge_system=flex`, `recommended_face_shapes` omitido (como Katleen), SKU placeholders `STREWN-MBLK/CRY/MROSE`. **SEO**: primaria carril TRANSPARENTE `anteojos transparentes mujer` 390 (NO forma — Katleen defiende cuadrados-mujer); cross-link Strewn↔Katleen; bloque agregado a SEO_STRATEGY. **Carpeta bucket**: el founder la renombró mid-carga de `vulk-strewn/` → `vulk-strewn-receta/` → paths del seed actualizados + verificados HTTP 200. **Scale 2 familias** (foto CRY es 5365×3577 vs MBLK/MROSE 900×442): MBLK/MROSE 1.1/1.0, CRY 1.2/1.05 PROVISIONAL. Docs: seed 81, CLOUD_APPLIED, PRODUCTS_INVENTORY, SEO_STRATEGY, image-scale-overrides. **⚠️ SIN commitear/pushear** (código: seed + scale + docs; DB ya live). **✅ SKUs reales aplicados (2026-07-07)**: MBLK 113140 / CRY 113145 / M.ROSE 113143 (UPDATE por mercadolibre_item_id + seed sincronizado). **⬜ Pendiente menor**: reverificar scale de CRY vs las 2:1 en el grid tras deploy (regla 15).

✅ **APLICADO: 5ª variante de Rusty Xold receta — MDEMI-MBLK (2026-07-07)**. Founder pidió agregar SOLO la variante que faltaba a un producto EXISTENTE (`rusty-xold-receta`, product_id 95c1861a…). Datos ML (`scripts/ml-item.ts` MLA2152487922, var 204937786143): **MDEMI-MBLK OPTICS SKU 125719** (frente carey mate / patillas negro mate), $83.078→8307800c, stock 2, sort 5. Aplicado vía MCP: variante + 2 imágenes (`XOLD_MDEMI-MBLK_perfil/frente.jpg`, sort 8/9, scale 1.15/1.0 como el resto; medidas→sort 10). Convención receta (solo model_code + frame_color "carey-mate"; **SIN bluecut** — founder explícito, igual que el seed original). Medidas ya en DB (no se tocan). Verificación MCP: `variantes=5 activas, stock_total=9, imagenes=11, orden 125748:1..125719:5`. Docs: seed 31 (comentario de adición), CLOUD_APPLIED, image-scale-overrides. **✅ Fotos subidas por el founder** como `XOLD_MDEMI-MBLK_OPTICAL_perfil/frente.jpg` (con "OPTICAL", igual que la MBLK; HTTP 200) → DB image storage_path + scale override actualizados a esos nombres reales. PDP live muestra la variante con fotos. **⬜ Pendiente menor**: scale override live tras deploy (founder chequea 1.15/1.0 en el grid receta).

✅ **APLICADO + LIVE: carga Vulk Nova SOL polarizado (2026-07-07, seed 80)**. `/anteojos-de-sol/vulk/vulk-nova` HTTP 200 (ISR revalidó solo): "Vulk Nova", $128.299, 6 badges Polarizado, "ovalados para mujer". Verificación MCP: `forma=ovalado, gender=female, lens_treatment incluye polarized, peso 35.8, variantes=3, stock_total=15, var_polarizadas=3, var_codes=3 distintos, imgs=7, primarias=1`. **Flow de norma cumplido**: `catalog-loader` (playbook, molde Bennie 51) + `seo-strategist` (slug/meta/keywords CSV real) invocados ANTES de escribir. **Fotos**: ya en bucket con nombres CRUDOS del founder (espacios/puntos/plural "perfiles") → se usaron verbatim en el seed (verificados HTTP 200) en vez de renombrar, para no divergir. **Fix asociado (product-page.tsx)**: `frameShapeToSpanish` no mapeaba `ovalado` (solo `oval` inglés) → subtítulo daba null; agregada la key (Nova es el 1er ovalado del grid sol; arregla también Clems receta retroactivo). Scale **1.1/1.0 PROVISIONAL** en `image-scale-overrides.ts`. Registrado en CLOUD_APPLIED (seed 80) + PRODUCTS_INVENTORY. **⚠️ SIN commitear/pushear**: seed 80 + fix product-page.tsx + scale override + docs. El fix del subtítulo y el scale override NO están live hasta deploy (la DB sí). **⬜ Próximo paso exacto**: commitear + (founder decide push) → tras deploy, founder reverifica el scale visual del grid /anteojos-de-sol junto a Bennie 51/Raven (regla 15; 1.1 provisional, subir a 1.15 si se ve chico / bajar a 1.0 si cropea). **Datos DB del turno anterior (referencia)**: precio $128.299→12829900c, stock 5 c/u, C1 968320/var189163990683, C2 968321/var189163990685, C3 968322/var204932326043. **✅ Datos ML confirmados vía `scripts/ml-item.ts` (MLA2197866182)**: título "Anteojos Lentes De Sol Polarizados Vulk Nova Gafas Modernas", **precio $128.299 → 12829900c** (IGUAL en las 3), **stock 5 c/u** (total 15), 3 variantes con var_id + SKU del founder: **C1 POL** var 189163990683 · SKU 968320 (dorado/carey, lente marrón) · **C2 POL** var 189163990685 · SKU 968321 (dorado, lente verde) · **C3 POL** var 204932326043 · SKU 968322 (dorado, lente marrón degradé). **Ficha**: frente metal, patilla metal+terminal acetato, lente polarizada 100% UVA/UVB, **35,8gr**, talle Large, forma OVALADA, género MUJER. **Medidas (medidas.png)**: frente 144 / lente 55×41 / puente 19 / patilla 145. **Fotos del founder** (nombres crudos con espacios/puntos/plural — NORMALIZAR): C1 frente+"perfiles", C2 FRENTE+PERFIL, C3 FRENTE+PERFIL, medidas.png. **Próximo seed = `80_vulk_nova_sol.sql`** (último aplicado = 79 rusty zinz sol). **⏳ 2 agentes en background (2026-07-07)**: `catalog-loader` (playbook: molde de seed sol Vulk, mapeo fotos→bucket con primaria=perfil, campos PRODUCT_SCHEMA, enum forma, marca polarizado→faceta, scale provisional comparado al grid) + `seo-strategist` (slug, meta, keywords con Ubersuggest reconectado/CSV, internal linking Vulk+mujer+polarizados). **⬜ Pendiente founder**: confirmar si las fotos ya están en el bucket o siguen locales (para pasarle nombres normalizados). **Próximo paso exacto**: recibir output de los 2 agentes → escribir seed 80 → fotos al bucket → scale override comparado con grid sol → typecheck → aplicar (MCP Supabase o founder) → verificar PDP+grids+faceta polarizados → PRODUCTS_INVENTORY + CLOUD_APPLIED.

🧾 **INTEGRACIÓN FACTURADOR ↔ ÓPTICA: bloqueada por RLS (2026-07-06, revisión, sin cambios de código)**. Founder hizo un módulo en su facturador de escritorio (`~/Facturador optica/cloud.mjs`) para subir facturas de la óptica automático y pidió revisar si "la página está adaptada". **Arquitectura real**: el facturador NO usa endpoint HTTP — se conecta DIRECTO al Supabase de la óptica (login email/password → JWT authenticated + anon key) y opera `orders` vía PostgREST: `fetchPedidos()` lee `orders` con `payment_status=eq.approved & invoice_cae=is.null`; `marcarPedidoFacturado()` hace `UPDATE orders` con el CAE. **Lo que YA cuadra ✅**: todas las columnas existen (`order_number`, `customer_name`, `customer_dni`, `total_cents`, `payment_status`, `invoice_cae/id/url`, `paid_at`); el filtro `payment_status='approved'` matchea porque el webhook MP escribe `payment_status: payment.status` y un aprobado es `"approved"`. **EL BLOQUEO 🔴**: la RLS de `orders` es `SELECT USING (auth.uid()=user_id)` (solo dueño) y NO tiene policy de UPDATE para authenticated. Las órdenes web se crean con el `user_id` del CLIENTE (checkout exige login). El admin web funciona porque usa **service_role** (saltea RLS); el facturador entra como usuario común → (1) `fetchPedidos()` devuelve `[]` en silencio (nunca ve órdenes de clientes), (2) `marcarPedidoFacturado()` afecta 0 filas (sin policy UPDATE). **Fix necesario (migración RLS en la óptica — NO aplicada, regla 7)**: Opción A (recomendada) = marcar la cuenta del facturador como staff (allowlist de su user_id o flag) + 2 policies (SELECT todas + UPDATE staff-only) + GRANT column-level solo `invoice_*` para acotar escritura; el `cloud.mjs` queda igual. Opción B = RPCs `SECURITY DEFINER` (más seguro pero hay que cambiar el facturador). Opción C (service_role en la app de escritorio) = descartada (leak de secreto). **Próximo paso exacto**: founder pasa el EMAIL de la cuenta Supabase que usa el facturador → escribo la migración (Opción A), founder la revisa y aplica. Hasta entonces la subida automática de facturas NO funciona.
**✅ MIGRACIÓN ESCRITA (2026-07-06, Opción A elegida por founder)**: `supabase/migrations/20260706000000_facturador_staff_rls.sql`. Cuenta del facturador = `facturador@opticacarballo.com`, `user_id=9f64cdeb-25f1-4a77-a6a2-98d26bfcae0c` (obtenido vía MCP `select from auth.users`). Contenido: (1) tabla `staff_users` (allowlist, RLS sin policies → solo service_role) + alta del uid; (2) `is_staff(uid)` SECURITY DEFINER; (3) policies `staff read all orders` + `staff update orders` + `staff read all order items` (se suman a "users read own orders"); (4) **candado de columnas**: `REVOKE UPDATE ON orders FROM authenticated` + `GRANT UPDATE (invoice_id, invoice_cae, invoice_url)` — staff solo escribe facturación, no precio/estado. El facturador solo escribe `{invoice_id, invoice_cae}` (verificado en engine.mjs:753). `cloud.mjs` NO se toca. **⚠️ Aplicación BLOQUEADA por el classifier** (regla 7, correcto): `apply_migration` denegado → la aplica el founder (SQL Editor de Supabase, pegar la migración completa, Run; o `supabase db push`). **Verificación pendiente post-aplicación**: el asistente simula acceso `authenticated` con el JWT de esa cuenta (SET ROLE + request.jwt.claims sub) y confirma SELECT ve aprobadas + UPDATE escribe CAE + no-staff sigue en cero (read-only, no lo bloquea el classifier). Registrar en CLOUD_APPLIED al aplicar.

📊 **MICROSOFT CLARITY montado (2026-07-06, sin pushear)**. Founder pidió instalar Clarity (project id `xi76xeam7m`). Nuevo componente `components/analytics/microsoft-clarity.tsx` (client, `next/script` afterInteractive, sin deps nuevas) que reusa el patrón de consent-gating de `GoogleAnalytics`: **carga SOLO con consent 'all'** (ley 25.326 — Clarity graba sesiones + cookies; cargarlo sin consent contradice el banner). Id overrideable con `NEXT_PUBLIC_CLARITY_ID` (fallback al provisto). Montado en `app/layout.tsx`. Typecheck OK. **Decisión que puede revisar el founder**: si quiere Clarity sin gate de cookies (no recomendado por legal), avisar. **Pendiente founder**: deploy para que empiece a grabar.

📊 **VERCEL ANALYTICS montado (2026-07-06, sin pushear)**. Founder pidió setear `import { Analytics } from "@vercel/analytics/next"`. Instalé `@vercel/analytics@2.0.1` **con pnpm** (⚠️ `npm install` falla en este repo por peer opcional `@remix-run/react` que pide react ^18 — el gestor del repo es pnpm, `npm` corrompe el árbol; restauré package.json y usé `pnpm add`). Agregué import + `<Analytics />` en `app/layout.tsx` junto a `<GoogleAnalytics />` y `<SpeedInsights />` (ya existentes). Typecheck OK. **Pendiente founder**: activar Analytics en dashboard Vercel (Project → Analytics → Enable) + deploy; los datos aparecen con tráfico real post-deploy. **Próximo paso**: volver a la verificación del flow de pagos (webhook MP + env vars Vercel + redeploy + test de pago real) que quedó a mitad.

💳 **AUDIT: activar pagos en producción — código completo, faltan credenciales prod + webhook (2026-07-06, sin cambios de código)**. Founder pidió terminar de habilitar el procesador de pagos ("creo que solo falta setear claves de producción"). Audit read-only del flow MP: `lib/mp/client.ts` (singleton lazy, detecta TEST vs prod solo por prefijo del token — pasar a prod NO toca código), `lib/mp/preferences.ts` (Checkout Pro V1 redirect, back_urls + notification_url desde `NEXT_PUBLIC_SITE_URL`, envío como item inline, auto_return condicionado a no-localhost), `lib/mp/webhook.ts` (validación HMAC-SHA256 con `MP_WEBHOOK_SECRET` — **obligatorio en NODE_ENV=production, rechaza todo sin él**; fetch del payment vía SDK; map status; timingSafeEqual), `app/api/mp/webhook/route.ts` (idempotencia por payment_id, emails Resend cliente+admin al pasar a `paid`, siempre 200 a MP). **Conclusión: el código está completo y probado (smoke 5/5 histórico), no hay que tocar nada.** Hallazgo clave del audit: `MP_PUBLIC_KEY`/`NEXT_PUBLIC_MP_PUBLIC_KEY` **NO se usan en ningún archivo** (Checkout Pro es redirect puro; sería para Bricks futuro) → la única credencial de pago que importa es `MP_ACCESS_TOKEN`. **Env vars consumidas por el flow de venta** (grep sobre lib/checkout|cart|mp|emails): `MP_ACCESS_TOKEN`, `MP_WEBHOOK_SECRET`, `NEXT_PUBLIC_SITE_URL`, `NEXT_PUBLIC_CHECKOUT_ENABLED`, `CART_COOKIE_SECRET`, `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `BUSINESS_ADMIN_EMAIL`. **Lo que falta (TODO del lado del founder — las credenciales prod NO se pegan en chat ni se commitean, van en Vercel)**: (1) en Vercel Production setear `MP_ACCESS_TOKEN=APP_USR-...` (reemplaza el TEST), `MP_WEBHOOK_SECRET` (signing key que genera MP), `NEXT_PUBLIC_SITE_URL=https://opticacarballo.com.ar`, `NEXT_PUBLIC_CHECKOUT_ENABLED=true`, `CART_COOKIE_SECRET`, `RESEND_API_KEY`+`RESEND_FROM_EMAIL`+`BUSINESS_ADMIN_EMAIL`; (2) **configurar webhook en panel MP** (Tus integraciones → Notificaciones): URL `https://opticacarballo.com.ar/api/mp/webhook`, solo evento `payment` → MP genera la signing key → esa va en `MP_WEBHOOK_SECRET` (sin esto las órdenes quedan `pending` para siempre); (3) verificar dominio `opticacarballo.com.ar` en Resend (lo que más suele frenar los emails); (4) redeploy. **Próximo paso exacto**: founder entra a su cuenta MP → copia el Access Token de producción (`APP_USR-...`) + configura el webhook para obtener la signing key → carga las vars en Vercel + redeploy. Ofrecí (sin ejecutar aún) dejar un checklist marcable en `tasks/todo.md` y verificar el estado del dominio en Resend — esperando OK del founder.
**EN CURSO (2026-07-06)**: founder tiene las credenciales de producción y está configurando. Screenshot del panel MP → Webhooks (ambiente **Productivo**): URL correcta (`https://opticacarballo.com.ar/api/mp/webhook`), pero **Eventos mal** = "Vinculación de aplicaciones, Alertas de..." en vez de **Pagos** → el código solo procesa `type==='payment'`, con esos eventos nunca se notifica un pago y las órdenes quedarían en `pending`. **Acción pendiente founder**: en "Configurar notificaciones" destildar todo y dejar solo "Pagos"; confirmar que la signing key quedó en Vercel como `MP_WEBHOOK_SECRET` (obligatorio en prod); redeploy; test de pago real end-to-end. No accedo al navegador del founder ni el MCP de MP está autorizado (OAuth no disponible en sesión no-interactiva) → configuración manual guiada paso a paso.
**Config webhook confirmada en panel MP (2026-07-06)**: evento correcto = **"Pagos (legacy)"** (manda `type: "payment"` + `data.id`, que es lo que procesa nuestro código — NO usar "Order (Mercado Pago)", formato distinto no soportado). ⚠️ Detectado: al abrir "Configurar notificaciones" el campo "URL de producción" aparece vacío (placeholder) y "Guardar" queda deshabilitado hasta re-cargar la URL `https://opticacarballo.com.ar/api/mp/webhook`. **Verificación desde el repo (2026-07-06)**: `GET https://opticacarballo.com.ar/api/mp/webhook` → **HTTP 200** `{"ok":true,"endpoint":"mp-webhook"}` (endpoint vivo en prod). NO se pueden leer las env vars de Vercel desde acá (MCP Vercel no las expone, CLI no instalado, son secretas) → el founder confirma en el dashboard. **Próximo paso exacto**: (1) founder confirma env vars en Vercel Production (`MP_ACCESS_TOKEN` APP_USR, `MP_WEBHOOK_SECRET`, `NEXT_PUBLIC_SITE_URL`, `NEXT_PUBLIC_CHECKOUT_ENABLED=true`) + webhook guardado con URL; (2) test de pago real end-to-end (producto barato, tarjeta real → orden pasa a `paid` + 2 emails + % notificaciones MP sube de 0%); (3) si falla, con el `payment_id` se revisan logs.

🔵 **PRÓXIMO: retomar carga de productos (2026-06-11)**. Founder quiere seguir subiendo productos nuevos. Último seed aplicado = `58_rusty_esvep`. Playbook aceitado (stock=ML, primaria=perfil, polarizadas→/polarizados, scale comparado con el grid, medidas/peso/forma/género, verificación PDP+grid, update PRODUCTS_INVENTORY + CLOUD_APPLIED). **⚠️ El conector Supabase (MCP) se desconectó** — los seeds se aplican por ahí → reconectar (o el founder aplica) cuando lleguemos al paso de aplicar. Recordatorios activos en memoria: [[stock-siempre-ml]], [[feedback-grid-primaria-perfil]], [[feedback-c2-vrast-no-mencionar]]. **EN CURSO (2026-06-11): Rusty R-CY 02** — armazón de RECETA eco/reciclado (MLA-1388464699, multi-variante; 11,3g, medidas 140/54×38/18/135, bisagras metálicas, unisex; lens_compat mono/bi/progresivo, sin polarizado). **5 variantes CONFIRMADAS** (2026-06-11): MBLK `960186` (negro mate, fotos propias) · SBLK `960186` (negro brillo, **reusa fotos de MBLK** vía copia en bucket con sufijo — patrón CCCP por el constraint UNIQUE(product_id,storage_path)) · SBLUE `960187` · STEEL BLUE `960186` · L GREY `960188`. (SKU `960186` compartido en 3 colores → el id real por variante es el var code de ML.) **✅ DATOS COMPLETOS traídos de ML (2026-06-11) vía nuevo `scripts/ml-item.ts`** (lee+desencripta el token OAuth de `marketplace_integrations` con service-role key + `APP_ENCRYPTION_KEY` — no necesita MCP ni sesión admin): **precio $84.592,27** (igual en las 5) · stock + var_id por color → MBLK 2 (var 188766461829) · SBLK 2 (188766461831) · STEEL BLUE 1 (178752745788) · SBLUE 0 (178752745790) · LGREY 0 (178752745792). **⬜ ÚNICO pendiente: founder confirma la FORMA** (parece wayfarer) → con eso escribo el seed (medidas 140/54×38/18/135, 11,3g, bisagras metálicas, reciclado, unisex, lens_compat mono/bi/progresivo, primaria=perfil, SBLK reusa fotos de MBLK vía copia, scale 1.15/1.0 receta). **✅ SEED ESCRITO (2026-06-11): `supabase/seeds/59_rusty_rcy_02_receta.sql`** (slug `rusty-r-cy-02-receta`, forma `rectangular`, eco/reciclado en descripción+callout, frame_material OMITIDO a propósito, hinge_system metalica, 5 variantes con var_id + precio 8459227c + stock). **SKU**: founder dijo que el 960186 repetido es error de ML pero igual son los que correspondían → la columna `sku` es UNIQUE, NO admite el mismo en 3 variantes, así que quedaron `960186-MBLK/-SBLK/-STEELBLUE` (preservan su 960186 + único) + 960187 (SBLUE) + 960188 (LGREY). **✅ Fotos en bucket** `products/rusty-r-cy-02/` (founder subió 8 + medidas.webp; el asistente copió MBLK→SBLK vía storage API; todas HTTP 200; ⚠️ L GREY tiene DOBLE espacio en el nombre). **✅ Scale overrides** agregados (1.15/1.0 receta) + typecheck OK. **✅ APLICADO + LIVE (2026-06-11)**: el founder reconectó el MCP (ojo: reconectar a mitad de sesión NO refresca las tools — necesitó reiniciar/reconectar hasta que aparecieron) y el asistente aplicó el seed vía `execute_sql`. Verificación MCP: `producto=1, forma=rectangular, variantes=5, stock_total=5, imagenes=11, primarias=1, var_codes_unicos=5`. PDP en vivo HTTP 200 con nombre + fotos del bucket + precio $84.592. Registrado en CLOUD_APPLIED + PRODUCTS_INVENTORY. **⬜ Pendiente menor**: founder chequea el scale visual (1.15/1.0) en el grid de receta; si tiene 3 SKU distintos reales para MBLK/SBLK/STEEL BLUE, los cambia (hoy `960186-COLOR`). **✅ model_code con OPTICS** (founder aclaró: las 5 son OPTICS) aplicado a base + seed (commit `e4ea915`). **R-CY 02 COMPLETO.** **Próximo paso exacto**: founder pasa el link de ML del siguiente producto → asistente saca precio/stock/var_id solo con `scripts/ml-item.ts`. (Founder trabajando en paralelo en perf/animaciones — varios componentes UI sin commitear, NO tocar.) **✅ +Rusty Opposit receta COMPLETO (seed 60, 2026-06-11)**: wayfarer femenino, G-Flex+patillas metal/acetato, 3 var SKU distintos, medidas de la FOTO (147/53×47/17/140), SEO por `seo-strategist` (slug/meta/keywords). Aplicado+live. Ubersuggest autorizado pero las tools no surgieron mid-session → keyword research fino + cluster Rusty en SEO_STRATEGY = PENDIENTE sesión nueva. **✅ RENAME Rusty receta → "Optics" (2026-06-11)**: los 5 armazones de receta Rusty pasaron de "X Receta" → "X Optics" en el `name` (Misty/Opposit/R-CY 02/Spell/Xold) — DB vía MCP + seeds 48/59/60 sincronizados (31/35 documentales); meta_title mantiene "Armazón de Receta" (SEO). ⏳ La grilla/PDP en vivo muestra "Optics" recién tras revalidar el **ISR** (o redeploy). **✅ Fix legal/óptico (2026-06-11, commit `97c438f`)**: el founder (óptico) marcó que el FAQ decía MAL que "necesitás receta para comprar" — comprar el ARMAZÓN no requiere receta (viene con lentes demo); la receta solo hace falta para los cristales graduados. Corregido en `components/product/product-faqs.tsx` (FAQ de todas las fichas de receta) + `lib/content/faqs.ts` (FAQ global "sin-receta"). Términos ya estaba bien (habla de "anteojo recetado/armado"). **✅ Opposit scale +0.02** (1.15/1.0 → 1.17/1.02; R-CY 02 quedó bien en 1.15/1.0). **✅ Keyword data REAL** (carpeta `KEYWORDS OPTICA/` con CSVs de Ubersuggest — ver memoria [[keywords-optica-csv-folder]]): "armazón de receta" casi no se busca (0 vol); los términos fuertes son `anteojos rusty` (3.600, dif 8), `anteojos recetados` (720, dif 9), `anteojos/armazones mujer` (480-880), `lentes wayfarer` (590). **⬜ OFRECIDO al founder**: armar el cluster Rusty en SEO_STRATEGY.md + refinar los meta de los productos de receta con esta data (pasar de "armazón de receta" a "anteojos rusty/recetados/mujer"). **Próximo paso**: siguiente producto (link ML), o el cluster SEO, o lo que pida el founder.

🐛 **FIX: pedidos cancelados ya NO permiten generar envío (2026-06-15, commit `74b4156`, SIN pushear)**. Founder reportó que en `/admin/pedidos` un Cancelado se podía seleccionar y entrar en "Generar envíos" → daba de alta una etiqueta REAL en Correo (no era cosmético). Causa: elegibilidad chequeada solo por `shippingMethod !== 'pickup'`, sin mirar `order.status`. Fix en 4 capas: nuevo helper de dominio `canGenerateShipment()` + `NON_SHIPPABLE_STATUSES` (cancelled, refunded) en `lib/orders/order-status.ts`; guard server en `generateShipmentForOrder` (protección real, cubre lote + individual); listado admin no deja seleccionar cancelados; detalle oculta el botón; corregido acoplamiento del badge "Retiro en local" (dependía de `!eligible`). typecheck+lint+build OK. **⬜ Pregunta abierta**: ¿pedidos `pending` (sin pago) también deben bloquearse? Hoy NO (solo cancelled/refunded) — esperando decisión del founder. Documentado en MISTAKES 2026-06-15.

🎨 **MINIATURAS DE VARIANTES SIN RECUADRO (2026-06-11, commit `6e745e4`, pusheado en `ad04987`)**. Founder no quería la cajita con borde alrededor de cada thumbnail de color en el grid (se veían como cajones con líneas divisorias). `components/product/product-card.tsx` `VariantThumbnails`: borde completo → sin marco sobre el fondo; estado activo pasó de `border` (caja) a `border-b-2` (subrayado fino); el cuadrito `+N` de overflow mobile también sin borde (solo texto). Aplica a TODOS los grids vía ProductCard (regla 15). typecheck+lint OK. **⬜ Pregunta abierta al founder**: ¿deja la activa con subrayado, o sin ningún indicador (se distingue solo por la foto grande de la card)?

🟢 **SIMULADOR DE VISIÓN CONSTRUIDO — esperando calibración visual del founder (2026-06-11, commit `b8e392e`)**. Founder eligió el simulador del brainstorm (se había confundido con el probador — aclarado: el simulador muestra cómo VE el cliente, no usa fotos de producto; corre 100% client-side, **0 IA por uso**, la receta no sale del navegador). **Flow de agentes cumplido**: `optical-expert` validó/corrigió la física ANTES de codear (3 correcciones clave: σ paralela al eje = k·|S| y perpendicular = k·|S+C| — evitó bug de 90°; zona muerta 0.25D; modelo unificado de acomodación A\*=clamp(SE+D, 0, U) que resuelve lejos/cerca/hipermetropía/presbicia con una fórmula; binocular = MEJOR ojo, nunca peor/promedio; luces nocturnas en capa blend `screen` porque el blur directo las apaga; ghost desplazado = efecto "¡así veo yo!" del astígmata) + `seo-strategist` definió slug `/simulador-de-vision`, title/meta, H2s del cluster "[condición] como ven" (~480/mes, dif 7-14, data CSVs), WebApplication+FAQPage schema, internal links (guías miopía/astigmatismo/presbicia/receta existentes + lector + /anteojos-de-receta). **Archivos**: `lib/vision-sim/optics.ts` (física pura documentada), `components/tools/vision-simulator.tsx` (3 escenas SVG sintéticas — lejos/cartel con letras, cerca/menú, noche/luces; blur direccional feGaussianBlur con rotar→blur→contra-rotar + overscan; tabs escena + OD/OI/Ambos + toggle sin/con anteojos + ADD/edad + slider de calibración k), `app/(storefront)/simulador-de-vision/page.tsx` (estática ○ 5.2kB/111kB First Load, **noindex TEMPORAL** con TODO hasta aprobación, disclaimer YMYL exacto de optical-expert). typecheck+lint+build OK, smoke local 200 con JSON-LD. **⬜ PRÓXIMO**: (1) founder calibra en el celu contra su propia receta (slider "ajuste fino del realismo") y valida el feel; (2) con su OK → sacar noindex + pase de `ui-motion-designer` (diseño quedó funcional a propósito — prototipo primero, plan acordado) + posible reemplazo de escenas sintéticas por fotos reales + link desde lector/footer/guías; (3) avisar a `nextjs-performance` no hace falta — ruta nueva aislada, sin tocar layouts/queries compartidas, bundle verificado en build. **Pendiente separado**: opinión sobre el PROBADOR dada (recomendado versión "asesor por selfie" sin overlay AR — ver mensaje 2026-06-11), sin decisión aún. **🔧 1ra ronda de feedback del founder APLICADA (commit `5c7b5c0`)**: (a) blur en px fijos era invisible en pantallas grandes ("se ve igual con y sin anteojos") → k ahora es % del ancho real del viewport (ResizeObserver, 0.6%/D default, slider en ×); (b) "Tostado de jamón y queso" pisaba el precio → "Tostado mixto"; (c) nítido-de-cerca con miopía es física CORRECTA pero parecía bug → mensaje explícito "ventaja del miope" que invita a probar lejos/noche. ⚠️ Commits `b8e392e`/`7b9a443`/`5c7b5c0` SIN pushear (classifier bloqueó push directo a main — necesita OK explícito del founder).

🔵 **BRAINSTORM "herramientas Fable 5" ENTREGADO — superado por la elección de arriba (2026-06-11)**. Founder pidió lista de herramientas mind-blowing para clientes que con Opus 4.8 salían limitadas. Entregadas 6 ideas en 2 grupos: **(A) ingeniería compleja** — (1) simulador de visión según receta (blur calibrado por dioptría + astigmatismo direccional, física validable con optical-expert; wow máximo, esfuerzo alto), (2) calculadora visual de espesor de cristal (sagital esfero-cilíndrico + índice + aro; empuja venta de alto índice; 1-2 días), (3) probador con análisis de rostro por foto (vision multi-paso; ⚠️ biométrico = no almacenar nada, mismo patrón que recetas); **(B) modelo en producción vía API** (`claude-fable-5` $10/$50 existe) — (4) asesor óptico agéntico con function calling sobre catálogo+stock real, (5) "tu receta explicada" en criollo + detección de incoherencias (extensión del lector, 1 día), (6) comparador que razona el caso del cliente. **Recomendación dada: orden 2 → 5 → 1** (validar con las baratas antes de invertir en el simulador grande). Regla 14 recordada: audit del código existente antes de estimar fino. **⬜ Esperando: founder elige cuál armar.** Ninguna pisa los items pendientes del plan aprobado (3: admin UI de carga; 4: guía piloto).

🟢 **VULK MY CREW RECETA CARGADO + CLUSTER RUSTY HECHO (2026-06-11, mismo turno)**. **(A) Producto**: seed 61 `vulk-my-crew-receta`, armazón de RECETA redondo/ovalado UNISEX (G-Flex frente+patillas, bisagras flex, 16g, medium). 4 variantes en 1 MLA (MLA1432179921), todas $83.107,23, datos de ML vía `scripts/ml-item.ts`: SBLK 125580 stock 3 (primary), 669K 125583 stock 4, MBLK 125584 stock 3, 388 125582 stock 1 (total 11 = ML). `frame_shape='round'` (catalog-loader: el enum NO tiene `oval`), gender `unisex`, hinge_system omitido (al callout). Medidas 137/49×40/21/140 (de la FOTO; texto ML decía 145). Scale **perfil 1.3 / frente 1.15** — comparación visual REAL (bajé las fotos: son 2:1, el anteojo se ve ~15% más chico que Opposit@1.15 → +scale para emparejar ~85% del card; así evité la iteración "se ve chico"). Aplicado MCP + PDP live 200 en `/anteojos-de-receta/vulk/vulk-my-crew-receta` (nombre+precio $83.107+9 imgs). **NORMA founder cumplida**: invoqué `seo-strategist` (slug/meta/keywords) + `catalog-loader` (validación schema). **(B) Cluster SEO**: agregados a SEO_STRATEGY.md el **cluster RUSTY** (head `anteojos rusty` 3.600/dif8; por producto Opposit/R-CY/sol; ⚠️ `lentes rusty` dif 49 NO usar) y el mini-cluster **VULK MY CREW receta** (head `anteojos vulk` 4.400 al hub, producto targetea redondo+unisex). Regla #1 de "futuros productos" reescrita con la NORMA (siempre agentes + Ubersuggest/CSV). Ver memoria [[norma-carga-producto-agentes-keywords]]. **⬜ Pendiente menor founder**: chequeo visual del scale My Crew en el grid de receta; redeploy/ISR para ver "Optics" + FAQ + My Crew en vivo (la PDP ya da 200).

🟢 **TRUST SIGNALS PDP — compactado y REVERTIDO (2026-06-14)**. Founder pidió comprimir el alto del bloque de 6 garantías (`components/product/product-trust-signals.tsx`). Propuso carrusel auto-avanza de 3 visibles; **conversion-optimizer (prioridad) + ui-motion-designer desaconsejaron** el carrusel: ocultar/rotar trust signals daña CRO (funcionan por presencia simultánea) y 2 son links legales de Defensa del Consumidor (devolución + arrepentimiento) → no se esconden. El problema real era alto vertical. Founder eligió "Compactar" (AskUserQuestion) → se implementó layout horizontal de 1 línea (commit `4dd44df`). **Pero al verlo NO le gustó → REVERTIDO al diseño original** (grilla 2 filas, ícono + título + descripción; commit `bc50d5f`). Estado actual = como estaba antes del turno. **Pendiente abierto**: el founder sigue queriendo comprimir el alto de ese bloque pero el approach de compactar a 1 línea no convenció — falta otra idea visual si lo retoma (la opción "filas finas horizontales con descripción" quedó sin probar). **Próximo paso exacto**: esperar input del founder (siguiente producto a cargar, o nueva idea para el alto de los trust signals). Sin trabajo en curso; working tree limpio, todo pusheado (`bc50d5f`).

🟢 **TRUST SIGNAL FACTURA — copy actualizado (2026-06-14)**. Founder marcó que "cumple AFIP" es confuso. argentine-ecom verificó (web): AFIP se disolvió → ahora **ARCA** (Decreto 953/2024); mejor NO nombrar al organismo; quitarlo no rompe nada legal (la obligación es emitir factura con CAE, ya se hace vía Tusfacturas). Cambiado `"Electrónica, cumple AFIP"` → `"Electrónica y oficial"` en `components/product/product-trust-signals.tsx`. No quedó "cumple AFIP" en otros componentes. Commit `2bc6fe9` pusheado. ⬜ Deuda menor separada: las páginas legales (`/defensa-del-consumidor`, `/politica-de-devolucion`, `/boton-de-arrepentimiento`) y el OG image todavía mencionan "AFIP" en contexto legal → revisar si conviene actualizar a ARCA en una pasada futura. **Próximo paso exacto**: esperar input del founder.

🟢 **3 BUGS CORREGIDOS post-code-review (2026-07-01, commit `360be52`)**:
(1) `swipe-deck.tsx`: eliminada prop `isAuthenticated` y sus 14 branches condicionales — dead code desde que /descubrir tiene gate duro; el deck siempre persiste en DB ahora. (2) `brand-page.tsx`: faceta hombre/mujer solo se genera si hay productos `male`/`female` explícitos (antes `|| genders.has('unisex')` generaba ambos links con catálogo idéntico → duplicate content SEO). (3) `tool-auth-gate.tsx`: eliminado JSDoc que prometía gatear lector-de-receta (esa página no guarda datos, gatearlo contradice su propuesta de valor). Typecheck OK. **Próximo paso exacto**: esperar input del founder — lo siguiente puede ser continuar la carga de productos o cualquier otra tarea.

🟢 **CODE-REVIEW AUDIT COMPLETADO (2026-07-01)**. Founder pidió buscar bugs. Se corrió `/code-review high` sobre los últimos 10 commits. 8 ángulos de revisión + 5 verificadores. Findings verificados entregados al founder (ver mensaje de cierre). Findings verificados finales: (1) **CONFIRMED** `isAuthenticated` prop en `swipe-deck.tsx` tiene 14 branches condicionales que son dead code — la prop defaultea a `false` y el único call site siempre pasa `true`; confunde futuros mantenedores y el localStorage fallback nunca corre. (2) **PLAUSIBLE** marca con solo productos `unisex` genera links `/hombre` y `/mujer` con contenido casi idéntico (`brand-page.tsx` líneas 62-67: `genders.has('unisex')` activa AMBAS condiciones); Vulk ya está cerca (solo unisex+female, sin male-only). (3) **PLAUSIBLE** `lector-de-receta/page.tsx` no tiene gate de auth pese a que el JSDoc de `ToolAuthGate` promete que lo tendrá. REFUTADOS: ISR cache poisoning (Next.js auto-opta la ruta a dinámica por la llamada a cookies); fetch secuencial (correcto: necesita saber si hay user antes de disparar data); doble auth round-trip (solo en server actions, no en page load). **Próximo paso exacto**: founder decide si aplicar fixes — los 2 más útiles: (a) limpiar `isAuthenticated` de swipe-deck.tsx (14 branches muertos), (b) agregar auth gate a lector-de-receta si corresponde.

🟢 **AJUSTE SCALE Rusty The Take SOL (2026-07-01)**. Founder pidió agrandar el The Take en el grid de `/anteojos-de-sol`. Solo tiene 1 variante (L.GREY-MBLK), foto 900×442. Subí perfil en rondas: 1.0 → 1.15 (commit 764bec6) → **1.20** (founder ajustó de 1.25 a 1.20). Frente queda en 1.0. Visible tras ISR. ⬜ Commit + push pendiente. **Próximo exacto**: founder confirma si el scale visual está bien o ajusta más.

🟢 **AJUSTE SCALE Rusty Zinz SOL var2 (2026-07-01)**. Founder pidió agrandar un poco más la variante 2 (MBLK-S10, sku 126805) en el grid de `/anteojos-de-sol`. Como el grid usa la foto de perfil (primaria), subí `rusty-zinz/ZINZ-MBLK-S10-POL-PERFIL.jpg` de **1.1 → 1.3** en `lib/catalog/image-scale-overrides.ts`. Frente queda en 1.0 (el founder habló del grid de sol). Aplica a todas las categorías vía pipeline central (regla 15). Var1 (669K) sin tocar. Visible tras ISR. Commit local pendiente (founder pushea). **Próximo exacto**: founder chequea el scale visual en el grid; si querés más/menos lo ajusto.

🟢 **AJUSTE SCALE Vulk 53&3 Marky Ramone (2026-06-14)**. Founder pidió achicar 3 perfiles en la grilla de categoría: v2 LG/02 POL 1.15→1.12 (−0.03), v3 MG/20 POL 1.15→1.10→**1.07** (founder pidió −0.03 más en 2do pase), v5 MBLK/03 POL 1.15→1.12 (−0.03). Solo `lib/catalog/image-scale-overrides.ts` (perfiles; v1 S/G15 y v4 S/25 quedan en 1.15). Mapeo variante→path confirmado por `sort_order` en DB vía MCP antes de tocar. Sin cambios de datos/queries. Visible tras ISR. Commit local pendiente (founder pushea).

🟢 **VULK THE TRIAL — SOL CERRADO + RECETA CARGADO (2026-06-14)**. (1) **SOL**: founder subió las 9 fotos a `vulk-the-trial/`, 9/9 HTTP 200, nombres = seed exacto (incl. doble espacio del MDEMI) → sin reconciliar. Scale **corregido 1.20→1.0 en las 8** (confirmado visual: 900×442 más anchas que el card 3:2 → object-contain llena por ancho, el 1.20 del precedente Gresent cropeaba; mismo caso que Raven). (2) **RECETA** (hermano, seed 72, `vulk-the-trial-receta`, name "Vulk The Trial Optics"): armazón aviador doble puente unisex, G-Flex + patillas Monel/acetato hecho a mano, lentes demo, 19,5g. Convención receta OK (solo lens_compatibility, sin lens_*, verificado MCP `tiene_treatment=false`; model_code OPTICAL). 2 MLAs paused stock 0 (is_active=true): 670-056 125721 $83.745 (primary, celeste translúcido + patillas metal — founder; ML decía "gris oscuro/doradas" pero la foto confirma celeste); MBLK-046 125722 $83.736 (negro mate). SEO por seo-strategist (primaria `anteojos recetados` 720/9 + carril aviador; meta "Armazón de Receta..."; sin cuotas). Scale 1.0 (mismo encuadre). 5 fotos HTTP 200, typecheck OK. **Pendiente**: ⬜ precio MDEMI sol provisional (BACKLOG). **Próximo**: siguiente producto o lo que pida el founder.

🟢 **VULK KATLEEN RECETA — +1 variante MDEMI (2026-06-15)**. Founder agregó la variante MDEMI (carey mate) al Katleen receta (ya cargado, misma multi-variante MLA2014157548). var 204547293605, $86.541,45, stock 3. ⚠️ ML sin SKU → placeholder KATLEEN-MDEMI (sync usa varcode). Inserté SOLO la variante + 2 imágenes (katleen-mdemi-P/F.jpg, HTTP 200) vía MCP, sin tocar las 3 existentes. Scale 1.15/1.0 (= resto del modelo). Seed 53 sincronizado + override + CLOUD_APPLIED. Verificación MCP: 4 variantes, 2 imgs c/u. **🔧 FIX post-carga (founder detectó, mismo patrón en Spell)**: (1) la imagen `medidas` quedaba ANTES de las fotos de las variantes nuevas (sort medio) → `medidas` a sort 99 (siempre última) en Katleen Y Spell; (2) faltaba listar las variantes nuevas en la `description` → agregadas en Katleen (MDEMI) y Spell (LGREY+MDEMI-MBLK), conteos actualizados. Seeds 53/35 sincronizados. **MISTAKES.md**: entry nueva con checklist de "agregar variante a producto existente". ⬜ Sin commitear. **Próximo**: redeploy/ISR; o siguiente.

🟢 **RUSTY THER RECETA — APLICADO (2026-06-15)**. Seed 75 aplicado vía MCP. Verificación OK: `forma=round, mat=metal, peso 14.5, sin lens_treatment, 3 variantes, stock 122/2/2, model OPTICAL, 2 imgs c/u`. 7 fotos HTTP 200, scale 1.0 (900×442), typecheck OK. SEO: primaria `anteojos redondos`+carril metal; anti-canib. vs Misty receta (acetato vs metal) → bloque + nota en SEO_STRATEGY. ✅ Commiteado `64dc3d6` + pusheado. **RE-VERIFICADO 2026-06-29 (post-reinicio del proceso)**: los 2 agentes async (catalog-loader + seo-strategist) figuran "failed" porque el proceso de Claude se cerró mientras corrían — pero eran asesores y el trabajo real (seed 75 + apply + commit) ya había quedado completo. Confirmado contra DB: `forma=round, mat=metal, lens_compat 4, 3 variantes (M020/M036/M033), OPTICAL, 2 imgs c/u`; M020 stock 122→2 (sync ML normal). Nada roto, nada que re-aplicar. ⬜ Cross-link Misty↔Ther sugerido (no implementado; sistema "redondos similares" futuro).

~~🟡 RUSTY THER RECETA — EN CURSO (2026-06-15)~~ (APLICADO arriba). Armazón de receta **redondo unisex de METAL** (patillas metal + terminales acetato, bisagra acero inox flex, lentes demo, 14,5g, adapta mono/bi/multifocal). Link era catálogo MLAU → ítems reales por API search "ther": 3 colores mate, todos **$74.990**: M036 Violeta 128171 MLA1858781821 stock 2; M020 Cobre 128172 MLA1858781819 **stock 122**; M033 Bordó 128173 MLA1858820529 stock 2. Medidas 135/49×47/23/145 (foto). 6 fotos + medidas ya en bucket `rusty-ther/`. **⏳ Esperando validación de catalog-loader + seo-strategist (background)** para: frame_shape round, frame_material metal (confirmar enum), lens_compatibility, primary (¿M020 por stock 122 o criterio color?), scale, meta/keywords. **Próximo exacto**: con los resultados → escribir seed 75, aplicar por MCP, scale overrides, verificar, actualizar CLOUD_APPLIED/PRODUCTS_INVENTORY/SEO_STRATEGY. weight 14,5g (lo da → NO a pendientes).

🟡 **3 PEDIDOS DEL FOUNDER (2026-06-29) — 1 diagnosticado + 2 features a planear**:

**(A) BUG formas duplicadas en filtro [✅ RESUELTO 2026-06-29, founder dio OK]**: `frame_shape` tenía valores mezclados inglés (mis cargas) vs español (viejos) → chips duplicados + /aviador no matcheaba The Trial. **Normalizado a ESPAÑOL** (canónico real de las rutas+mayoría): UPDATE DB (round→redondo, square→cuadrado, aviator→aviador; 8 productos) + 8 seeds sincronizados + PRODUCT_SCHEMA corregido (enum español) + BACKLOG enum-audit (a) cerrado. Verificado: solo quedan aviador(6)/cuadrado(14)/redondo(12), sin duplicados. The Trial ahora matchea /aviador. Error en MISTAKES (enum-drift). ⬜ Sin commitear aún. 

**(B) Scanner de receta tras login** [feature, a planear]: pedir login/crear cuenta antes de escanear → guardar graduación en la cuenta; si es graduación alta, permitir igual pero avisar que para colocar cristales graduados hay que ir presencial + explicar por qué. ⚠️ Cambia el modelo de privacidad actual (hoy la receta NO se guarda, corre client-side) → dato sensible de salud, consentimiento; validar con argentine-ecom + optical-expert (mensaje graduación alta) + conversion-optimizer (gate duro vs blando).

**(C) Swipe/Match tras login [EN CURSO — gate BLANDO confirmado por founder]**: AUDIT hecho — el feature ya está 80% construido: `/descubrir` (swipe-deck.tsx 570 líneas), tabla `swipe_matches` (RLS), página `/mi-cuenta/matches`, auth `/registro`+`/ingresar` (con `?next=`), y **sync localStorage→DB al loguearse YA implementado** (en swipe-deck useEffect, corre en /descubrir). Hoy el anónimo swipea y el match queda solo en localStorage SIN pedir nada → falta el prompt de captura. **PLAN**: (1) en `SwipeResults` (definido dentro de swipe-deck.tsx, ~línea 442), si `!isAuthenticated` && hay matches → CTA "Creá tu cuenta para guardar tus N matches" → `/registro?next=/descubrir` (ahí el sync persiste localStorage→DB = mail+datos capturados); (2) pasar `isAuthenticated` a SwipeResults (hoy no se le pasa); (3) mejora: tras sync-on-login en swipe-deck, mostrar resultados (setShowResults) para que vea sus matches guardados. ⚠️ Gap resuelto en el plan: el sync sólo corre en /descubrir → por eso `next=/descubrir`, no /mi-cuenta/matches. **✅ IMPLEMENTADO 2026-06-29** (conversion-optimizer definió el diseño): banda compacta de captura ARRIBA de la grilla en `SwipeResults`, gatillada SOLO con `matched.length >= 2 && !isAuthenticated` (con 0-1 la propuesta es débil); copy "Guardá tus {N} matches" + "Guardar mis matches" → `/registro?next=/descubrir` + microcopy "Es gratis y toma 30 segundos"; NO nudge mid-mazo, NO banner persistente. Logueado → confirm "Tus matches están guardados en Mi cuenta →". `isAuthenticated` ahora se pasa a SwipeResults. Mejora: tras sync-on-login exitoso, `setShowResults(true)` (el recién registrado ve sus matches, no arranca de cero). Cambios solo en `components/swipe/swipe-deck.tsx`. typecheck + lint OK. ⬜ Sin commitear. **Próximo exacto**: (1) commit+push + verificar flujo en vivo (anón ≥2 matches → registro → matches en DB + visibles); (2) luego (B) Scanner de receta tras login. Métricas a trackear post-launch (data-analyst): tasa de captura = registros vía next=/descubrir ÷ anónimos con ≥2 matches en resultados.

✅ **RUSTY ZINZ SOL — CARGADO (2026-06-30)**. Hermano de sol del Zinz receta. slug `rusty-zinz` en `/anteojos-de-sol/rusty/rusty-zinz`. Seed 79 aplicado vía MCP. Cuadrado unisex polarizado, G-Flex frente+patillas, bisagras flex customizadas, 25,7g, policarbonato polarizada UV400 cat3. **Multivariante** (MLA1549978087, variation_code=var_id): 669K-SBLK/DRT23 POL 126807 var 185897876260 stock 10 (PRIMARY +stock, gris transparente); MBLK/S10 POL 126805 var 192324627407 stock 4 (negro brillo). $78.869,25 → 7886900c ambas. **2/2 pol → afirma "Polarizado"** + /polarizados (runtime). Medidas 137/51×42/20/145. Fotos 669K 900×442 + MBLK 1200×589 (ratio 2.04:1) → scale 1.1/1.0 en override. SEO: primaria forma `lentes/anteojos de sol cuadrados`, meta_title "Anteojos de Sol Rusty Zinz Cuadrados Polarizados | Carballo". Verificado MCP (cuadrado, cat 3, policarbonato, 2 var pol, var_id poblado, imgs 5, primaries 1). Docs: CLOUD_APPLIED + PRODUCTS_INVENTORY + SEO_STRATEGY (ficha sol + contralink en receta) + override. ⚠️ Discrepancia códigos ML vs founder → mapeo por color, model_code founder guardado, reconcilia por var_id. **Próximo**: reverificar grilla/scale `/anteojos-de-sol` post-ISR (regla 15); recommended_face_shapes pendiente para ambas versiones Zinz (BACKLOG); commit cuando founder dé OK.

---

✅ **RUSTY ZINZ RECETA — CARGADO (2026-06-30)**. Founder pasó los 2 SKU (669K-SBLK=126803, MBLK=126800). Seed 78 aplicado vía MCP: 3 variantes activas (SBLK 126801 $79.790 stock 2 primary; 669K-SBLK 126803 / MBLK 126800 ambos $82.745,69→8274500c stock 1), cuadrado unisex, G-Flex + bisagras metálicas flex, 25,7g, medidas 137/51×42/20/145. Fotos 1200×589 → scale 1.1/1.0 en override. SEO: meta_title "Armazón de Receta Rusty Zinz Cuadrado | Óptica Carballo", primaria `anteojos/lentes cuadrados`, anti-canib. vs Spell (masculino)/Katleen (femenino). Verificado MCP (forma=cuadrado, gender=unisex, 3 var, imgs 7, primaries 1). Docs: CLOUD_APPLIED + PRODUCTS_INVENTORY + SEO_STRATEGY (3 fichas cuadrados receta) + override. **Próximo**: reverificar grilla/scale `/anteojos-de-receta` post-ISR (regla 15); commit cuando founder dé OK.

---

**[HISTÓRICO] Rusty Zinz — entrada previa (bloqueada, ya resuelta arriba):** 🟡 EN CURSO, bloqueado en 2 SKU (2026-06-29). Armazón receta cuadrado unisex, G-Flex + bisagras metálicas flex, lentes demo, 25,7g. Medidas 137/51×42/20/145. 3 variantes (items simples): 669K-SBLK MLA2053711666 $82.745,69 stock 1 (frente gris oscuro/patillas negro brillo); MBLK MLA2016918974 $82.745,69 stock 1 (negro mate); SBLK MLA1866189073 $79.790 stock 2 **SKU 126801** (negro brillo). ⚠️ **BLOQUEANTE**: 669K-SBLK y MBLK NO tienen SKU en ML (verificado) y el founder pidió que el SKU esté → esperando que pase los 2 SKU (no se inventan). ⚠️ Duplicados en ML: MBLK (MLA2016918974+MLA2017035964) y SBLK (MLA1866189073+MLA1866313867, mismo SKU 126801) — usar 1 de cada. Fotos en `rusty-zinz-receta/` (ZINZ-{669K-SBLK,MBLK,SBLK}-{PERFIL,FRENTE}.jpg + medidas.png). **Agentes terminados** (2026-06-30) y validados: precio var 1+2 = **8274500 cents** (catálogo sin decimales de centavo), var 3 (SBLK) = 7979000; scale baseline **perfil 1.1 / frente 1.0** + reverificar grid post-deploy (sub-regla 15); SEO listo → meta_title "Armazón de Receta Rusty Zinz Cuadrado | Óptica Carballo" (54), primaria `anteojos cuadrados` (480) / `lentes cuadrados` (880), H1 "Anteojos de Receta Rusty Zinz — Cuadrados Unisex", anti-canibalización vs Spell (masculino) y Katleen (femenino): Zinz se queda con la **forma neutra unisex**, NO usa `cuadrados hombre/mujer`. Ofrecidas al founder 2 opciones: **A)** pasa los 2 SKU → cargo las 3 activas; **B)** cargo solo SBLK activa y 669K-SBLK+MBLK con `is_active=false` hasta tener SKU. **Pendiente SEO_STRATEGY.md**: Spell/Katleen/Zinz receta (3 cuadrados) sin fichas en el cluster Rusty — agregar tabla de anti-canibalización. **Próximo exacto**: founder responde A o B → escribo seed (frame_shape="cuadrado", lens_compat 4, OPTICAL, medidas.png sort 99, scale 1.1/1.0, 8274500 cents) → aplico + docs + fichas SEO.

🟢 **SCALE grid receta emparejado (2026-06-29)**. Founder: el Spell se veía grande vs el resto. Iteraciones: Spell 1.2→1.1; Ther y The Take 1.0→1.1 (agrandar los otros); pero Spell SEGUÍA grande (su foto encuadra el anteojo más grande que las demás) → **Spell perfiles 1.1 → 0.9** (los otros quedan en 1.1; Woxi 1.2). Frentes sin cambio. Commits 849e2f4 + d373f3b + 0d1c850. `image-scale-overrides.ts`. ⬜ Founder reverifica en vivo; si Spell queda chico/grande, afinar 0.85-0.95.

🟢 **RUSTY PATIEN RECETA — +1 variante SBLK (2026-06-29)**. Founder agregó SBLK negro brillo al Patien receta (misma multi-variante MLA1388467447). var 204692788223, SKU 126098, $83.078 stock 3. Inserté variante + 2 imágenes (PATIEN_SBLK-perfil/frente.jpg) **en sort 4/5, ANTES de medidas (sort 99)** + actualicé la descripción (2→3 colores) — aplicando las 2 lecciones que el founder recordó. Scale 1.1/1.0 (= resto). Seed 66 sincronizado. Verificación MCP: 3 variantes, 2 imgs c/u, fotos HTTP 200, typecheck OK. ⬜ Sin commitear. **Próximo**: commit/push; o (B) Scanner.

🟢 **RUSTY THE TAKE SOL — APLICADO + fix doble puente al receta (2026-06-29)**. Sol aviador doble puente unisex polarizado (seed 77, `rusty-the-take`). 1 var MBLK/S10 POL 129234 MLA2498747644 $85.917,95 stock 3. Frente G-Flex + bisagras flex, **patillas ACETATO** (≠ receta), policarbonato polarizada UV400 cat3, 18g. frame_material g-flex + temple_material acetate. 1/1 pol → afirma "Polarizado" + /polarizados. Scale 1.0 (900×442). medidas sort 99. SEO: primaria `lentes de sol aviador`; anti-canib. vs Vulk The Trial sol (ambos aviador doble puente) y vs The Take receta. **+ CORRECCIÓN founder**: el receta The Take (seed 76) TAMBIÉN es aviador doble puente (lo había puesto "clásico") → corregido short_desc + description + callout en DB + seed 76 + nota SEO. Verificación MCP OK, fotos HTTP 200, typecheck OK. ⚠️ El Take sol tiene 1 sola foto de modelo+frente (sin foto de contexto). ⬜ Sin commitear. **Próximo**: commit/push (se acumulan: The Take sol+receta, tipografía, swipe gate duro, normalización formas); o (B) Scanner gate+guardado.

🟢 **RUSTY THE TAKE RECETA — APLICADO (2026-06-29)**. Armazón receta **aviador unisex** (seed 76, `rusty-the-take-receta`). Norma: seo-strategist + catalog-loader (async). Link era catálogo MLAU → MLA reales por API search. 2 items simples $79.790 stock 3: MDEMI-MBLK OPTICAL 129230 MLA3537508372 (primary, carey/negro mate); L.ROSE OPTICAL 129233 MLA1866094293 (caramelo). G-Flex frente+patillas + bisagras flexo, lentes demo, 18g, `frame_shape="aviador"`, lens_compat los 4. Medidas 136/51×46/16/145. Scale 1.0 (900×442). medidas.png sort 99. SEO primaria `anteojos recetados`+aviador; anti-canib. vs Vulk The Trial receta (aviador doble puente). Verificación MCP OK (aviador, g-flex, sin lens_treatment, peso 18, 2 var, 2 imgs c/u), 5 fotos HTTP 200, typecheck OK. ⚠️ existe The Take SOL (129234) no cargado. ⬜ Sin commitear. **Próximo**: commit/push (junto con tipografía + swipe gate duro pendientes); o (B) Scanner gate+guardado.

🟢 **TIPOGRAFÍA → la de ShotPilot (2026-06-29)**. Founder pidió usar la misma fuente que su app ShotPilot (`Proyectos web/shotpilot`, proyecto HTML). Su UI usa **Bricolage Grotesque** (display) + **Manrope** (cuerpo). Aplicado en óptica: `app/layout.tsx` (next/font: Fraunces→Bricolage_Grotesque, Inter→Manrope) **manteniendo los nombres de variable `--font-sans`/`--font-serif`** → todas las clases `font-sans`/`font-serif` del sitio toman las nuevas sin tocar componentes (⚠️ `font-serif` ahora = Bricolage, no es serif real). `tailwind.config.ts`: fallback de serif Georgia→sans. Agente `ui-motion-designer` actualizado (3 menciones de Fraunces→Bricolage/Manrope). Pierde el carácter editorial Fraunces (cambio de identidad, decisión del founder). typecheck OK. ⬜ Sin commitear. **Próximo**: ver en vivo tras deploy (verificar que carguen bien); si querés afinar pesos por nivel de título, se ajusta.

🟢 **SWIPE → GATE DURO (2026-06-29, founder cambió de opinión: duro, no blando)**. El founder pidió gate DURO en ambas herramientas. Swipe: `/descubrir` ahora requiere login para swipear (mensaje: "creá tu cuenta y los anteojos con los que matcheás quedan guardados"). Implementado: nuevo `components/auth/tool-auth-gate.tsx` (muro reusable: eyebrow/title/description/next + CTAs Crear cuenta/Ya tengo cuenta → /registro|/ingresar?next=); `descubrir/page.tsx` gatea si `!user` → ToolAuthGate (antes mostraba el deck a anónimos). Saqué del swipe-deck la banda blanda de captura (ya inalcanzable; el deck solo monta autenticado) — queda el confirm "guardados en Mi cuenta". El sync localStorage→DB se mantiene (migra matches anón de antes del gate). typecheck+lint OK. ⬜ Sin commitear. **Próximo**: (B) SCANNER gate duro + guardar en cuenta — usa el MISMO `ToolAuthGate` en `lector-de-receta/page.tsx`, + server action que inserte en tabla `prescriptions` (ya existe, RLS), + guardar foto al bucket `prescriptions` (privado, ya existe), + **CAMBIAR el copy "la receta NO se guarda"** (descripción + FAQ) porque ahora SÍ se guarda → validar consentimiento con `argentine-ecom` + mensaje alta-graduación con `optical-expert` (la lógica `evaluateInPerson`/`InPersonHandoff` ya existe; ajustar para que alta graduación PERSISTA igual + muestre presencial). Decisión founder: gate duro + guardar foto+valores.

**Próximo paso exacto**: (1) founder confirma normalizar formas a español → aplico DB+seeds+schema+agente; (2) founder elige por dónde seguir con las features (sugerido: Swipe primero, luego Scanner). Nada commiteado de esto aún (solo MISTAKES + CURRENT_STATE).

🟢 **RUSTY SPELL RECETA — +2 variantes (2026-06-15)**. Founder agregó 2 colores nuevos al Spell receta (ya cargado, mismo MLA multi-variation MLA1510168748, "usar mismos detalles técnicos"): LGREY (gris transparente, var 204557471801, stock 2) y MDEMI-MBLK (carey mate/patillas negro mate, var 204557471803, stock 2), ambas $83.078. ⚠️ ML las trae **sin SKU** → SKU placeholder descriptivo (SPELL-LGREY / SPELL-MDEMI-MBLK; el sync usa var_id, no SKU) — reemplazar por SKU real si el founder lo tiene. Inserté SOLO las 2 variantes + 4 imágenes (sin tocar las existentes) vía MCP. Fotos en `rusty-spell-receta/` (4 nuevas HTTP 200), scale 1.2/1.05 (igual al resto del modelo). Verificación MCP: 4 variantes, 2 imgs c/u. Seed 35 (documental) + override + CLOUD_APPLIED actualizados. ⬜ Sin commitear. **Próximo**: redeploy/ISR para ver las 4 variantes; o siguiente.

🟢 **RUSTY WOXI RECETA — APLICADO (2026-06-15)**. Seed 74 aplicado vía MCP (el conector se recuperó al reintentar). Verificación MCP OK: `forma=rectangular, lens_compat=[monofocal], tiene_treatment=false, variantes=3, stock 2/2/2, model OPTICAL, imgs 2/variante`. 7 fotos HTTP 200. **Scale ajustado (founder 2026-06-15)**: agrandar un poco todas menos la v2 (SBLK) → perfil v1 (MBLK) y v3 (SBLKCL) 1.15→1.20; v2 SBLK queda en 1.15; frentes en 1.0. ⬜ Peso a BACKLOG. Detalle abajo. ⬜ Sin commitear aún (seed 74 + scales + docs).

~~🟡 RUSTY WOXI RECETA — seed ESCRITO, pendiente APLICAR (MCP sin permiso) (2026-06-15)~~ (APLICADO arriba). Armazón de receta **rectangular pequeño**, G-Flex, **SOLO monofocal** (founder: el talle chico no entra bi/progresivo), livianos. Norma: seo-strategist + catalog-loader. El founder pasó un link de CATÁLOGO ML (`/up/MLAU4173891880`, da 404 en /items) → encontré sus MLA reales del seller por API (`/users/81654493/items/search?q=woxi`): 3 colores, todos $87.890 stock 2 — MBLK 111191 MLA3520951126 (primary), SBLK 111190 MLA3520938342, SBLKCL-SBLK 111194 MLA3520964380. `frame_shape="rectangular"` (confirmado por foto), `lens_compatibility:["monofocal"]` (¡solo mono!), name "Rusty Woxi Optics", model_code OPTICAL. Medidas 138/52×37/18/145 (foto). SEO: primaria `anteojos recetados` (NO "rectangulares" para no canibalizar R-CY 02); meta "Armazón de Receta Rusty Woxi Rectangular…". **✅ Seed 74 escrito** + scale overrides (1.15/1.0 provisional, fotos full-res 3:2) + typecheck OK. **✅ 7 fotos en bucket `rusty-woxi/` HTTP 200**. **⬜ BLOQUEANTE**: el MCP de Supabase devuelve "permiso denegado" (se desconectó al reconectar ML) → **founder reconecta el conector Supabase** (o reinicia VS Code) y ahí aplico el seed 74 vía execute_sql + verifico + actualizo CLOUD_APPLIED/PRODUCTS_INVENTORY/SEO_STRATEGY. weight OMITIDO → BACKLOG. **Próximo exacto**: reconectar Supabase MCP → aplicar seed 74.

🟢 **FIX desborde thumbnails de variantes tras grilla 4-col (2026-06-15)**. Con las cards más angostas (4/fila → ~218px en laptops 1024-1279), la tira de miniaturas de color (hasta 5 a 80px) se desbordaba y se mezclaba con la card vecina (founder reportó "problemas estéticos"). Fix en `components/product/product-card.tsx`: `MAX_VISIBLE_THUMBS` 5→3 + tamaño uniforme `size-14` (56px, antes `md:size-20`=80px) + cuadrito "+N" size-14 + `max-w-full overflow-hidden` en la fila (tope de seguridad anti-bleed). Saqué `MAX_VISIBLE_THUMBS_MOBILE` (código muerto) y comentarios viejos. typecheck + lint OK (warning de `variants`/useMemo es preexistente, ajeno). Sin commitear aún. **⬜ Preguntado al founder**: si ve otros problemas estéticos además del desborde. **Próximo**: push + ver en vivo; o el siguiente ajuste.

🟢 **GRILLAS DE CATEGORÍA: 1 col mobile / 4 col desktop (2026-06-15)**. Founder pidió que todas las categorías se vean como vulk.com: 4 modelos por fila en PC, 1 por fila en mobile (antes 2 mobile / 3 desktop). Cambiado el grid a `grid-cols-1 sm:grid-cols-2 lg:grid-cols-4` en **8 archivos** (7 grillas catálogo + favoritos) + `catalog-grid-skeleton` (matchea, sin CLS). Actualizado el `sizes` de las imágenes en `product-card.tsx` (primaria `(max-width:640px)100vw,(max-width:1024px)50vw,25vw`; secundaria acorde). **nextjs-performance aprobó**: sizes correctos, ISR intacto, skeleton con `aspect-[3/2]` = sin CLS. **CRO flag dado al founder**: 1/fila mobile = más scroll/menos productos por pantalla (look editorial tipo Vulk, reversible). typecheck OK. ⬜ **Vigilar post-deploy**: LCP mobile p75 en Speed Insights (mobile ahora 100vw sin `priority`); si sube de 2.5s → evaluar `priority` SOLO en la 1ra card (no a ciegas) — anotado en BACKLOG. **Próximo**: ver en vivo tras deploy.

🟢 **RESUELTO: foto Bennie 51 S10 salía con lente "lavada"/clara (2026-06-14)**. Founder reportó que la S10 perfil se veía como lente clara (receta) en vez de negra. Causa REAL (mi 1ra teoría de semi-transparencia PNG fue ERRADA): el objeto bajo el nombre `BENNIE 51 MBLK-BLK perfil.jpg` quedó **stale en Supabase** — resubir el archivo corregido CON EL MISMO NOMBRE no surtía efecto (servía la versión vieja/clara), aunque los headers decían `cf-cache-status: EXPIRED`/`no-cache`. **El founder lo resolvió renombrando** el archivo a `BENNIE 51 MBLK-BLK P.jpg` (y el frente a `frente.jpg`) → un path nuevo sirve los bytes correctos (lente negra, verificado visual). **Sincronizado**: DB (2 UPDATE vía MCP) + seed 73 + scale override apuntan a los nombres nuevos. 7/7 HTTP 200, sin paths muertos, typecheck OK. **Learning** (ver LEARNINGS): sobreescribir un objeto de Storage con el mismo nombre puede servir contenido viejo; el fix confiable es subir con un nombre nuevo.

🟡 **VULK BENNIE 51 SOL — CARGA ABIERTA (2026-06-14)**. Sol **REDONDO unisex** small (seed 73, `vulk-bennie-51`). Norma: seo-strategist + catalog-loader. Frente G-Flex + patillas metal (barra metálica, sistema flex, terminales de goma), policarbonato UV400 cat 3, 18,9g. 3 MLAs separados, data ML: S10 POL 123600 $95.306,25 stock 3 (primary — hero polarizado, NO por stock), G.GREY 123609 $86.573,31 stock 2 (gris degradé → lens_color), GREEN 123760 $86.573,31 stock 7 (verde + AR interno → callout, no enum). 1 POL → /polarizados. `frame_shape="round"`. Medidas 135/51×37/18/140 (foto). **HONESTIDAD** 1/3 pol → title "Redondos". SEO: primaria de MARCA `lentes de sol vulk` 1.300; **anti-canibalización vs Rusty Blinded (redondo)**: Bennie=marca-first, Blinded=forma-first, ambos → categoría /redondos + cross-link. Verificación MCP: `forma=round, peso 18.9, variantes=3, pol=1, stock 3/2/7, imgs 2/variante`. typecheck OK. **✅ CERRADO**: founder subió las 7 fotos (7/7 HTTP 200, nombres = seed exacto), scale **1.0 CONFIRMADO visual** (900×442 más anchas que el card → llenan ~95%; el "redondo small → 1.15" NO aplicó). **Próximo**: siguiente producto o lo que pida.

🟡 **BUG heart en grilla "abre la publicación y no guarda" — fix probable, SIN verificar (2026-06-14)**. Founder reportó: tocar el corazón en la grilla de categoría abre la PDP y no agrega a favoritos. Investigación: el `WishlistButton` (variant card) YA está fuera del `<Link>` con `preventDefault`+`stopPropagation`, y eso está deployado (commit c64f315) → **post-hidratación un tap preciso NO puede navegar** (el evento nunca llega al Link). Diagnóstico = **mis-tap**: el target era 36px (`size-9`, < 44px mínimo táctil) → en mobile el dedo cae sobre la imagen (que es el Link) → abre PDP + no guarda. Fixes aplicados en `components/wishlist/wishlist-button.tsx`: (1) área táctil 36→44px (`size-9`→`size-11`); (2) botón interactivo desde el SSR (antes placeholder muerto pre-hidratación; gateé `animate-pop` con `mounted`). typecheck OK. ⚠️ **NO reproducido en navegador** (no puedo testear táctil desde acá) → es el mejor diagnóstico, no certeza. **Próximo paso exacto**: pushear → founder prueba en el celu; si tocando BIEN el corazón sigue abriendo la PDP, pedir cuándo pasa (siempre / solo al cargar) para un caso más raro. Si confirma que era el touch target → documentar learning "44px mínimo en botón flotante sobre card-link". Sin commitear aún.

🔵 **CONSULTA UGC: ¿extraer fotos de reviews de ML? — RECOMENDADO NO (2026-06-14)**. Founder preguntó si podía sacar fotos reales de productos de las reviews de ML y subirlas al sitio. Opinión dada: **NO** — (1) copyright: la foto de la review es del comprador, los términos de ML no te dan licencia para republicarla en tu sitio comercial; (2) scrapear ML viola sus términos (riesgo a la cuenta de vendedor que sincroniza stock/precio); (3) choca con la regla dura #7 (trust signals reales/en contexto) + calidad despareja vs estética editorial. **Alternativa recomendada (legal + alto valor): UGC propio** — pedir foto+permiso a clientes post-compra por WhatsApp; fotos de cliente en reviews del sitio; lifestyle propio. ⬜ **OFRECIDO**: armar el flujo de pedido de foto+permiso post-compra (encaja con el handoff WhatsApp) y/o reviews con foto en el sitio; + opción de verificar la norma fina (propiedad intelectual + ToS ML) con argentine-ecom. **Esperando decisión del founder.**

🟢 **FIXES UI quiz/recomendador (2026-06-14)**. (1) Emoji "Corazón" del quiz de forma de cara: 🫀 (anatómico) → ❤️ (clásico), `components/quiz/quiz-stepper.tsx`. (2) Fondo gris en la card de resultado del recomendador: el contenedor de imagen usaba `bg-muted` → cambiado a `bg-background` (blanco puro, como las grillas de categoría y el rail "para vos"), `app/(storefront)/encontra-tu-anteojo/resultados/page.tsx`. typecheck OK. Sin commitear aún.

🟢 **AJUSTE FINO SCALE The Trial (2026-06-14, founder en el grid)**. SOL (varios pases, valores finales): V2 MBLK 053/S15 antifog perfil **1.18**; V1/V3/V4 perfil **1.08**. RECETA: ambas perfil 1.03. Solo perfiles (foto del card), frentes en 1.0 (criterio Marky Ramone). Cambio de valores en `image-scale-overrides.ts`.

🟢 **CROSS-LINK SOL↔RECETA EN LA PDP — IMPLEMENTADO (2026-06-14)**. Founder lo validó ("muchas veces ven el de sol y lo quieren en receta"). Conecta las 2 versiones del MISMO modelo (6 pares hoy: rusty-misty/patien/spell/xold, vulk-katleen, vulk-the-trial). Flow de agentes: **conversion-optimizer** (prioridad, sales surface) definió ubicación (link-card liviano debajo del selector de variantes, zona de decisión; sin color de acción para no competir con el CTA; mismo tab; NO en sticky bar) + copy; **nextjs-performance** aprobó (ISR intacto, cliente static sin cookies, 0 JS, sin riesgo LCP/CLS, índice UNIQUE de slug cubierto). **Sin cambio de schema**: el vínculo se deriva por convención de slug (receta = sol + "-receta"), validado en DB. Archivos: `lib/catalog/queries.ts` (`fetchCompanionModality`, 1 query en `Promise.all` con relatedProducts → 0 latencia extra), `components/product/cross-modality-link.tsx` (NUEVO, server component, thumbnail + 2 líneas + chevron), `components/catalog/product-page.tsx` (render debajo de VariantList). Copy: sol→receta "¿Lo necesitás con tu graduación? Ver versión para anteojos de receta"; receta→sol "¿Lo querés también de sol? Ver versión con lentes solares". typecheck + lint OK. ⚠️ Deuda de proceso: la convención de slug sol↔receta es load-bearing para este link (anotado en BACKLOG para el playbook de catalog-loader). **Próximo**: ver en vivo tras deploy; o siguiente producto.

~~🟡 VULK THE TRIAL SOL — CARGA ABIERTA (2026-06-14)~~ (CERRADO arriba). Sol **AVIADOR DOBLE PUENTE unisex** (seed 71, `vulk-the-trial`). Norma: seo-strategist + catalog-loader. Frente G-Flex + patillas Monel/acetato hecho a mano + bisagra integrada, policarbonato UV400 cat 3, **19,5g** (ML lo da), large. **3 MLAs + 1 SIN MLA**, data ML: SBLK/S10 POL 125728 $86.082,29 stock 3 (primary, polarizado), MBLK 053/S15 125725 $78.751,73 stock 3 (ANTIFOG, no pol), SBLK/ORANGE 125727 $74.243,83 stock 3 (lente naranja, no pol), MDEMI-068/UPG15 POL 968279 **sin MLA** stock 0 is_active=true (carey verde G15, pedido; **precio PROVISIONAL = S10**, actualizar al alta en ML → BACKLOG). 2 POL → /polarizados. `frame_shape="aviator"` (founder define aviador; doble puente a descripción). ANTIFOG → descripción/callout (no enum, igual criterio espejado/AR). Medidas 147/50×49/15/150 (foto). **HONESTIDAD** 2/4 pol → meta_title "Unisex" no "Polarizados". SEO por seo-strategist (primaria `lentes de sol vulk` 1.300; carril AVIADOR libre; saqué "comprá en cuotas" del meta — cuotas no habilitadas). Scale 1.20/1.0 PROVISIONAL (precedente Gresent doble puente). Verificación MCP: `forma=aviator, peso 19.5, variantes=4, pol=2, stock 3/3/3/0, mla null en MDEMI, imgs 2/variante`. typecheck OK. **⬜ BLOQUEANTE founder**: subir 9 fotos a `vulk-the-trial/` (bucket VACÍO; nombres inconsistentes/dobles espacios → al subir verifico HTTP 200 + reconcilio paths + reconfirmo scale). **Próximo**: founder sube fotos → reconcilio; o siguiente producto.

🟢 **VULK RAVEN SOL — CERRADO (2026-06-14)**. El founder subió las 7 fotos (todas HTTP 200, nombres con espacios/typo coinciden exacto con el seed). **Scale CONFIRMADO visual = 1.0 en las 6** (bajé y miré las fotos): son 900×442 (2.04:1) = más anchas que el card 3:2 → con `object-contain` el anteojo ya llena ~90-95% del ancho; el 1.15 provisional habría recortado las patillas (≠ The Sil, que traía más margen y sí usa 1.15). Lente: el perfil se ve "transparente" por el ángulo (canto), el frente confirma la lente gris polarizada; la revo confirma el espejado azul. Dimensiones del seed (900×442) coincidieron exactas — sin corrección. **Rename fotos (2026-06-14, 2do pase)**: founder renombró 4 fotos a formato limpio (`RAVEN SBLK/MBLK P/F.jpg`); actualizado en DB (4 UPDATE vía MCP) + seed + scale overrides; las 7 HTTP 200, DB sin paths muertos. ⚠️ La REVO BLUE quedó con su nombre viejo (`RAVEN-MBLK-R blue ... GALERIA.jpg`) — founder no la renombró. Detalle de carga abajo. **Próximo**: redeploy/ISR para ver grilla + PDP en vivo; o siguiente producto.

🟡 **VULK RAVEN SOL — detalle de carga (2026-06-14)**. Sol **WAYFARER/cuadrado unisex** (moda, NO deportivo → sin `line`) (seed 70, `vulk-raven`). Norma: seo-strategist + catalog-loader. 3 MLAs separados, data ML vía `scripts/ml-item.ts`: SBLK/S10 POL 109142 $98.948,61 stock 5 (primary, negro brillo gris), MBLK/S10 POL 109148 $98.948,61 stock 2 (negro mate gris), MBLK/REVO BLUE 109149 $94.751,47 stock 1 (no pol, espejada azul revo + AR cara interna). 2 POL → /polarizados. `frame_shape="wayfarer"`. revo: espejado en lens_color, AR a descripción/callout (no a lens_treatment, regla del enum). **weight 26g** (ML lo da → NO va a pesos pendientes). Medidas 140/55×46/15/142 (de la FOTO). **HONESTIDAD**: 2/3 polarizadas → meta_title destaca "Unisex" no "Polarizados" (criterio Play/Patien). SEO por seo-strategist (head `lentes de sol vulk` 1.300; carril wayfarer libre en el cluster; bloque agregado a SEO_STRATEGY). Verificación MCP: `variantes=3, forma=wayfarer, peso 26, pol=2, stock 5/2/1, imgs 2/variante`. typecheck OK.

🟢 **RUSTY AND NOW SOL CARGADO (2026-06-13)**. Sol **ENVOLVENTE/deportivo** unisex (seed 69, `rusty-and-now`). Norma: seo-strategist + catalog-loader. 3 MLAs separados, data ML: SBLK/S10 POL $93.631 stock 8 (primary), MBLK/S10 POL $93.631 stock 6, REVO BLUE $89.630 stock 5 (no pol, espejada azul + AR interior). 2 POL → /polarizados. `frame_shape="envolvente"`+`line="deportiva"` (como Esvep → /deportivos; NO enum inglés). revo: espejado en lens_color, AR interior a descripción (no a lens_treatment, regla del enum). **SOLO PERFIL** (founder: no hay frente; "...pol-frente.jpg" ES perfil) → 1 img/variante + medidas. Fotos 667×442 chicas. Scale 1.1 (anteojo grande, NO 1.5 de Esvep). Medidas 140/64×39/18/108 (⚠️ varilla 108 corta, founder puede reconfirmar). SKUs cortos (16118/16122/1110, del founder). PDP 200 + fotos 200. Bloque al cluster (carril deportivo). Peso omitido → BACKLOG. **Próximo**: siguiente producto o lo que pida.

🟢 **RUSTY BLINDED SOL — CERRADO (2026-06-13)**. El founder subió las 5 fotos (todas 200, PDP renderiza), scale 1.15/1.0 confirmado visual (anteojo llena ~88%), y aclaró que el **MDEMI está sin stock pero REINGRESA** → no discontinuada, queda is_active=true (stock 0, se sube solo al reingresar). ⬜ Único pendiente menor: el SKU del MDEMI sigue asumido en 129105 (founder escribió "29105" y no lo corrigió). Detalle de la carga abajo. ~~carga ABIERTA~~

🟡 **RUSTY BLINDED SOL — detalle de carga (2026-06-13)**. Sol **REDONDO unisex** liviano 22,7g (seed 68, `rusty-blinded`). Norma: seo-strategist + catalog-loader. 2 MLAs separados, data ML: MBLK/S10 129103 $73.661 stock 8 (primary), MDEMI-MBLK/BROWN 129105 $73.957 stock 0 paused (carey, lente marrón). NINGUNA polarizada (no /polarizados). G-Flex, antirreflejo interior en las 2 → al callout (NO en lens_treatment: "antirreflejo-interno" no es valor del enum — catalog-loader; igual decisión a futuro para auditar Play/Patien que sí lo tienen, ver BACKLOG). `frame_shape="round"` (NO "redondo"; los 5 seeds con "redondo" quedan como deuda en BACKLOG). Medidas 130/48×43/22/145. Scale 1.15/1.0 PROVISIONAL. **⬜ BLOQUEANTES founder**: (1) subir 4 fotos a `rusty-blinded/` (sin eso PDP sin imágenes + scale sin confirmar); (2) **confirmar SKU 129105** (escribió "29105", asumí 129105); (3) confirmar que MDEMI paused = sin stock temporal, no discontinuada. PDP 200 (sin imágenes aún). **Próximo**: founder sube fotos + confirma → verifico scale; o siguiente producto.

🟢 **VULK THE SIL SOL CARGADO (2026-06-13) — desbloqueado**. El founder reconectó ML (token nuevo) y subió las fotos → cargado (seed 67, `vulk-the-sil`). Sol **cuadrado UNISEX** talle large, **Grilamid → `frame_material:"tr-90"`** (el enum no tiene "grilamid"; Grilamid TR-90=tr-90, catalog-loader; "Grilamid" en copy), bisagras plásticas reforzadas, 28g. 3 MLAs separados, todas polarizadas (→/polarizados): SBLK/S10 $81.064 stock 25 (primary), MBLK/S10 stock 18, MBLK/G15 stock 17 (verde G15). Medidas 144/55×51/20/145. frame_shape square. **Naming fotos**: `THESIL-...-`=perfil, `THE SIL-...`=frente (verificado bajando imágenes; G15 perfil sin guion final). Scale 1.15/1.0. SEO por seo-strategist (head `lentes de sol vulk` 1.300; saqué "óptica autorizada" del meta). PDP 200, typecheck OK. **+ Patien sol scale +0.05** (1.2/1.05→1.25/1.10) en todas menos MBLK/S10 POL (founder lo pidió). **⬜ Pendiente founder**: redeploy + ver grillas. **Próximo**: siguiente producto o lo que pida.

~~🔴 EN CURSO / BLOQUEADO: VULK THE SIL — token ML 401 + fotos sin subir~~ (RESUELTO arriba 2026-06-13). Sol **cuadrado unisex polarizado**, material **Grilamid** (no G-Flex), 28 g, talle Large, bisagras plásticas reforzadas. 3 colores: MBLK/G15 POL 128302 (verde G15), MBLK/S10 POL 128301 (gris), SBLK/S10 POL 128303 (gris). MLAs: 1391852396 (SBLK/S10), 1398498685 (MBLK/G15), 1532396744 (MBLK/S10). Medidas (foto) 144/55×51/20/145. Plan: `frame_shape='square'` (cuadrado, sin ruta dedicada — como Tour 81), gender unisex, las 3 polarized:true → /polarizados, default sol (cat3+policarbonato+uv400). Slug `vulk-the-sil`, cluster Vulk. **DOBLE BLOQUEO** (ninguno resoluble por el asistente): (1) **token ML caído** — `ml-item.ts` da HTTP 401 "invalid access token" (funcionaba más temprano hoy; se venció/rotó) → falta precio/stock/var_id reales (no se inventan). El founder debe **reconectar ML** (flujo `?ml_oauth=success&user_id=81654493`). (2) **fotos NO subidas** al bucket `vulk-the-sil/` (storage vacío para "sil") → además hay 2 sets de nombres por color (`THE SIL-...` y `THESIL-...-`) y falta verlas para mapear perfil/frente. **Próximo paso exacto**: cuando el founder (a) reconecte ML y (b) suba las 6 fotos → correr `ml-item` ×3, confirmar perfil/frente bajando las imágenes, invocar seo-strategist + catalog-loader (norma), escribir seed 67 + aplicar + scale + verificar PDP. Nada aplicado al cloud todavía.

🟢 **RUSTY PATIEN OPTICS RECETA CARGADO + PESO al sol (2026-06-13)**. Versión de RECETA del mismo frame Patien (seed 66, `rusty-patien-receta`). Norma: seo-strategist + catalog-loader. **Multivariante** (1 MLA MLA1388467447, var_id poblado — ≠ el Patien sol que eran items simples): 669K-SBLK OPTICAL 126096 var ...291 stock 1 (primary, translúcido patillas negras); MBLK OPTICAL 126090 var ...289 stock 0. $83.078 ambas. **Convención receta**: SOLO lens_compatibility (sin uv/cat3/policarbonato/polarized — founder aclaró que receta no lleva eso). Medidas 140/49×40/21/145 (foto). **Peso 23,6g** — founder lo pasó y se aplicó a AMBOS Patien (receta seed 66 + sol seed 65, mismo frame; el sol se updateó en DB + seed). Scale 1.1/1.0 (fotos 900×442 llenas, carpeta separada `rusty-patien-receta/`; NO 1.2 del sol que era 1200×589). PDP 200, fotos 200, typecheck OK. Bloque al cluster SEO con anti-canibalización sol↔receta. Peso de Patien tachado de la lista de pendientes en BACKLOG (quedan Tour 81, Play, Terdey). **Próximo**: siguiente producto o lo que pida.

🟢 **RUSTY PATIEN SOL CARGADO (2026-06-13)**. Anteojos de SOL wayfarer **UNISEX**, **bisagras metálicas flex** (≠ Play/Terdey plásticas), 4 colores (seed 65, `rusty-patien`). Norma cumplida: seo-strategist + catalog-loader. 4 MLAs separados, data ML: SBLK/S15 $73.661 stock 17 (primary, antirreflex), Revo Blue $79.373 stock 13 (espejada), MBLK/S10 POL $85.924 stock 6 (POL→/polarizados), 669K-SBLK/SG91 POL $86.011 stock 0 (POL, lente gris degradé, bicolor gris-transparente/negro; cargado activo por regla "todas aunque 0"). **Corrección founder 2026-06-13**: el 669K se había puesto mal como "sin polarizar 669K/SBLK" → en realidad es `669K-SBLK/SG91 POL`, polarizado gris degradé (DB+seed+docs corregidos, commit 19bed37). → **2/4 polarizadas** → meta NO afirma "polarizados" de TODO el modelo (honestidad, como Play). Medidas 140/49×40/21/145 (foto). frame_shape wayfarer, gender unisex. Scale **1.2/1.05** (fotos 1200×589, anteojo ~70% → zona My Crew/Tour 81; el catalog-loader frenó el 1.3/1.15 que el founder ya reportó "grande" 3 veces). PDP 200, fotos 200, typecheck OK. Bloque agregado al cluster SEO con anti-canibalización (Patien unisex+branded, Terdey polarizado-de-modelo, Play hombre). **Default global SOL aplicado** (founder 2026-06-13): cat 3 + policarbonato + 100% UV por defecto salvo aclaración — ver memoria [[default-sol-cat3-policarbonato-uv]]. **⬜ Pendiente founder**: redeploy + ver grillas + pesar (lista BACKLOG: Tour 81, Play, Terdey, Patien). **Próximo**: siguiente producto o lo que pida.

🟢 **RUSTY TERDEY SOL CARGADO (2026-06-13)**. Anteojos de SOL wayfarer **UNISEX**, **las 3 variantes polarizadas** (seed 64, `rusty-terdey`). Norma cumplida: seo-strategist + catalog-loader. 3 MLAs separados (items simples), data ML: SBLK/S10 $77.375 stock 21 (primary), MBLK/S10 $77.375 stock 16, Revo Red espejada roja $82.380 stock 1 (total 38). Las 3 `polarized:true` → /polarizados. G-Flex, policarbonato UV400 cat 3, wayfarer, gender unisex. Medidas 145/54×47/16/140 (foto). **GALERIA=perfil, AGALERIA=frente** (verificado bajando las fotos). Scale 1.15/1.0 (encuadre GALERIA-WEB). PDP 200, fotos 200, typecheck OK. Bloque agregado al cluster SEO. **4ª variante MBLUE/Revo Blue (SKU 128824, MLA1518722044, $82.711, ML stock 6) AGREGADA 2026-06-13 pero DESACTIVADA** (`is_active=false`): el founder confirma stock el martes y avisa para activar. Founder dijo "(no polarizados)" → polarized:false (⚠️ ML title dice "polarizados"). Frame azul mate, lente espejada azul. **⬜ Pendiente founder**: (1) confirmar stock del azul el martes → activar; (2) redeploy + ver grillas; (3) **pesos**: founder pesa con balanza de precisión y pasa gramos (lista viva en BACKLOG: Tour 81, Play, Terdey); si no los pasa = el fabricante no da el dato. **Gotcha aprendido**: Supabase execute_sql envuelve el batch multi-statement en transacción → si el SELECT final falla (ej. columna ambigua), revierte TODO el batch incluidos los INSERT; correr los INSERT solos. **Próximo**: siguiente producto o lo que pida.

🟢 **RUSTY PLAY SOL CARGADO (2026-06-13)**. Anteojos de SOL wayfarer hombre (seed 63, `rusty-play`, `/anteojos-de-sol/rusty/rusty-play`). Norma cumplida: `seo-strategist` + `catalog-loader`. **4 variantes en 4 MLAs separados** (items simples, var_id NULL), data de ML: C1 antirreflejo $81.090 stock 8 (primary), SBLK/S10 POL $93.631 stock 7, Revo Blue espejado $88.571 stock 5, MBLK/S10 POL $93.631 stock 4 (total 24). **2 POL → /polarizados** (polarized:true por variante). G-Flex, policarbonato UV400 cat 3, frame_shape `wayfarer` (validado: ruta + brand-filters mapean), gender male. Medidas 141/61×47/12/140 (foto). **CCCP**: el C1 no tenía perfil propio → copié `PLAY-MBLK-S10-PERFIL.jpg`→`PLAY-MBLK-S10-C1-PERFIL.jpg` vía storage copy API (Node `--env-file`; el sourcing shell de .env.local falla por línea con caracteres especiales → usar Node, no `source`). Scale **1.1/1.0** (visual: anteojo ocupa ~95% del frame, mucho más lleno que My Crew → scale bajo, NO 1.3). 10 fotos HTTP 200. **Peso OMITIDO** (ML no da → BACKLOG). Bloque Rusty Play agregado al cluster SEO. PDP 200; grillas /polarizados+/wayfarer+/rusty tras revalidación ISR/redeploy. **⬜ Pendiente founder**: redeploy + ver grillas + pesar para weight_grams. **Próximo**: siguiente producto o lo que pida.

🟢 **TUNING DE SCALE My Crew/Clems — drift de 0.01 diagnosticado + corregido a pasos visibles (2026-06-11, post-cierre)**. El founder afinó el tamaño en la grilla `/anteojos-de-receta/vulk` con varios pasos de ±0.01/±0.005 y reportó "no surten efecto" (incluso incógnito+cache). **Diagnóstico** (curl al HTML de prod, ver LEARNINGS): los valores SÍ estaban aplicados (`x-vercel-cache: HIT`, scale correcto en el HTML) — el problema era doble (ver MISTAKES): (a) 0.01 ≈ 0,8% = invisible; (b) en el Clems se tocaban CRY/SBLK pero el card muestra la **MBLK** como primaria. **Solución**: pasos visibles ≥0.05 sobre la foto primaria real. **Valores FINALES** (commit `6f37cd1`): **My Crew perfil 1.19 / frente 1.05** (−0.1 desde 1.29); **Clems** CRY/SBLK 1.195/1.045, **MBLK 1.095/0.945** (−0.05, incluida la MBLK que es la del card). Tour 81 sigue 1.3/1.15 provisional (sin fotos). **⬜ Pendiente founder**: redeploy + mirar la grilla; estos pasos SÍ se notan (8%/4%). Si hay que afinar, pasos ≥0.05. **Commits de tuning** (post-push del cierre anterior): 88ddab2, cbce26b, 0d53cb4, 5059b1d, 6f37cd1 (+ docs de este cierre).

🟡 **VULK TOUR 81 RECETA — seed aplicado, carga ABIERTA (2026-06-11, mismo turno)**. Armazón de RECETA **cuadrado unisex** (seed 62, `vulk-tour-81-receta`). Norma cumplida: `seo-strategist` + `catalog-loader` invocados. 3 variantes 1 MLA (MLA1476831627), data de ML, todas $83.443: MBLK 125890 stock 0 (primary), CRY 125892 stock 0, 315 125894 stock 1. `frame_shape='square'`. Medidas 140/50×47/21/138 (de la FOTO; texto ML 139/40). **Peso OMITIDO** (ML no da → no inventamos; founder pesa la 315 → BACKLOG). Scale: founder subió las fotos y confirmó visual → de 1.3/1.15 bajó a 1.25/1.10 y luego a **1.20/1.05** ("un poco más", 2026-06-13). Aplicado MCP, PDP da 200. **✅ Fotos YA subidas al bucket `vulk-tour-81/`** (la carga dejó de estar bloqueada). ~~⬜ BLOQUEANTE: founder NO subió las 7 fotos~~ → sin imágenes hasta que las suba (naming exacto: `TOUR 81-MBLK PERFIL/FRENTE GALERIA.jpg`, `TOUR 81 CRY PERFIL/FRENTE GALERIA.jpg`, `TOUR 81- 315 OPTICS PERFIL/FRENTE GALERIA.jpg`, `medidas.jpeg`). Después: verificar grid + ajustar scale + pesar la 315. **Próximo**: siguiente producto (link ML) o lo que pida.

🟢 **PARALLAX EN LA HOME — "A través del lente" + "Ventanas con vida" (2026-06-11)**. Founder pidió "parallax scrolling that blows minds". **El sistema de agentes nuevo se estrenó completo y funcionó**: trigger automático → `ui-motion-designer` leyó la home real y propuso 4 direcciones con wow honesto (5-10) → founder eligió la recomendada vía AskUserQuestion → `conversion-optimizer` dio GO con condiciones para la superficie de venta (todas aplicadas). **Implementado**: (1) *lens reveal* en HowWeWork — la sección dark entra al viewport a través de una máscara circular que crece con el scroll, con aro brand que se desvanece y contenido que "enfoca" (scale 1.05→1); (2) *parallax frame* en las 2 cards de CategoriesSection — la imagen deriva ±3.5% dentro del marco (overscan -inset-y-[7%] en wrapper separado del hover-scale). **Técnica: scroll-driven CSS puro** (`animation-timeline: view()`, doble gate reduced-motion + @supports) — **0 bytes de JS**, home sigue ISR ○ 174kB. Soporte: Chrome/Edge/Samsung/Safari 26+ (~80-85% del tráfico); Firefox ve la página estática (fallback invisible). Detalle del spec descartado a propósito: animar letter-spacing del título (dispara layout por frame). Archivos: `app/globals.css` (bloque parallax), `components/home/how-we-work.tsx`, `components/home/categories-section.tsx`. Learnings técnicos en LEARNINGS.md 2026-06-11 "Parallax con costo cero". **⬜ Pendiente del founder: VERLO en el celular** (Chrome o Safari actualizado) — el efecto es scroll-driven, no se puede verificar con curl; si el feel no convence, se calibra (`cover 0% cover 30%`, radios, amplitud ±3.5%) o se agrega el 2º punto de lente (hero→contenido). **Métrica de rollback (CRO)**: CTR home→categorías en GA4 no debe caer + INP <200ms mobile.

🟢 **INVOCACIÓN AUTOMÁTICA DE AGENTES ACTIVADA (2026-06-11, mismo ciclo)**. Founder preguntó "¿hay forma de que se invoquen automáticamente?". Implementados los 2 mecanismos sin costo: **(1)** descripciones de 9 agentes reescritas con "USAR PROACTIVAMENTE cuando..." (mecanismo nativo de Claude Code: auto-delegación por matching de descripción); **(2)** bloque "Triggers AUTOMÁTICOS" en CLAUDE.md (148 líneas <200 ✓) con 7 disparadores obligatorios para el asistente principal: link ML→catalog-loader · layout/queries/imágenes→nextjs-performance antes de cerrar · texto legal→argentine-ecom antes de publicar · claim óptico→optical-expert · ruta/contenido nuevo→seo-strategist · superficie visual→ui-motion-designer (+conversion-optimizer con prioridad en venta) · cambio IA→ai-features-engineer+smoke. **⬜ Ofrecido, espera decisión del founder**: mecanismo 3 = cron quincenal del `/agent-review` como tarea programada en la nube (se factura — necesita su opt-in explícito). data-analyst queda pasivo hasta post-launch.

🟢 **3 AGENTES NUEVOS CREADOS — el sistema pasa de 8 a 11 (2026-06-11, mismo turno que el review)**. Founder eligió "vamos con todos" en el AskUserQuestion + pidió uno propio de diseño. Creados (todos con tools read-only + web, el patrón ya aprobado): **(1) `nextjs-performance`** — guardián del frontend, nace del bug de los 13 días sin cache; codifica los learnings de ISR/cookies/bundle/imágenes y es el único agente con líneas base numéricas que defender (TTFB 0,2-0,5s HIT, First Load 144-153kB, 187 rutas prerendered). **(2) `catalog-loader`** — dueño del playbook de carga (stock=ML, primaria=perfil, scale vs grid regla 15, ml-item.ts, CCCP, SKU único, checklist de 10 pasos); cuando exista el admin UI (item 3) pasa a validador. **(3) `ui-motion-designer`** — pedido del founder ("diseños de vanguardia / animaciones / cosas espectaculares"); nace con presupuesto de perf INNEGOCIABLE (CSS-first, framer solo en chunks de ruta — no devolver los −40kB ganados hoy), prefers-reduced-motion obligatorio, mapa de calor de audacia por superficie (home=alto, catálogo/PDP=quirúrgico, checkout=mínimo), proceso de 3-4 direcciones visuales antes de construir, y jerarquía clara: veto técnico de nextjs-performance + prioridad de conversion-optimizer en superficies de venta. CLAUDE.md tabla actualizada (11 agentes, 139 líneas <200 ✓) + AGENT_PERFORMANCE.md con entries v1.0 + historial. **Próximo paso del plan sigue siendo: (3) admin UI de carga de productos** (salvo que el founder quiera estrenar el ui-motion-designer antes).

🟢 **REVIEW DE AGENTES: los 8 actualizados a v1.1 (2026-06-11)**. Founder pidió "chequeá los agentes y mejoralos". Hallazgos y fixes (detalle completo en AGENT_PERFORMANCE.md historial + causa raíz en MISTAKES.md 2026-06-11 "agentes congelados"): **(1) Bug sistémico de tools** — los 8 declaraban `tools: web_search, web_fetch` (nombres inválidos en Claude Code) y NINGUNO podía leer archivos del repo aunque sus instrucciones lo exigen; fix → `Read, Grep, Glob, WebFetch, WebSearch` (solo lectura, sin Edit/Bash), **aprobado explícitamente por el founder vía AskUserQuestion** (el clasificador de permisos había bloqueado correctamente el cambio autónomo). **(2) Drift de contenido corregido**: argentine-ecom (Andreani→Correo único ADR-026 con "NO re-proponer", marco legal 2026 verificado, estado real del checkout, regla dura de verificar normas con WebSearch); ai-features-engineer (tabla de modelos REALES por endpoint, adaptive thinking obligatorio, patrón fetch+tool use+Zod en vez del ejemplo con modelo que se retira el 15/6 + JSON.parse, features marcadas como implementadas, rx:smoke); content-writer/seo-strategist/conversion-optimizer (SIN número de matrícula — decisión founder 2026-06-09; logo logística solo Correo; flag checkout; /marcas hub; lección ISR/cookies); optical-expert (→BRANDS.md); data-analyst (→track.ts). **Próximo paso del plan aprobado sigue siendo: (3) admin UI de carga de productos.**

🟢 **ITEM 1 HECHO: política de devolución DEFINITIVA — los 3 `[PENDIENTE]` legales eliminados (2026-06-11)**. Founder pasó los datos (10 días corridos desde entrega; sin uso + etiquetas; "envío lo paga el comprador"; recetados solo garantía; cambios sin contestar). **Verificación legal previa con `argentine-ecom` + web search corrigió 3 supuestos** (ver LEARNINGS 2026-06-11 "Texto legal"): (a) el envío de devolución en ARREPENTIMIENTO lo paga el VENDEDOR — irrenunciable (art. 34 LDC + 1115 CCyC), el "comprador paga" del founder se aplicó solo a cambios voluntarios donde SÍ es legal; (b) garantía legal = 1 AÑO para nuevos (Ley 27.701), no 6 meses — corregida también `/defensa-del-consumidor` que lo tenía mal; (c) botón de arrepentimiento ya no obligatorio (Res. 139/2025 derogó 424/2020) pero se mantiene como trust signal. Cambios voluntarios: 30 días — NO fue invento, ya estaba definido en BUSINESS_POLICIES.md §4 desde antes. Recetados a medida: excluidos de arrepentimiento/cambio por CCyC art. 1116 inc. a (respaldo legal exacto del instinto del founder), garantía completa por defectos. **Páginas reescritas**: `/politica-de-devolucion` (estructura: resumen rápido + arrepentimiento + cambios 30d + exclusiones + garantía 1 año + trámite), `/boton-de-arrepentimiento` (reintegro 10 días hábiles, envío a cargo nuestro, sin PlaceholderNote, contacto vía getBusinessInfo), `/defensa-del-consumidor` (garantía 1 año). BUSINESS_POLICIES.md §4 reescrita como DEFINITIVA + changelog. Build OK, 3 páginas estáticas (1d). **⬜ Defaults elegidos por el asistente que el founder puede ajustar en 1 línea**: reintegro "10 días hábiles de recibido el producto". **Próximo item del plan aprobado: (3) admin UI de carga de productos.**

🟢 **PLAN DEL AUDIT APROBADO + ITEM 2 HECHO: lector de receta migrado a Opus 4.8 (2026-06-11)**. Founder aprobó el plan ("hagamos tus recomendaciones") y preguntó cuánto mejor hago las 20 guías vs Opus 4.8 — respuesta honesta dada: la diferencia real está en sostener el corpus de 20 como sistema (interlinking, consistencia terminológica, criterio YMYL de qué NO afirmar), marginal en una guía individual; se acordó arrancar con UNA guía piloto para calibrar con feedback de la regente cuando lleguemos a ese item. **Item 2 ejecutado**: `app/api/prescription/route.ts` — extracción `claude-sonnet-4-6`→`claude-opus-4-8` (visión high-res 2576px, mejor OCR de recetas borrosas) + `budget_tokens: 2000` (deprecado) → `thinking: adaptive` + `max_tokens` 4096→8192; el verificador adversarial QUEDA en Sonnet 4.6 a propósito (modelo distinto = modos de falla distintos, y más rápido). **Smoke test nuevo `pnpm rx:smoke`** (`scripts/prescription-smoke.ts`, patrón mp-smoke): valida la combinación modelo+thinking+tools+few-shot contra la API real sin levantar el server — pasó (200, tool_use OK, isPrescription=false para PNG 1×1, 9s, thinking autorregulado 25 tokens). AI_PROMPTS.md → PROMPT-001 v1.2 + changelog; ARCHITECTURE.md § IA actualizado. **⬜ Post-deploy: verificar el endpoint en prod + founder prueba con una receta REAL en /lector-de-receta** (el smoke valida parámetros, no calidad de OCR). **Próximos items del plan aprobado**: (1) política de devolución — ESPERANDO que el founder pase plazos/condiciones reales (reemplaza el `[PENDIENTE]` legal); (3) admin UI de carga de productos; (4) guías (piloto primero).

🔵 **AUDIT DE GAPS + LISTA DE MEJORAS ENTREGADA (2026-06-11) — esperando decisión del founder**. Founder pidió audit de cosas pasadas por alto + lista de agregados posibles. Hallazgos clave: **(1)** las 4 features de IA están operativas (chat RAG con pgvector real, lector receta, face-shape, DNP) pero `app/api/prescription/route.ts:269` usa `budget_tokens` (DEPRECADO en Sonnet 4.6 — migrar a `thinking: adaptive`) y ningún endpoint usa structured outputs ni prompt caching; chat corre en Haiku 4.5, resto en Sonnet 4.6 (ARCHITECTURE.md dice "Sonnet 4" — doc desactualizado); **(2)** NO hay paginación en categorías (riesgo a futuro, hoy OK); **(3)** `/politica-de-devolucion` tiene `[PENDIENTE]` — viola regla de negocio #6 (botón arrepentimiento/devolución visibles); **(4)** contenido: 1 guía publicada + 3 drafts de 25 planificadas (20%); **(5)** carga de productos = seeds SQL manuales (sin admin UI; los endpoints ML import-preview/find-item ya existen como piezas); **(6)** búsqueda básica sin facets; **(7)** 4 endpoints ML admin sin consumidor (debug). Schema de PDP OK (AggregateOffer+Brand, sin aggregateRating — correcto, no hay reviews). Opciones ofrecidas al founder: admin UI de carga, guías de contenido, upgrade lector de receta (Opus 4.8 + structured outputs), reviews reales post-compra, paginación. Pendiente: founder elige por dónde seguir.

🟢 **OPTIMIZACIÓN DE PERFORMANCE, parte 2 — framer-motion fuera del bundle global (2026-06-11, mismo día)**. Founder: "¿se puede mejorar más?" → "dale hacelo". Los 12 componentes que cargaban en TODAS las páginas e importaban framer-motion (~34kB gz) se convirtieron a CSS/vanilla: mega-menú del header (`animate-in`), MagneticButton (mousemove + transition con overshoot), ScrollProgress (rAF + transform directo), BackToTop/FloatingWhatsapp/CookiesBanner/CompareBar/FloatingChat (tailwindcss-animate; animaciones de SALIDA omitidas a propósito — desaparición instantánea), NewsletterForm del footer, FaqAccordion (CSS grid-rows 0fr→1fr; **bonus SEO: las respuestas quedan siempre en el HTML**, antes solo al abrir), Wishlist/CompareButton (keyframe `pop` nuevo en tailwind.config). **Resultado build**: First Load JS categorías 185→144 kB, PDPs 193→153 kB, guías 187→144 kB (−40 kB c/u, −21%). Framer queda SOLO en rutas puntuales: home (hero/letter-reveal/tilt-spotlight), tools, descubrir (swipe-deck), FAQ search. Smoke test local: todas las páginas 200. **⬜ Pendiente del founder: revisión visual** — las animaciones se replicaron pero no son idénticas al pixel (springs → cubic-bezier); chequear mega-menú hover, banner de cookies, barra comparar, corazón de favoritos, accordion de FAQs en marcas.

🟢 **OPTIMIZACIÓN DE PERFORMANCE — sitio entero pasado a ISR real + cache de imágenes (2026-06-11)**. Founder: "mejorá las cargas de imágenes y la respuesta general, optimizá todo". Audit en producción reveló que **TODAS las páginas públicas se servían dinámicas** (`cache-control: no-store`, `x-vercel-cache: MISS` siempre; TTFB home 2,1s, categoría 1,1s) a pesar del `revalidate = 300`: el ISR estaba anulado. Causas raíz (4, todas `cookies()`/searchParams en el render path): **(1)** `CompareBarWrapper` (server) en el layout del storefront leía el cookie del comparador → TODAS las páginas dinámicas; **(2)** `queries.ts`+`metadata.ts` usaban el cliente Supabase con cookies en 5+5 funciones (marca/PDP/related) — data 100% pública; **(3)** `RecentlyViewed` (server) leía cookie `oc_recent` → home + PDPs dinámicas; **(4)** la PDP llamaba `getCurrentUser()`+`getMyAlertFor()` server-side solo para el botón de alertas. Además **las páginas de categoría leían `searchParams`** (filtros) → dinámicas siempre. **Imágenes**: los archivos eran livianos (5-7KB AVIF) pero el optimizador de Vercel re-optimizaba cada hora (Supabase sirve `max-age=3600`; los seeds no setearon cacheControl) → TTFB 0,4-0,8s por imagen × 174 imágenes en categoría. **Fixes (commit de este turno)**: CompareBar y RecentlyViewed ahora 100% client-side (leen cookie en browser + APIs nuevas `/api/compare/thumbs` y `/api/recently-viewed/cards`, cacheables s-maxage=300); queries/metadata → `createStaticClient`; CreateAlertButton autoresuelve sesión leyendo cookie `sb-*` (sin supabase-js en bundle, evitó +64kB); categorías sol/receta → filtrado client-side con URL como fuente de verdad (page ISR, fallback de Suspense renderiza el grid completo en HTML estático para SEO; chips de filtro ahora instantáneos); `images.minimumCacheTTL = 31 días` (⚠️ regla nueva: si se REEMPLAZA una foto, subirla con OTRO nombre de archivo); middleware saltea `getUser()` si no hay cookies `sb-*`. **Resultado verificado**: build pasa de 147 → **187 rutas prerenderizadas** (home, categorías, PDPs todas ISR 5m); smoke test local prod: home 52ms, categoría 8ms, PDP 6ms (antes 600-2100ms); grid completo presente en HTML estático (SEO OK). `/favoritos`, `/comparar`, carrito/checkout/cuenta siguen dinámicas (correcto, personalizadas). **⬜ Post-deploy**: verificar `x-vercel-cache: HIT` en prod + chequear en pagespeed.web.dev; queda en BACKLOG dinamizar framer-motion (~60kB) y backfill de cacheControl en objetos viejos del bucket (mitigado por minimumCacheTTL).

🟠 **Founder decide ESCONDER cart + checkout → volver a vitrina + WhatsApp (2026-06-09)**. NO requiere código: el flag existente `NEXT_PUBLIC_CHECKOUT_ENABLED` ya lo hace. Acción del founder: poner `NEXT_PUBLIC_CHECKOUT_ENABLED=false` (o borrarla) en Vercel Production + **redeploy** (es `NEXT_PUBLIC_` → necesita redeploy). Efecto: botón "Agregar al carrito" → "Consultar por WhatsApp", CartBadge oculto, `/checkout` → 404, `/carrito` vacío. Webhook + admin NO dependen del flag (siguen vivos). **→ Go-live Fase 2 (cobrar con MP prod) queda EN PAUSA** hasta nueva decisión. Todo reversible (volver el flag a `true`). ⬜ Pendiente: founder hace el cambio en Vercel + avisa para verificar en vivo.

🟢 **Chequeo de pre-lanzamiento — TODO CORRECTO (2026-06-09)**. Founder pidió verificar que todo esté bien. Resultado (sin hallazgos negativos): **Código** typecheck limpio + último código deployado y sirviéndose en prod (CSP header presente, alt text live, cotizador CP-only sin select). **Sitio en vivo** home/categorías/PDP/guía/tools → 200, `/checkout` → 307 (login), `/admin` → 404 (gateado), sitemap+robots OK. **Datos** 9 órdenes todas `cancelled` (slate limpio, 0 pedidos reales colgados), 0 productos activos sin foto, 0 placeholders `[PH]`, stock test corregido (Arvin 4 / Day Light 3). **Seguridad** RLS ON en todas las tablas sensibles (orders/order_items/addresses/prescriptions[4 policies]/profiles/etc.); `coupons` RLS-on + 0 policies = deny-all a clientes (validación server-side, correcto); rate-limit + CSP activos; 0 self-claims de matrícula. **Infra** checkout prendido en prod, webhook `{ok:true}`, Resend verificado. **Veredicto: sano y listo; lo único entre esto y vender = Fase 2 (creds MP prod).** **⬜ Próximo**: Fase 2 go-live (founder).

🟢 **Mejoras post-features — rate-limit + CSP (2026-06-09)**. Founder: "qué más mejorar" → "vamos con todo". Hechas + pusheadas este turno: **(1) Rate-limit + cache del cotizador** `/api/shipping/quote` — `lib/rate-limit.ts` (fixed-window 30/min por IP → 429+Retry-After; `TtlCache` 10min por CP; en memoria V1, por-instancia, sin Redis). Probado: 2º request idéntico `cached:true`, burst corta en ~30 con 429. Cierra la deuda de superficie de abuso (MISTAKES 2026-06-08). Commit `d4e1e15`. **(2) CSP Report-Only** en `next.config.mjs` — no bloquea, solo reporta (afinar orígenes MP/Supabase/GA4/Vercel/cámara antes de enforcing). Header verificado en runtime. **(3) BACKLOG**: sacados ítems de matrícula obsoletos (decisión 2026-06-09). Commit `b84a917`. **✅ (a) alt text HECHO (2026-06-09, commit `3e54347`)**: helper `lib/catalog/image-alt.ts` (marca+nombre+categoría) en product-card (grid) + related-products + galería PDP. Insight: el bloque de imagen está `aria-hidden` → el alt descriptivo es solo-SEO, no toca a11y (ver LEARNINGS). Verificado en `/anteojos-de-sol`. **⬜ Resto de "todo" (necesitan input/data del founder)**: (b) **sticky add-to-cart mobile** en PDP (lo construyo, pero necesita revisión visual en pantalla); (c) **seguimiento Correo en vivo** para el cliente (necesita un nº de tracking REAL para validar el quirk GET+body de la API + `fetchCorreoTracking` hoy usa query param que da 415 → hay que pasarlo a https nativo). **⬜ GO-LIVE (parte del founder, lo más importante para vender) — checklist ESCALONADO entregado (2026-06-09)**. **Fase 1 (probar en el sitio real, SIN plata real)**: en Vercel Production setear `NEXT_PUBLIC_CHECKOUT_ENABLED=true` + `BUSINESS_ADMIN_EMAIL` + `CART_COOKIE_SECRET` (random, distinto al local) — **dejando las creds MP de PRUEBA** (el deploy ya las tiene; por eso el webhook del sandbox funcionó) → redeploy → comprar con tarjeta APRO en `opticacarballo.com.ar` → confirmar orden+mail+webhook en prod. + Supabase Auth: Site URL + redirect URLs de prod. **Fase 2 (plata real)**: swap `MP_ACCESS_TOKEN`/`MP_PUBLIC_KEY` a `APP_USR-` (prod) + `MP_WEBHOOK_SECRET` del webhook de prod (configurar webhook en panel MP modo producción) → redeploy. Resend ya verificado. **✅ FASE 1 VERIFICADA (2026-06-09)**: founder seteó las env vars en Vercel + redeployó; el asistente confirmó desde afuera que el checkout está PRENDIDO en prod (`opticacarballo.com.ar/checkout` → 307 no 404, `/carrito` → 200, `/api/mp/webhook` → `{ok:true}`). El sitio corre con creds MP de PRUEBA → cobra con tarjetas de test, NO plata real todavía. **Próximo paso exacto: FASE 2** (founder) — swap a creds MP `APP_USR-` (producción) + webhook en panel MP modo prod + `MP_WEBHOOK_SECRET` de prod → redeploy → 1 compra real chica de validación. Recién ahí cobra plata real.

🟢 **MP e2e VALIDADO (webhook real) + cleanup de pruebas con un error de stock (2026-06-09)**. Founder completó una compra de prueba real (sandbox MP, tarjeta APRO): `OC-2026-00009` quedó `paid` con `mp_payment_id=1327385348` y `paid_at` → **el webhook MP corrió de verdad** (MP notificó la `notification_url` de prod, que comparte la DB cloud; la página `/checkout/exito` es SOLO informativa, NO marca paid — confirmado en código). Flujo de pago validado de punta a punta. **Cleanup pedido por founder**: cancelé `OC-2026-00008` (pending) + `OC-2026-00009` (paid), ambas `status=cancelled` + `stock_released_at` seteado ✓. **⚠️ ERROR de stock**: reintegré vía SQL crudo (RPC `increment_variant_stock` NO idempotente, en query multi-statement) y se aplicó 2× → stock +1 de más: Vulk Arvin (112941) = 5 (correcto 4); Vulk Day Light (194185) = 4 (correcto 3). El classifier de prod bloqueó (bien) el SET correctivo. Ver MISTAKES 2026-06-09. **✅ Resuelto (founder decidió)**: el stock lo ajusta el founder en ML (su fuente de verdad → sincroniza a Supabase). Referencia: Supabase muestra Arvin 5 / Day Light 4 (inflados por mi error); real pre-prueba = 4 / 3. No toco más la base. **⬜ Features pedidas por founder (sin empezar)**: (1) **"Retomar pago"** en pedidos `pending` desde `/mi-cuenta/pedidos` (hoy el detalle es solo informativo, no hay forma de re-pagar una orden pendiente; la orden guarda `mp_preference_id`); (2) **selección múltiple (checkboxes) en `/admin/pedidos`** para generar envíos de Correo EN LOTE + indicador "envío ya generado" por pedido (hoy es 1×1 vía `generateShipmentAction`). Founder eligió hacer LAS DOS, en orden. **✅ (1) Retomar pago CONSTRUIDO (2026-06-09)**: core compartido en `lib/mp/preferences.ts` (`createPreferenceFromItems` privado + `createCheckoutPreferenceFromOrder`; `createCheckoutPreference` mantiene su firma → cero cambio al checkout) + `resumeOrderPaymentAction(formData)` en `lib/checkout/actions.ts` (valida dueño vía RLS con `fetchOrderById` + estado `pending`, recrea preferencia, `updateOrderMpPreference`, redirige a MP; en fallo → /checkout/pendiente?mp_error=1) + botón "Pagar ahora" en `components/account/order-detail.tsx` (solo si `pending`, debajo del tracker). typecheck+lint OK; la ruta compila (307). **⬜ Falta verificación runtime** (necesita un pedido pending real + login → founder; cancelé el único pending que había). **✅ Retomar pago + CP-only commiteados y pusheados (commit `34b004e`)**. **✅ (2) Envíos en lote CONSTRUIDO (2026-06-09)**: migración aditiva `orders.shipment_imported_at` (aplicada a cloud vía MCP, ver CLOUD_APPLIED) + `generateShipmentsBulkAction` + core `generateShipmentForOrder` (setea `shipment_imported_at` con claim sobre null, idempotente) en `app/admin/pedidos/actions.ts` + `components/admin/orders-admin-list.tsx` (checkboxes, "seleccionar los N sin envío", botón "Generar envíos (N)", badge "Envío generado" por pedido, resumen post-acción) + `ShipmentControl` con `alreadyGenerated` en el detalle. **✅ (3) Factura por LINK CONSTRUIDO (2026-06-09, founder eligió link, no upload)**: `setOrderInvoiceAction` (admin pega URL http(s) → `orders.invoice_url` + opción "avisar al cliente por mail" → `sendInvoiceToCustomer`) + `components/admin/invoice-control.tsx` en el detalle admin; el cliente YA veía "Ver factura" si hay `invoice_url`. typecheck+lint OK; rutas admin compilan (404 = gate de `requireAdmin`). **⬜ Verificación runtime de las dos** (necesita admin + pedidos reales → founder). **⬜ Sin commitear**: factura + envíos en lote + migración + `types/supabase.ts` + docs.

🟢 **Cotización de envío en el carrito ahora es CP-only (2026-06-09)**. Founder (durante el test de MP): el estimador del carrito debe cotizar con SOLO el CP, sin obligar a elegir provincia (el botón "Calcular" quedaba gris con el CP cargado por `disabled={!province...}`). **Hallazgo**: la API de Correo `/rates` cotiza con SOLO el `postalCodeDestination` — la provincia NI se le manda; solo se usaba para el fallback por zonas. **Cambios**: `components/cart/shipping-calculator.tsx` (saqué el `<select>` de provincia; el CP de 4 díg es el único campo requerido; el botón habilita con CP válido; la provincia queda como valor OPCIONAL pre-cargado de la dirección del user logueado, solo para afinar el fallback) · `app/api/shipping/quote/route.ts` + `lib/shipping-server.ts` (`provinceName` opcional; sin provincia → zona conservadora `interior_lejano`, no underprice). **NO toca el checkout real** (sigue usando la dirección guardada con su provincia); el widget del carrito es solo un pre-estimado y el precio cobrado lo recalcula el checkout. Verificado: typecheck + lint OK; `POST /api/shipping/quote` con solo CP devuelve tarifas reales distintas por CP (3342 → $7.947/$5.169; 1414 → $9.991/$6.881); `/carrito` ya sin el select. Nota cosmética: el `zoneLabel` de la respuesta queda en "NEA/NOA/Cuyo" (label del fallback) pero NO se muestra en el widget. **⬜ Sin commitear** (decidir si va con el resto al cerrar el test de MP).

🔵 **MP go-live — audit de readiness (2026-06-09)**. Founder: "ya podés habilitar MP con las credenciales correctas". **Aclaración clave**: habilitar NO es un cambio de código (el código ya está) — es setear env vars en Vercel + configurar el webhook en el panel de MP + prender el flag + redeploy. Son acciones de dashboard que el asistente NO puede hacer (no ve ni setea secretos de Vercel). **Código: production-ready ✅** — `lib/mp/client.ts` (singleton lazy, detecta TEST/PROD por prefijo), `lib/mp/webhook.ts` (firma HMAC fail-closed en prod, fetch del payment, mapeo de estados), `app/api/mp/webhook/route.ts` (idempotencia, anti-subpago, release de stock en cancel/refund, emails cliente+admin SOLO en la transición a paid). `pnpm mp:smoke` VERDE con TEST (preferencia creada, live_mode prueba → wiring OK). Bug del trigger de la 1ra compra ARREGLADO y verificado aplicado al cloud (`20260608010000`). **⚠️ HALLAZGO**: `BUSINESS_ADMIN_EMAIL` AUSENTE en `.env.local` → `getAdminEmail()` devuelve null → NO se manda el aviso de venta al admin (= la falla "la 1ra venta no avisó al founder"). Setearlo en Vercel es OBLIGATORIO. **Estado env local**: MP_ACCESS_TOKEN/PUBLIC_KEY = TEST (correcto dev); WEBHOOK_SECRET + NOTIFICATION_URL set; NEXT_PUBLIC_CHECKOUT_ENABLED ausente (off); RESEND_API_KEY + FROM set; BUSINESS_ADMIN_EMAIL ausente. **Checklist go-live (Vercel Production)**: `MP_ACCESS_TOKEN=APP_USR-…`, `MP_PUBLIC_KEY=APP_USR-…`, `MP_WEBHOOK_SECRET` (del panel MP), `MP_NOTIFICATION_URL=https://opticacarballo.com.ar/api/mp/webhook`, `NEXT_PUBLIC_CHECKOUT_ENABLED=true`, `BUSINESS_ADMIN_EMAIL=<email founder>`, `RESEND_*` (con dominio verificado en Resend) + en panel MP developers: webhook con evento "Pagos" → copiar secret + redeploy. **⬜ Esperando founder**: confirmar dónde están las creds PROD (¿ya cargadas en Vercel? ¿webhook configurado en panel MP?) + decidir si primero corremos un e2e local con TEST (riesgo cero) o vamos directo al go-live guiado. Conecta con MISTAKES "activar checkout sin verificar TODAS las env vars". **▶ TEST LOCAL EN CURSO (founder eligió "probemos en local primero", 2026-06-09)**: (1) `.env.local` → `NEXT_PUBLIC_CHECKOUT_ENABLED=true` + `BUSINESS_ADMIN_EMAIL=juanmirande10@gmail.com` (gitignored, NO se commitea, etiquetado "test local"). (2) Checkout surface VIVA en local: `/checkout` pasó de 404 a 307 (redirige a login), `/carrito` 200 con CTA de compra. (3) **Infra de email VALIDADA**: nuevo `scripts/email-smoke.ts` (self-contained — no importa `lib/emails` porque es `server-only` y rompe en tsx) mandó un mail real desde `Óptica Carballo <hola@opticacarballo.com.ar>` → entrega OK (id Resend) = **dominio verificado en Resend + entrega a la casilla del founder funciona** (la falla previa, cubierta a nivel infra). **Founder CONFIRMÓ recepción en INBOX (no spam) 2026-06-09** → buena reputación de remitente. (4) `pnpm mp:smoke` verde con TEST (preferencia, live_mode prueba). **⬜ Falta para e2e completo**: el round-trip de pago — founder hace una compra con tarjeta TEST (APRO) en el sandbox (crea la orden `pending` → valida el trigger en local) y el asistente SIMULA el webhook con ese payment id real (MP no puede pegarle a localhost) → ejercita firma + fetchPaymentById + orden→paid + emails con templates reales. `scripts/email-smoke.ts` queda sin commitear (decidir si se suma como tool, junto a un script `email:smoke` en package.json).

🟡 **Diseño de guías — auditoría + triage de ideas del founder (2026-06-09)**. Founder pasó ideas de UX/diseño para las guías (romper muro de texto, CTA del lector destacado, listas visuales, widgets inline, scrollytelling, audio TTS, bento, modo oscuro, tipografía fluida). **Audit (regla 14)**: el sistema YA tiene casi todo — `mdx-components.tsx` expone `ToolCta` (bloque gradiente del lector IA), `KeyTakeaway`, `MedicalDisclaimer`, `ProductCta`, `RelatedProducts`, tablas con scroll mobile; `RevealOnScroll` ya anima al scrollear; prose con jerarquía H2/H3. **El problema NO es el sistema: `content/guias/como-leer-receta-anteojos.mdx` está en markdown plano y no usa ninguno** (el lector aparece como link de texto = queja exacta del founder). → ~80% de los "fundamentos" = re-maquetar, sin libs. **Triage**: SÍ (barato, sin libs) = imagen real de receta anotada (hoy `heroImage: null`), lector embebido inline (show-don't-tell; `components/tools/prescription-reader.tsx` ya existe), barra de progreso de lectura. DESPUÉS = resumen IA con toggle (pre-gen en build), bento grid (CSS), tipografía fluida (clamp). NO ahora = audio TTS voz-clonada (proveedor pago nuevo + costo/artículo + ~0 tráfico → gimmick, viola regla 6), scrollytelling pesado (perf/gimmick), modo oscuro con toggle (decisión de todo el sitio, aparte). **⚠️ HALLAZGO COMPLIANCE (leftover de la decisión de matrícula)**: `como-leer-receta-anteojos.mdx` líneas 72/169 ("la regente") y 221 ("La regente revisa... María Carlota valida"), + `mdx-components.tsx:44` (MedicalDisclaimer de TODAS las guías: "revisada por una óptica matriculada"). Las refs a "oftalmólogo matriculado" (251/279) son legítimas (el médico, no nosotros). ~24 ocurrencias de regente/matriculad en código → la remoción anterior tocó frontmatter/JSON-LD pero NO el cuerpo de las guías ni el disclaimer compartido → falta barrido. Ver MISTAKES 2026-06-09. **✅ FOUNDER ELIGIÓ "fix matrícula + re-maquetar" — HECHO (2026-06-09)**. (1) **Barrido matrícula/regente**: corregidos todos los self-claims user-facing → `mdx-components.tsx:44` (MedicalDisclaimer de TODAS las guías: "revisada por nuestro equipo de óptica"), `app/(storefront)/medidor-de-dnp/page.tsx` (description + FAQ), `app/(storefront)/politica-de-devolucion/page.tsx`, `components/tools/pd-measure-tool.tsx` (PrivacyNote), `lib/pd-measure/types.ts` (label de precisión), `lib/pd-measure/calculate.ts` (2 softWarnings), `como-leer-receta-anteojos.mdx` (líneas 72/169/221). `article-jsonld.tsx`: solo comentario viejo (el JSON-LD ya era data-driven, `article-authors.ts` tiene `matricula: null` → NO emite matrícula ni nombre de regente). Verificado: **0 ocurrencias de "regente"/"María Carlota" en el HTML renderizado**. Quedan refs legítimas a "oftalmólogo matriculado" (el médico) + comentarios internos + campos de tipo `regente*` en `business.ts` (env vacías, no se renderizan). (2) **Re-maquetación de la guía de receta** (`como-leer-receta-anteojos.mdx`, que estaba en markdown plano sin usar ningún componente): `KeyTakeaway` arriba (resumen escaneable de los 5 valores) + `ToolCta tool="lector-de-receta"` (bloque gradiente destacado, antes era link de texto) + las "4 partes" convertidas a `<Steps>`/`<Step>` (componente NUEVO reutilizable: tarjetas con badge numerado, agregado a `mdx-components.tsx`) + `ToolCta tool="medidor-de-dnp"` en la sección DNP + `CategoryCta` a /anteojos-de-receta + `MedicalDisclaimer` al final (reemplaza el disclaimer en cursiva plano). Patrón validado contra `astigmatismo.mdx`/`miopia.mdx` (ya usaban estos componentes — la de receta era la única sin tratamiento). typecheck + lint OK; guía renderiza HTTP 200, MDX compila, componentes presentes. **✅ Commiteado + pusheado a main** (`1c01997`, 2026-06-09). **⬜ Diferido (no pedido aún)**: imagen real de receta anotada, lector embebido inline funcional, barra de progreso de lectura.

🟠 **Decisión founder: SACAR claims de matrícula/regente del sitio — HECHO (2026-06-09)**. El founder decidió NO mostrar matrícula y eliminar el framing "óptica/regente matriculada", dejando "óptica" + experiencia. Reglas aplicadas: (1) sin auto-claims de "matriculado/a" sobre Óptica Carballo ni su gente; (2) receta = **del oftalmólogo** (no "técnico óptico matriculado" — en Argentina receta la da el médico); (3) se mantienen referencias genéricas al **oftalmólogo** (el médico que receta); (4) se quitó el **rol/nombre profesional de la regente** (María Carlota): tarjeta en sobre-nosotros, employee en JSON-LD, reviewer en las 3 guías, FAQ de regente, footer, términos, OG image, system-prompt del chatbot, emails/estados de pedido ("Revisado por óptica"), copys de herramientas (lector, recomendador, pd-measure), llms.txt, metadata de catálogo. Commits `9eca7c0` (UI/catálogo/guías) + `d07b843` (backend/páginas/JSON-LD). Quedan solo: comentarios internos, env vars vacías (REGENTE_*), y refs al oftalmólogo. typecheck+lint OK. **⚠️ Tradeoff**: esto saca la señal de E-E-A-T #1 que la auditoría destacaba (decisión consciente del founder). **⬜ Falta push** de estos 2 commits + verificar en deploy.

🟢 **Auditoría completa del sitio (SEO + GEO + conversión + código) + fixes — 2026-06-09**. Se corrió una auditoría con 5 agentes en paralelo (técnico, contenido, GEO, visual, código) sobre `opticacarballo.com.ar` + repo. **Hallazgos verificados a mano** (2 agentes se equivocaron: el de GEO dijo "0 JSON-LD, crítico" → FALSO, hay Product/AggregateOffer/BreadcrumbList/FAQPage; el visual reportó truncación de chips → FALSO, ya usan flex-wrap). **Fixes aplicados** (commits): **`f5ceedb` (🔴 crítico)** — los 6 endpoints `/api/admin/*` + `debug-last-error` estaban SIN auth (verificado en vivo: `ml-me` devolvía datos de la cuenta ML a cualquiera) → `getAdminUserOrNull()` + guard 401; webhook MP ahora valida monto (anti-subpago), libera stock al rechazar/cancelar (3ra puerta), exige firma en prod; cron con red de seguridad. **`c93a101` (SEO)** — guías al sitemap (faltaban), Product schema con image+seller, títulos duplicados, AFIP badge lazy, headers de seguridad (sin CSP), `public/llms.txt`. **`b3f796d` (UX+mdx)** — `remark-frontmatter` (saca el YAML del render de guías), banner cookies a 1 línea, home hero mobile (texto/CTA primero, antes la foto tapaba el fold), línea de confianza en carrito vacío. typecheck+lint OK en todo. **⬜ Diferido a BACKLOG** (decisión founder): cache/rendering `no-store` (arquitectónico, sesión aparte), CSP, alt-text de imágenes, barra fija de compra en PDP mobile. **⬜ Necesita datos del founder**: nº de matrícula + dirección/horario (para footer/sobre-nosotros/llms.txt). **⬜ Falta**: pushear los commits + el founder verifica en deploy (PIN, guías sin YAML, hero mobile).

🟢 **Seguridad admin (PIN 2FA) + ciclo de vida del stock (reintegro al cancelar + auto-cancelar abandonados) — CONSTRUIDO, DEPLOYADO y PROBADO EN PROD (2026-06-09)**. Tres entregas del founder en esta sesión:
**(1) PIN de 4 dígitos como 2º factor del admin** (commit `7f31ff7`). Tras el login con email autorizado, `/admin/*` pide un PIN (env `ADMIN_PIN`, 4 díg). Cookie de SESIÓN firmada con HMAC (reusa `CART_COOKIE_SECRET`); se re-pide al cerrar el navegador. Si `ADMIN_PIN` no está seteado, el 2º factor se omite (a prueba de errores). `lib/auth/admin.ts` separado en `requireAdminEmail` (solo email, lo usa la pantalla de desbloqueo) + `requireAdmin` (email+PIN, el resto). Nuevos: `lib/auth/admin-pin.ts` (verify timing-safe + cookie + límite 5 intentos/5min en memoria, best-effort) + `app/admin/desbloquear/` (page+form+action). PIN auto-envía al 4º dígito + se limpia si falla (commit `b9cf57c`). Founder ya cargó `ADMIN_PIN` en Vercel. **Verificado en prod**: `/admin/desbloquear` responde 404 (gateado, no 500 → `CART_COOKIE_SECRET` presente).
**(2) Reintegro de stock al CANCELAR** (commit `f914147`). El stock se descuenta al crear la orden (`reserve_stock`); faltaba devolverlo al cancelar. Migración `20260608020000_orders_stock_release.sql` (col `stock_released_at` + backfill de cancelled/refunded viejos) **aplicada al cloud + verificada**. `releaseOrderStock(orderId)` en `lib/checkout/orders.ts`: claim atómico sobre `stock_released_at` (idempotente — `increment_variant_stock` suma cada vez, hay que protegerlo) + reintegro DB + push a ML. `updateOrderStatusAction` lo dispara solo en la transición a `cancelled`. **PROBADO EN PROD**: OC-2026-00007 (creado post-migración) → cancelado → `stock_released_at` en el mismo instante + 1u de SKU 958211 de vuelta.
**(3) Auto-cancelar abandonados (24h)** (commit `f914147`). Cron `app/api/cron/cancel-abandoned-orders` (hourly, `CRON_SECRET`): cancela `pending` de +24h sin pago + libera stock (mismo `releaseOrderStock`). `vercel.json` actualizado. **Verificado en prod**: responde 401 sin auth (deployado + guard OK). UX extra (commit `b9cf57c`): el checkout ya NO filtra el error crudo de Postgres al cliente si el stock se agota en la carrera ("se quedó sin stock, revisá tu carrito").
**Estado**: commits `7f31ff7`, `f914147` ya en `origin/main` (deployados). **⬜ Falta**: (a) push de `b9cf57c` (los 2 pulidos de UX) — `git push origin main` lo hace el founder (el guard de Claude bloquea push directo a main); (b) founder confirma el PIN entrando a `/admin/pedidos`. **⚠️ Edge anotado en BACKLOG**: el cron de 24h podría cancelar un pedido "pago OK pero webhook MP falló" (raro) → blindar consultando MP antes de cancelar, cuando haya volumen. typecheck+lint OK en todo.

🟢 **Cotización por CP en el carrito (CONSTRUIDO) + prueba real del tracking (pendiente) (2026-06-08)**. Founder pidió: (1) probar el tracking en un caso real; (2) en el carrito (estimador con selector de provincia), tener en cuenta la dirección registrada del user + agregar cálculo de costo con el CP (4 díg). **Audit**: el **CHECKOUT ya** usa las direcciones del user + cotiza con el CP real (`resolveShippingQuotes` por dirección, `app/(storefront)/checkout/page.tsx`) — la parte (a) ya existía ahí. Faltaba en el **CARRITO** (`ShippingCalculator` estimaba solo por provincia/tabla). **(2) ✅ CONSTRUIDO**: endpoint nuevo `app/api/shipping/quote/route.ts` (POST provincia+CP+subtotal+items → `resolveShippingQuotes` → domicilio+sucursal reales; público, valida inputs y acota rangos; **sin rate-limit en V1** → ver BACKLOG). `ShippingCalculator` reescrito: selector de provincia (estimación tabla instantánea) + input de **CP 4 díg** + botón "Calcular" → costo **real** de Correo (domicilio + sucursal). `cart-page.tsx` + `carrito/page.tsx` pre-cargan provincia+CP de la dirección default del user logueado (auto-cotiza al montar). typecheck+lint OK. **(1) ⬜ PENDIENTE — prueba real del tracking**: requiere un pedido real → botón "Generar envío" (genera envío REAL en Correo prod, idempotente) → copiar nº del panel MiCorreo → cargarlo + marcar "Enviado" → ver email. Guía dada; verifico en DB cuando el founder lo haga. Opción de prueba: compra de prueba (MP sandbox) + generar el envío (queda dado de alta en Correo; no despachar = no se cobra). **Reframe (2026-06-09, founder repreguntó por `/shipping/tracking`)**: el flujo se parte en dos — (a) **capturar el nº** = manual 1 vez (el alta solo devuelve `{createdAt}`; el nº vive en el panel); (b) **actualizar el estado** = automatizable con `/shipping/tracking` (CONSUME el nº, no lo genera: en el ejemplo del manual `shippingId` enviado == `trackingNumber` devuelto; la 3ª respuesta `"No existe el cliente o pedido"` es lo que dieron nuestros ids). **Decisión abierta (founder)**: ¿construimos el auto-update (CRON que refresca los eventos al cliente con el nº ya cargado)? Validación previa, chica: pegarle a `/shipping/tracking` un nº REAL del panel (nunca probado — solo con nuestros ids → "No existe").

🔵 **Fase 3 (alta de envío Correo) — founder eligió tracking automático; contrato del manual obtenido + HALLAZGO clave (2026-06-08)**. Founder pasó el manual (`~/Downloads/apiMiCorreo.pdf`, 18 págs; lo leí con venv temporal + pypdf). Consulté `argentine-ecom` (buen análisis: recomendó alta MANUAL desde panel, no auto al pago, por el armado post-pago + volumen bajo + cancelación incierta — coincido). **Contrato `/shipping/import` [POST]** (lo tengo completo): body = `customerId`, `extOrderId` ✔, `orderNumber`, `sender{name,phone,cellPhone,email,originAddress{...}}`, `recipient{name✔,phone,cellPhone,email✔}`, `shipping{deliveryType✔ D/S, productType✔ "CP", agency (obligatorio si S), address{...} (obligatorio si D), weight✔, declaredValue✔, height✔, length✔, width✔}`. Province codes: **Corrientes = "W"** (origen Virasoro). **⚠️ HALLAZGO CRÍTICO**: el import **NO devuelve el número de tracking** — su Response 200 es solo `{createdAt}`. El endpoint `/shipping/tracking` [GET] pide `shippingId`, y en los ejemplos el `shippingId` == el `trackingNumber` (circular) → **el número de seguimiento NO se obtiene de la API tras el alta; sale del panel MiCorreo** (por `orderNumber`/`extOrderId`) o de la etiqueta. O sea: se puede automatizar el ALTA (no cargar a mano en el panel) y la consulta de EVENTOS si ya tenés el número, pero la **captura automática del número tras el alta NO es directa** (a CONFIRMAR en test — la realidad puede devolver más que el manual). Errores del import: "La orden ya fue importada" (idempotencia por extOrderId), validaciones de peso/sucursal/provincia. **Plan Fase 3**: (1) ✅ `lib/correo/import.ts` CONSTRUIDO (`importShipment()` alta D/S + `fetchCorreoTracking()` experimental para probar la hipótesis del tracking; tipos en `types.ts`; sender desde env de la óptica; idempotencia "ya importada"; typecheck OK) — **pendiente validar en test, NO usar en prod** (genera envío real); (2) ⬜ botón "Generar envío" en `/admin/pedidos/[id]` → import → marca `shipped` + guarda referencia; (3) ⬜ resolver captura del tracking en test. **Founder eligió Fase 3 y le extrañó que no sea automático (con razón)**: hay una HIPÓTESIS plausible de que SÍ sea automático — que `/shipping/tracking` acepte nuestro `extOrderId`/`orderNumber` y devuelva el `trackingNumber`. El manual es ambiguo; se confirma SOLO probando en test (`fetchCorreoTracking` existe para eso).
**🧪 PRUEBA contra ambiente TEST (apitest) 2026-06-08 — 3 hallazgos**: (a) **las credenciales actuales (user/pass) FUNCIONAN en test** (`/token` HTTP 200 en apitest Y en api) → **NO hay que pedir credenciales de test** (contra lo que supuso argentine-ecom; el manual solo trae la URL de test, no credenciales). (b) **PERO el `customerId` de prod NO existe en test**: `/shipping/import` → HTTP 400 `"No existe el cliente"`. El `customerId` es por-ambiente → falta un customerId válido en test (opciones: registrar un usuario de prueba vía `/register` en test → devuelve customerId, o pedir uno a Correo). (c) **Bug detectado + corregido**: `fetchCorreoTracking` usaba `GET` con body (como el `curl` del manual), pero `fetch`/undici lo prohíbe → cambiado a query param `?shippingId=`. La hipótesis del tracking automático sigue SIN confirmar (el import falló antes por el customerId). **✅ PROBADO E2E EN TEST (apitest) 2026-06-08 — CONCLUSIÓN DEFINITIVA**: registré usuario de prueba con **DNI** (`/register` con CUIT falla: "no se pudo obtener condición impositiva"; con DNI = consumidor final → HTTP 200, customerId `00050053xx`). **`/shipping/import` HTTP 200 → `{createdAt}` y NADA MÁS** (confirmado contra server real: el alta NO devuelve número de tracking ni id interno). **`/shipping/tracking`**: el server EXIGE GET+body (query param → 415; con body vía `https` nativo de Node → 200). Probado con nuestro `extOrderId`, `orderNumber` y `customerId` → los 3 devuelven `{"error":"No existe el cliente o pedido"}`. **CONCLUSIÓN: el tracking NO es automático** — `/shipping/tracking` solo acepta el `trackingNumber` real de Correo (formato `000500...`), que la API NO entrega en el alta. **Re-verificado EXHAUSTIVAMENTE (2026-06-08, founder insistió, con razón de pedir certeza)**: (a) revisé los HEADERS del response del import → NO hay id/Location/tracking (solo metadatos JBoss/cookie); (b) reintenté `/shipping/tracking` con timing 0/5/10s × 4 ids (extOrderId, orderNumber, customerId, customerId+extOrderId) → los 12 intentos: `"No existe el cliente o pedido"`. 100% confirmado: el nº sale del **panel MiCorreo**. **`/shipping/tracking` SÍ sirve para OTRA cosa**: dado el nº real, devuelve los EVENTOS (PREIMPOSICION → en camino → entregado) → **mejora futura: mostrar el seguimiento en vivo al cliente dentro de la web** (ver BACKLOG; `fetchCorreoTracking` ya hace el GET+body nativo).
**⚠️ CAVEAT IMPORTANTE (2026-06-08, founder re-insistió 2×)**: TODA la prueba del tracking fue en **TEST (apitest)**. Es muy probable que el **sandbox NO alimente el sistema de seguimiento real** de Correo → de ahí el `"No existe el cliente o pedido"` constante, sin importar el id. **Eso hace la prueba del tracking INCONCLUSA para producción**: no se puede descartar que en PROD `/shipping/tracking` acepte nuestro `extOrderId`/`orderNumber` (la redacción del manual, "Identificador del envío del cliente", lo deja abierto). El alta devolviendo solo `{createdAt}` SÍ es firme (body+headers, ambos ambientes). **✅ PRUEBA EN PRODUCCIÓN HECHA (2026-06-08, con OK del founder)**: import real OK contra `api.correoargentino.com.ar` (customerId real del founder `0001912426`; envío de prueba **`extOrderId=OC-PRUEBA-572831371` / `orderNumber=OCPRB5728`**, datos ficticios, domicilio CABA, queda en **preimposición** en la cuenta → no despachar = no se cobra). `/shipping/tracking` en prod con extOrderId/orderNumber/customerId a 0/15/30s → los 9 intentos: `"No existe el cliente o pedido"`. **Caveat restante**: timing largo (un envío en preimposición puede tardar HORAS en entrar al seguimiento). **⬜ ÚLTIMO PASO (founder)**: mirar el **panel MiCorreo** → buscar el envío `OC-PRUEBA-572831371` → ver el **nº de seguimiento** que Correo le asignó → pasármelo → pruebo `/shipping/tracking` con ESE nº (confirma que el endpoint sirve para eventos) y veo si el nº se deriva o no de nuestro id (definir automático vs panel). + anular/ignorar ese envío de prueba en el panel. El número se obtiene del **panel MiCorreo** (el envío importado aparece por nuestro `orderNumber`/`extOrderId`) o de la etiqueta. **Plan final realista**: botón "Generar envío" en panel → `importShipment` (alta automática, no hay que cargar a mano en el panel) + **campo para pegar el nº de tracking** (copiado del panel) → cliente lo ve + email. O sea **alta automática + número semi-manual** (máximo que permite la API). **Nota técnica para implementar el tracking events**: si más adelante queremos mostrar el detalle de seguimiento al cliente, `/shipping/tracking` requiere GET con body → NO usar `fetch` (undici prohíbe body en GET, da 415), usar `https` nativo de Node. **✅ FASE 3 WIRING CONSTRUIDO (2026-06-08, founder eligió "Fase 3 completa")**: (1) `generateShipmentAction(orderId)` en `app/admin/pedidos/actions.ts` → lee la orden (`fetchOrderByIdAdmin`), mapea a `importShipment` (D si delivery, S si branch; peso por items; declaredValue = total; idempotente por `extOrderId`=order.id; error claro si pickup o faltan datos). (2) `updateOrderStatusAction` extendido con `trackingNumber?` → guarda `tracking_number` + lo usa en el email de "enviado". (3) `components/admin/shipment-control.tsx` (botón "Generar envío en Correo" + feedback). (4) `OrderStatusControl` con campo de tracking (input mono, se incluye en el email al marcar shipped). (5) `/admin/pedidos/[id]` muestra el botón (si no es pickup) + el campo. typecheck + lint OK. **Flujo del founder**: en el pedido → "Generar envío" (alta en Correo) → copia el nº del panel MiCorreo + imprime etiqueta → pega el nº en "Cambiar estado" + marca "Enviado" → email al cliente con tracking. **⚠️ El botón genera un envío REAL en Correo** (prod usa la URL de api, no apitest) — la 1ra vez, probarlo con un pedido real al despachar (o un pedido de prueba propio). Idempotente: re-apretar no duplica. **⬜ Sucursal (S) NO probada en test** (solo domicilio D); validar el alta a sucursal cuando haya un pedido branch. **⬜ Opcional futuro**: mostrar los EVENTOS de seguimiento al cliente (requiere `/shipping/tracking` con el nº real vía `https` nativo — `fetchCorreoTracking` ya existe). **⬜ BLOQUEANTE FOUNDER**: credenciales del **ambiente TEST de Correo** (`apitest.correoargentino.com.ar`, DISTINTAS a las de prod; pedirlas al contacto de API/panel "Mi Correo Empresas") — el import en PROD genera envíos reales (preimposición), hay que probar en test primero. **⬜ Esperando founder**: conseguir creds test + decidir si arranco a escribir el módulo ya (a ciegas, basado en el manual) o espero el test. Manual de referencia: copiarlo al repo (`docs/`?) o queda en Downloads.

🟢 **Envíos = SOLO Correo Argentino + audit del panel admin + estado del tracking (2026-06-08)**. Founder pidió 3 cosas: (1) envíos solo por Correo Argentino → cambiar "Andreani" en todos lados, incluido el mail de venta; (2) un dashboard admin de pedidos; (3) preguntó si el tracking aparece automático con la API de Correo.
**(1) HECHO + pusheado**: quité "Andreani" de UI pública (home `value-props`/`how-we-work`, `sobre-nosotros`, `terminos`, `privacidad`), FAQ (`lib/content/faqs.ts`), chatbot (`lib/chat/system-prompt.ts`), trust signals de PDP, comentarios técnicos, **email de venta** (`order-notification-admin`: "MiCorreo / Andreani" → "MiCorreo (Correo Argentino)"), seed SEO de Rusty (`09_brands_seo_text.sql` + UPDATE en DB), y docs (CLAUDE/ARCHITECTURE/BUSINESS_POLICIES/README/METRICS). **ADR-026 creado** (revierte ADR-017). `typecheck` OK. Memoria guardada (envios-solo-correo-argentino). El próximo mail de venta sale corregido.
**(2) Dashboard — YA EXISTE** `/admin/pedidos` (gateado `requireAdmin()`): listado (cliente/estado/total/fecha) + detalle (`[id]`: datos cliente PII, productos, totales, ID pago MP, envío domicilio/sucursal/pickup) + cambiar estado (`OrderStatusControl`) + nota por cambio (la ve el cliente en su tracker + va al email si el estado notifica). **Falta**: (a) cargar/editar el **tracking number** (hoy se MUESTRA si existe pero NO hay input → ver punto 3); (b) **comentarios internos** (no visibles al cliente); (c) buscador/filtros (por estado/cliente, útil con volumen). **⬜ Esperando founder**: qué construir (se le ofrecieron las 4 opciones por AskUserQuestion).
**(3) Tracking — NO automático hoy**. La API integrada hace `/rates` + `/agencies`, NO el alta de envío (`/import` = Fase 3, NO construida) que generaría el nº de tracking. El campo `tracking_number` existe + se muestra en panel/detalle + se manda en el email de "enviado" — pero **no hay forma de cargarlo** (ni manual ni auto). Opciones: **(A)** input manual en el panel (rápido, desbloquea ya); **(B)** Fase 3 automática (al despachar, alta en Correo por API → tracking solo; requiere credenciales del ambiente de import de Correo). Recomendado A primero, B con volumen. Ver BACKLOG.

🧹 **Limpieza de las 6 órdenes de prueba MP (2026-06-08)**. Tras validar el checkout e2e quedaron 6 órdenes sandbox: OC-2026-00001..00006 (4 `pending` + 2 `paid`). **Hallazgo de arquitectura**: el stock se descuenta al **CREAR** la orden (RPC `reserve_stock`, `lib/checkout/orders.ts`), NO al pagar → las **6** descontaron stock + push outbound a ML. Las `pending` abandonadas **no liberan stock solas** (no hay job de expiración de reservas — gap, ver BACKLOG). **Acción ejecutada vía MCP (founder OK por 2 preguntas)**: cancelé las 6 (`status='cancelled'`; las 2 paid conservan `payment_status='approved'` como registro) + reintegré stock en la DB: **Rusty Xold 13→16** (3 unidades: 00001/00002/00006), **Vulk Arvin 4→6** (2: 00003/00004), **Vulk Day Light queda en 3** (founder pidió NO reintegrar la unidad de 00005). Verificado: las 6 en `cancelled`, stock 16/6/3. **⚠️ PENDIENTE FOUNDER — editar ML (sino se revierte)**: ML es la fuente de verdad (cron `ml-reconcile-stock` cada 6h trae ML→DB). La DB ya está corregida pero **ML sigue con los valores descontados** → si el founder NO sube el stock en ML, el próximo reconcile (≤6h) pisa la DB de vuelta. **Founder debe poner en ML (valor absoluto)**: Rusty Xold (`MLA1388056603`)→**16**, Vulk Arvin (`MLA1391397105`)→**6** (Day Light `MLA2726903920` NO tocar, queda 3). Cuando lo haga → verifico con `/api/admin/ml-force-sync/<MLA>`. **No pude hacer el push a ML yo**: faltan `ML_CLIENT_ID`/`ML_CLIENT_SECRET`/`ML_REDIRECT_URI` en el `.env.local` local (solo están en Vercel) → `getMLConfig()` tira error. Ver LEARNINGS.

🟢 **Post-primera-venta: stock OK + 2 ajustes de email pendientes (2026-06-08)**. Founder confirmó tras OC-2026-00005: ✅ pago; ✅ **stock descontado en la página Y en Mercado Libre** (la sync de stock funciona); ✅ email de confirmación al **cliente** (pero **cayó en SPAM**); ❌ **NO llegó la notificación de venta al admin/founder**. **Causa notif admin**: `sendOrderNotificationToAdmin` (`lib/emails/send-order-emails.ts:93`) se saltea EN SILENCIO si `BUSINESS_ADMIN_EMAIL` está vacía (devuelve `{ok:true, id:'skipped'}`) — esa env var NO está cargada en Vercel. **Fix**: agregar `BUSINESS_ADMIN_EMAIL=<email del founder>` en Vercel + redeploy → en la próxima venta llega el aviso. **Importante**: NO se pierden ventas aunque falle el email — TODAS aparecen en el **panel `/admin/pedidos`** (`app/admin/pedidos/page.tsx`; la orden OC-2026-00005 ya está ahí en `paid`). El email es un aviso push; el panel es la fuente de verdad. (Nota: `BUSINESS_ADMIN_EMAIL` = notificaciones de venta; `ADMIN_EMAILS` = quién puede entrar al panel admin. Son distintas.) **Causa spam**: DNS del dominio (gestionado en **Vercel DNS**, ns1/ns2.vercel-dns.com) tiene **DKIM ✓** (`resend._domainkey`) + **SPF ✓** (return-path `send.opticacarballo.com.ar`→amazonses) pero **falta DMARC** (`_dmarc.opticacarballo.com.ar` vacío). Gmail/Outlook penalizan/spamean dominios sin DMARC desde feb-2024. **Fix**: agregar registro TXT `_dmarc.opticacarballo.com.ar` = `v=DMARC1; p=none; rua=mailto:hola@opticacarballo.com.ar` en Vercel → Domains → DNS + marcar el mail "no es spam" (entrena el filtro). Dominio nuevo = warm-up, mejora en días. **✅ Ambos fixes aplicados por el founder (2026-06-08)**: (1) `BUSINESS_ADMIN_EMAIL=juanmirande@yahoo.com.ar` cargado en Vercel → **el mail de venta YA llega** (a Yahoo); (2) **DMARC publicado y propagado** (`_dmarc` = `v=DMARC1; p=none; rua=mailto:hola@opticacarballo.com.ar`, verificado con dig). **Autenticación de email ahora COMPLETA: SPF ✓ + DKIM ✓ + DMARC ✓.** **Estado actual**: ambos mails (venta al founder + compra al cliente) **llegan pero todavía a SPAM**. Causa restante NO es autenticación (está perfecta) → es **reputación de dominio nuevo** (warm-up): un dominio que nunca mandó mails arranca con reputación neutra y los filtros (Gmail/Yahoo, este último estricto) lo mandan a spam los primeros días. Probable también que los mails ya recibidos se enviaran ANTES de propagar el DMARC. **Próximo (founder)**: (a) marcar "No es spam" + agregar `hola@opticacarballo.com.ar` a contactos en Gmail y Yahoo (lo más efectivo a corto plazo); (b) hacer una compra de prueba NUEVA (post-DMARC) y verificar en "Mostrar original" que diga SPF/DKIM/DMARC = PASS; (c) dar tiempo de warm-up (días). Sin fix instantáneo para reputación; la parte técnica ya está 100%. Ver MISTAKES (checklist de env vars al activar flujo en prod) + LEARNINGS (auth ≠ inbox).

🎉 **MP CHECKOUT PRO — PAGO E2E VALIDADO EN PRODUCCIÓN (2026-06-08)**. Tras corregir las credenciales (app `7985`, ver abajo) y que el founder las actualizara en Vercel + redeploy, **el pago de prueba APROBÓ**: orden **OC-2026-00005**, `mp_payment_id` `1327382092`, página de éxito OK. **Verificado vía MCP en la base** que NO es solo el redirect: `status='paid'`, `payment_status='approved'`, `paid_at` seteado, y el timeline (`order_status_events`) muestra `pending` (23:24:19, checkout creó la orden) → `paid` (23:25:16, **57s después**). Esto valida la cadena COMPLETA: checkout → orden creada (trigger AFTER OK) → MP procesa → **webhook recibe la notificación, valida la firma HMAC y marca `paid`** → trigger registra el evento. Envío de esa orden = `pickup` (retiro en local). **Toda la mecánica de pago + envío + tracking + emails funciona end-to-end.** ⚠️ **ESTADO ACTUAL = MODO PRUEBA EN PROD**: Vercel tiene credenciales `TEST-7985...` → ningún cliente real puede pagar todavía (sandbox). **⬜ Para ir a ventas reales** (decisión founder): (1) cambiar en Vercel `MP_ACCESS_TOKEN` + `MP_PUBLIC_KEY` a las de **producción** (`APP_USR-7985...` de la MISMA app) + `MP_WEBHOOK_SECRET` a la del **modo producción** (la sección Webhooks tiene una clave distinta para prod) + redeploy; (2) revisar el checklist de calidad de MP (se puede correr vía MCP `quality_checklist`); (3) decidir si el flag `NEXT_PUBLIC_CHECKOUT_ENABLED` queda ON (ventas vivas) u OFF hasta el switch. Mientras siga en TEST, conviene flag OFF o un aviso, para que un cliente real no se tope con un checkout que no cobra.

🟢 **MP — credenciales corregidas vía MCP: usar las de la APP, no las del test user (2026-06-08)**. Conecté el **MCP oficial de Mercado Pago** (`claude mcp add` + OAuth). `application_list` reveló que la app real es **OPTICA CARBALLO (`7985093275864504`, owner `81654493`)**, y `get_credentials` dio sus credenciales: prueba = `TEST-7985...` / prod = `APP_USR-7985...`. **Las credenciales que el founder había cargado eran de un TEST USER** (app `6981039109262255`, `APP_USR-6981...`) → ecosistema distinto, **incompatible con el webhook secret** (configurado en la app 7985). Corregido en `.env.local` a `TEST-7985...` (access token + public key de prueba de la app) → `pnpm mp:smoke` OK. Con `TEST-`, `isMpTestMode()`=true → el código usa `sandbox_init_point` (sandbox clásico, más permisivo para probar). Ver MISTAKES. **⬜ FALTA**: (1) founder actualiza en **Vercel** `MP_ACCESS_TOKEN` + `MP_PUBLIC_KEY` a las `TEST-7985...` (tenía las `6981`) + redeploy; `MP_WEBHOOK_SECRET` (modo prueba de la app 7985) y `MP_NOTIFICATION_URL` quedan igual; (2) reintentar el pago — con el ecosistema consistente debería aprobar con tarjeta APRO; si persiste "una parte es de prueba", loguearse en el sitio con un email de prueba. **Producción (futuro)**: `APP_USR-7985...` de la misma app + webhook secret de modo productivo.

🟢 **FIX bug bloqueante del checkout — trigger del tracker (2026-06-08)**. En la 1ra prueba e2e del checkout (founder, sitio en Vercel, flag prendido), "Confirmar pedido" falló con `violates foreign key constraint order_status_events_order_id_fkey`. **Causa raíz**: el trigger `on_order_status_change` (migración 20260601000000) era `BEFORE INSERT OR UPDATE OF status` e insertaba en `order_status_events` (FK→orders.id) con `NEW.id` durante el BEFORE INSERT, cuando la orden TODAVÍA NO existe → FK violation en cada creación de orden. Dormido hasta ahora porque los cambios de status manuales son UPDATE (la fila ya existe) y nunca se había creado una orden por checkout (estaba OFF). **Fix** (migración `20260608010000`, aplicada al cloud vía MCP con OK del founder + verificada): separar en 2 triggers — `set_order_status_timestamps_trg` (BEFORE, setea paid_at/shipped_at/delivered_at) + `record_order_status_event_trg` (AFTER, inserta el evento con la orden ya creada). El UPDATE manual de status sigue igual. Ver MISTAKES. **✅ Fix verificado end-to-end** (2026-06-08): el founder reintentó → la orden se CREÓ OK (OC-2026-00001 y -00002, status `pending`, **con evento de tracking 'pending'** — el trigger AFTER funcionó, sin FK error) y redirigió a Mercado Pago. **⬜ Pago en MP pendiente**: al pagar salió "Una de las partes con la que intentás hacer el pago es de prueba" = mezcla de cuenta de prueba (vendedor) con cuenta real (comprador). Es **mecánica de MP, no bug nuestro** (el checkout + creación de orden + tracking funcionan). Para completar el pago de prueba falta un **2do test user COMPRADOR** (el test user actual `3458884329` es el vendedor/dueño de las credenciales) → crear otra "Cuenta de prueba" en MP y loguearse con ESA al pagar (no con la cuenta real). Las órdenes de prueba quedan en `pending` (basura de test; se pueden borrar luego). **2do intento — causa identificada**: en "Revisá tu pago" de MP el payer figuraba con el **email real del founder (`jua...@hotmail`)**, que es su cuenta real de MP → "una de las partes es de prueba". Causa: el sitio manda `payer.email` = email del usuario logueado (preferences.ts:81), y el founder se logueó en el SITIO con su email real (cuenta MP real). **NO es bug** (en prod, cliente real + creds prod = OK). **Para testear**: el founder debe estar logueado en el SITIO con un email de PRUEBA — (a) un email propio NO registrado en MP, o (b) el email del 2do test user comprador (`test_user_*@testuser.com`, requiere desactivar confirmación de email en Supabase para poder registrarse con un email ficticio). Así el `payer.email` es de prueba y MP aprueba con tarjeta APRO.

🟢 **MP (Mercado Pago) — retomado: paso a paso de credenciales entregado al founder** (2026-06-08). El código de MP Checkout Pro YA está completo (`lib/mp/` client+preferences+webhook; `app/api/mp/webhook/route.ts` valida firma HMAC + marca `paid` + manda emails cliente/admin; `preferences.ts` con external_reference/back_urls/auto_return/notification_url). Falta SOLO conseguir + configurar 4 env vars: `MP_ACCESS_TOKEN`, `MP_PUBLIC_KEY`, `MP_WEBHOOK_SECRET`, `MP_NOTIFICATION_URL`. Invoqué `argentine-ecom` para el procedimiento actualizado (crear app en developers.mercadopago.com.ar → Checkout Pro → credenciales TEST → usuarios+tarjetas de prueba → webhook + signature secret → credenciales PROD → Vercel). **⚠️ El agente ALUCINÓ el estado del código tras un reboot de su shell** (reportó `lib/mercadopago/` y un webhook "stub con void payload" inexistentes); verifiqué con `grep`/`ls` que es FALSO (el webhook real procesa pagos completos). Ver AGENT_PERFORMANCE. **⬜ PENDIENTE FOUNDER**: seguir el paso a paso → conseguir credenciales TEST → pegarlas en `.env.local` + Vercel. Usar la MISMA cuenta MP de ML. Orden: TEST primero (probar con tarjetas APRO/OTHE/CONT), PROD al final. **Activar el checkout** = credenciales MP + `NEXT_PUBLIC_CHECKOUT_ENABLED=true` (la migración de envíos ya está aplicada al cloud ✅). 🔧 **Founder configurando la app en el panel (2026-06-08)**: llegó a la pantalla "Configuración y scopes" = panel AVANZADO de Mercado Libre/MP (OAuth, Negocios ML, Permisos/scopes de publicaciones/órdenes, Tópicos). **Guía dada**: (1) el webhook `/api/mp/webhook` va en **"Notificaciones callbacks URL"**, NO en "Redirect URIs" (founder lo había puesto mal — ver MISTAKES); (2) "Redirect URIs" (obligatorio) = `https://opticacarballo.com.ar`; (3) Flujos OAuth / PKCE / Negocios / Permisos / Tópicos = NADA (son de la API de ML → corresponden a la futura integración de sync de stock ADR-024, no a Checkout Pro). **Próximo paso founder**: crear la app → "Credenciales de prueba" (Access Token + Public Key `TEST-...`) + "Webhooks" (clave secreta) → pasármelas o pegarlas en `.env.local`. Si no aparecen esas secciones, evaluar crear la app desde `mercadopago.com.ar/developers` (camino Checkout Pro directo). **Aclaración clave (`preferences.ts:90`)**: el código YA manda `notification_url` en cada preference → **NO hace falta configurar la "Notificaciones callbacks URL" global ni marcar tópicos** en el panel (esos tópicos —Orders/Items/Shipments— son de la API de ML). El panel "le pide marcar un tópico para guardar la callback URL"; como no necesitamos esa URL global, se deja vacía y sin tópicos. Del panel solo se necesitan: (a) credenciales (Access Token/Public Key) y (b) la **clave secreta de webhooks** (sección "Webhooks" de Mercado Pago, tópico "Pagos"). **Corrección 2026-06-08**: el panel donde estaba el founder es el de **Mercado Libre developers** y OBLIGA a marcar ≥1 unidad de negocio (ML/VIS) + ≥1 permiso por scope (errores de validación confirmados en captura). Ese camino es para la **API de ML (marketplace)**, NO para Checkout Pro. **Decisión: crear la app de pagos desde `mercadopago.com.ar/developers` → "Crear aplicación" → producto "Checkout Pro"** (flujo directo: NO pide unidades de negocio ni scopes; entrega credenciales + sección "Webhooks" con clave secreta y tópico "Pagos"). La app de ML que quedó a medias NO sirve para pagos (sí, eventualmente, para el sync de stock ADR-024). ✅ **Credenciales de prueba MP cargadas + validadas (2026-06-08)**: founder creó la app (Checkout Pro), pasó Public Key + Access Token (esquema nuevo `APP_USR-`, de "Credenciales de prueba") + el test user (`TESTUSER...`). Guardadas en `.env.local` (MP_ACCESS_TOKEN + MP_PUBLIC_KEY; el test user/contraseña NO se persiste, es para login manual al probar). Nuevo `scripts/mp-smoke.ts` (`pnpm mp:smoke`) crea una preference de prueba → **validado OK** (preference creada, id arranca con el User ID del test user `3458884329`, no cobra real). **Bug cazado**: el `.env.local` tenía un `MP_ACCESS_TOKEN=TEST-...` viejo (otra app/proyecto); mi comando de guardado no lo reemplazó (append-if-missing en vez de upsert) → el smoke imprimió prefijo "TEST-" y lo detecté → reemplazado por el `APP_USR-` correcto. Ver MISTAKES. **Nota técnica**: `isMpTestMode()` (`lib/mp/client.ts`) detecta prefijo `TEST-`; con `APP_USR-` (esquema nuevo) devuelve false → el checkout usa `init_point` (correcto; `sandbox_init_point` es legacy). No bloquea probar, pero conviene revisar la función. **✅ `MP_WEBHOOK_SECRET` agregado** (2026-06-08): sección Webhooks → Modo de prueba → evento "Pagos" tildado + clave secreta de 64 chars; en `.env.local` (verificado fuera de git). **Las 4 credenciales MP de prueba están COMPLETAS en `.env.local`** (access token + public key + webhook secret + notification url). **Setup de credenciales MP = COMPLETO.** **⬜ FALTA — probar el flujo e2e** (carrito → checkout → pagar con tarjeta de prueba APRO → orden pasa a `paid` + emails). Bloqueo de entorno: el `.env.local` NO tiene Supabase → la web no corre full local. Opciones: (a) completar `.env.local` con las 3 vars de Supabase y probar en `localhost` (controlado, no toca el sitio público); (b) probar en Vercel cargando las 4 MP + prendiendo el flag en un entorno de PRUEBA (no en el dominio público con credenciales de prueba). Pendiente decidir con el founder. Extra opcional: "Simular notificación" del panel MP para validar la firma del webhook (requiere el secret en el entorno deployado). **2026-06-08 (cont.)**: founder cargó las 4 MP en Vercel + redeploy; **sitio TRANQUILO** (sin tráfico) → se puede probar en Vercel prod prendiendo el flag un rato. Verificado pre-prueba: las 3 páginas de retorno (`/checkout/exito|pendiente|error`) existen; hay productos con stock (Rusty Xold 41u, Rusty Spell 38u, Vulk Booping 33u…). **Plan de prueba e2e**: founder agrega `NEXT_PUBLIC_CHECKOUT_ENABLED=true` en Vercel + redeploy → navega a un producto (ej. `/anteojos-de-sol/rusty/rusty-xold`) → carrito → login → checkout → paga con tarjeta de prueba APRO → asistente verifica vía MCP que la orden pase a `paid` (valida webhook + firma) → apagar el flag. **⬜ Falta**: founder prende el flag en Vercel + hace el pago de prueba. (Gap menor corregido: `NEXT_PUBLIC_CHECKOUT_ENABLED` ahora documentado en `.env.example`.)

🟢 **API MiCorreo (Correo Argentino) — Fase 1: cotización a domicilio CONSTRUIDA + validada contra producción** (2026-06-08). Founder retomó "activar la API del correo para los envíos" y confirmó tener credenciales. **Construido**: `lib/correo/` (`constants.ts` env+config+normalizePostalCode, `types.ts`, `auth.ts` POST /token Basic→JWT bearer cacheado en memoria, `quote.ts` POST /rates con retry-on-401) + wrapper server-only `lib/shipping-server.ts` `resolveDeliveryQuote()` con **fallback automático** a tabla por zonas si la API no está configurada o falla. Integrado en `app/(storefront)/checkout/page.tsx` (cotización inicial con CP del default address) + `lib/checkout/actions.ts` (cotización FINAL persistida). `lib/shipping.ts` (tabla, pura, sync) queda **INTACTO** para los estimadores client-side de PDP + carrito (la API es server-only, secreta). `.env.example` con sección MICORREO_* + `pnpm correo:smoke [cp] [items]` (`scripts/correo-smoke.ts`, self-contained, no importa nuestro código). Decisiones técnicas: vencimiento del token leído del claim `exp` del JWT (el campo `expires` del response viene VACÍO); CP destino normalizado a 4 dígitos (acepta CPA `B1900ABC`→`1900`); peso default 500g +250g/item (`DEFAULT_PACKAGE`, ajustable); para domicilio se elige el servicio **Clásico (más barato)** entre los rates `D`. **Validado contra PROD**: login OK + `/rates` devuelve tarifas reales. `typecheck`+`lint` limpios. **Seguridad**: founder pegó las credenciales en `.env.example` (archivo trackeado) → las moví a `.env.local` (gitignored), restauré placeholders, verifiqué que NO llegaron a git (HEAD limpio). Ver MISTAKES. **HALLAZGO DE NEGOCIO**: la tabla hardcoded subcotizaba fuerte. Clásico domicilio real ~$8-10k nacional vs tabla: CABA $2.500→$9.991; Rosario $4.500→$9.177; Córdoba $4.500→$9.991; Ushuaia $9.500→$10.051; Virasoro $6.500→$7.947. En CABA se perdían ~$7.500/envío. **⬜ DECISIONES PENDIENTES FOUNDER**: (1) free shipping desde $80k absorbe ~$10k de envío real — ¿mantener/subir umbral/cobrar siempre? (2) ¿solo Clásico o también Expreso (1-3 días, ~2× precio) como upgrade? (3) ¿avanzo Fase 2 (toggle domicilio/sucursal + selector sucursal `/agencies` + migración 00007) y Fase 3 (alta automática `/shipping/import` post-pago)? (4) confirmar peso real de una caja (usé 500g estimado). (5) cargar las 5 env vars MICORREO_* en Vercel (prod). (6) opcional higiene: rotar password del portal. **NO commiteado.** Ambiente=PROD (`/rates` es read-only, seguro; para Fase 3 `/import` conviene credenciales TEST primero). El checkout sigue detrás del flag `NEXT_PUBLIC_CHECKOUT_ENABLED` (OFF) + MP no activo → esto NO está en vivo aún, pero el código y la cotización ya funcionan. 🔵 **Decisiones founder (mismo turno)**: (a) free shipping se MANTIENE en $80.000 (no se toca la tabla); (b) avanzar Fase 2 (sucursal). **Fase 2 BACKEND construido + validado contra PROD**: `lib/correo/provinces.ts` (nombre→código 1-letra, tabla oficial), `lib/correo/agencies.ts` (`listBranches(provincia)` = GET /agencies + cache 24h en memoria, filtra status ACTIVE + packageReception), `lib/shipping-server.ts` ahora expone `resolveShippingQuotes()` que cotiza domicilio + sucursal en 1 sola request a /rates (sucursal ~$3k más barata que domicilio). Validado: /agencies devolvió 84 sucursales en Corrientes con el formato esperado. typecheck+lint OK. **Fase 2 COMPLETA (founder OK: "con selector de sucursal")**: migración `20260608000000_orders_shipping_branch.sql` (agrega a `orders`: `shipping_delivery_type` CHECK D/S/null + `shipping_agency_code` + `shipping_agency_name` snapshot ADR-007; idempotente) + tipos en `types/supabase.ts` (3 cols × 3 bloques, editados a mano para que tsc pase antes de regenerar). `ShippingMethod` extendido a `'delivery'|'branch'|'pickup'`. UI: `shipping-method-selector.tsx` con 3 opciones (la de sucursal solo aparece si la API cotizó tipo S) + `<select>` de sucursales con snapshot del nombre en hidden input; `address-selector.tsx` con callback `onSelect`; `checkout-page.tsx` lee un map pre-cotizado por dirección (el server cotiza D+S+sucursales de CADA dirección del user, cap 6, en paralelo — **approach simple sin API route ni client fetch**; el client solo lee el map y resetea la sucursal al cambiar de provincia). `actions.ts`: discriminated union con `branch` (address_id + agency_code), cotiza branch server-side, error claro si la API no devuelve S. `orders.ts` persiste method + delivery_type + agency. Validado: **tsc OK, lint OK, `pnpm build` COMPILA + type-check OK** (el prerender corta solo por falta de `NEXT_PUBLIC_SUPABASE_URL` en el `.env.local` local — preexistente: nunca hubo `.env.local` con Supabase en esta máquina, el founder buildea en Vercel; NO es la Fase 2). Sucursal ~$2.8-3k más barata que domicilio (Corrientes: suc $5.169 vs dom $7.947). **✅ Migración APLICADA al cloud vía MCP** (verificada con `execute_sql`: las 3 columnas existen, data_type text) + **commit `a624dd5`** ("feat(envios): integración API MiCorreo"). Quedaron fuera del commit `DECISIONS.md` y `marketing/` (son de turnos previos, sin commitear). Founder cargó las 5 `MICORREO_*` en Vercel + confirmó que el archivo local es `.env.local` (no `.env`). **⬜ PENDIENTE**: (1) para activar de verdad: prender `NEXT_PUBLIC_CHECKOUT_ENABLED` + MP (cuando el founder active ventas online); (2) si quiere correr la web local, `.env.local` necesita también Supabase + resto (ver `.env.example`); (3) **Fase 3 (alta automática `/shipping/import` post-pago) NO hecha** — depende de MP activo + conviene credenciales de ambiente TEST de Correo (las de prod NO sirven en `apitest`, son distintas). **Tests**: cotización validada contra prod (login + /rates + /agencies, read-only = seguro, no genera envíos ni cobros); founder pidió testear antes de activar — nada está activado aún (checkout OFF, la migración solo agregó columnas vacías). 🧪 **Test cotización 24 CPs del founder (2026-06-08)**: 20/22 únicos cotizan OK (domicilio $7.947 NEA cercano → $10.051 Patagonia/Cuyo; sucursal SIEMPRE ~$2.800-3.100 más barata). **8100 y 5100 devuelven `rates:[]` (HTTP 202, sin error)** = Correo NO los reconoce como CP con cobertura (los reales cercanos 8000 Bahía Blanca y 5101 Córdoba SÍ cotizan; parecen CPs aproximados/no-localidad). Comportamiento ante CP no cotizable: `resolveShippingQuotes` → domicilio cae a tabla por zonas (fallback) + branch=null → el checkout NO se rompe, el cliente compra igual (pero a precio de tabla, más barato que el real). **Nota técnica**: /rates responde HTTP **202** (no 200); `res.ok` lo cubre OK. **⬜ Esperando founder**: ¿8100/5100 son CPs reales de clientes? → definir si el caso "CP sin cobertura" amerita bloquear el checkout (evitar subcotizar) en vez del fallback actual. Confirmado vía test (8100 probado PRIMERO + con prefijo `G8100`/`P5100`): no es posición ni bot ni formato — esos 2 CPs no existen en el nomenclador del Correo (cabeceras 8000/5101 sí cotizan). **`scripts/correo-smoke.ts` ahora tiene modo `pnpm correo:smoke zonas`**: lista curada de 22 cabeceras de cobertura garantizada (con reemplazos 8100→8000, 5100→5000 sugeridos por el founder) → verificación completa **22/22 OK**. ✅ **Fase 2 CERRADA — display de envío en pedidos** (2026-06-08): el detalle de pedido (cliente `components/account/order-detail.tsx` + admin `app/admin/pedidos/[id]/page.tsx`) ahora muestra el 3er método "sucursal" con nombre/código de sucursal; tipo `OrderDetail` + queries (cliente + admin) + select del webhook MP traen `shipping_delivery_type` / `shipping_agency_code` / `shipping_agency_name`. **Cacé un bug latente**: los emails de confirmación (cliente + admin) manejaban solo delivery/pickup → un envío a sucursal habría mostrado la dirección del domicilio; agregué el caso `branch` (+ `branchName`) a ambos templates + al armado en el webhook (`isDelivery`/`isBranch`). tsc + lint OK. **Próximo paso**: Fase 3 (alta automática `/shipping/import` post-pago) cuando haya MP activo + credenciales de ambiente TEST de Correo; o activar el checkout.
...
🟢 **Producto Vulk Lady Piny (sol, redondo femenino, cat 3) — APLICADO** (2026-06-04). Founder subió las fotos → verifiqué CDN (9×200, casing matcheó) → apliqué seed `56_vulk_lady_piny.sql` vía MCP + verificado runtime (PDP 200, 7 `<strong>`, "categoría 3" presente, "categoría 4" = 0, en grid mujer + /polarizados). 4 variantes simples: 285-UFC0096/GB10S 1194183 $90.584,21 stock 6 (primary, rosa); 292-UFQ0028/GS9B 1194182 $90.584,21 stock 6 (rosa pálido); SBLK-UFQ0003/SG16 1194180 $90.584,21 stock 3 (negro verde degradé); SBLK-UQ0178/BG26 1194181 $94.400 stock 0 paused (**POL**, marrón degradé). 1 de 4 pol → `lens_treatment ["uv400"]`. gender=female, 30,9g, **cat 3** (founder corrigió de cat 4). Frente G-Flex + patillas acetato + **bisagras metálicas sistema flex**. Medidas 141/57×54/17/135. Scale 1.15/1.0. Verificación MCP: `variantes=4, pol=1, stock=15, imgs=9, vars_sin_foto=0`. **⬜ Pendiente founder**: chequeo visual del scale + ¿frame_shape "redondo" OK o preferís "ovalado"? (ML mezclaba redondo/ovalado/"en puntas").

🟢 **Producto Vulk Clems RECETA (armazón ovalado ultra liviano) — APLICADO** (2026-06-05). Founder subió las fotos → verifiqué CDN (7×200, medidas.jpg) → apliqué seed `57_vulk_clems_receta.sql` vía MCP + verificado runtime (PDP 200, 5 `<strong>`, "progresivo", en grid receta). **Categoría receta**. 3 variantes en 1 MLA multi-variante (MLA1436023889), todas $83.410,02: CRY-GUN OPTICS 123036 stock 5 (primary, transparente, var 180952431090); MBLK-BLUE OPTICS 123030 stock 4 (negro mate, patillas azul, var 180952431086); SBLK-GUN OPTICS 123031 stock 2 (negro brillo, var 180952431088). Convención receta (`lens_compatibility` mono/bi/progresivo + `hinge_system "integrada"`). Ovalado, **14,3g** (de los más livianos), talle medium, frente G-Flex + patillas acero inoxidable con terminal de goma. Medidas 137/49×46/20/145. Verificación MCP: `variantes=3, stock=11, imgs=7, vars_sin_foto=0, con_varcode=3`. **Ajustes founder 2026-06-05**: (1) MBLK = variante predeterminada (sort_order 1 + is_primary; antes era CRY) → la card del grid muestra MBLK. (2) Scale: MBLK 1.15/1.0; CRY y SBLK se veían más chicas → subidas a perfil **1.25** / frente **1.10** para emparejar.

🟢 **Producto Rusty Esvep (sol, envolvente deportivo, polarizado) — APLICADO COMPLETO (3 var)** (2026-06-05). Seed `58_rusty_esvep.sql` aplicado vía MCP + verificado runtime (PDP 200, 3 variantes, en /deportivos + /polarizados). MBLK/S10 POL 112884 $76.194 stock 7 (primary); SBLK/S10 POL 112881 $76.194 stock 5; SBLK/S10 no-pol 112880 $66.724 stock 0 paused (founder pasó el SKU 112880 tras el 112881 duplicado). 2 de 3 pol → /polarizados. frame_shape envolvente → /deportivos. G-Flex, bisagras plásticas reforzadas, cat 3, UV400, medidas 140/60×46/10/130. Verificación MCP: `variantes=3, pol=2, imgs=7, vars_sin_foto=0`. **Scale (3 iteraciones)**: founder "considerablemente más chico" → 1.6/1.3 (como Sotion/Eslav) → "sigue chica" → 2.0/1.6 (iter 2) → "quedó muy grande" → 1.8/1.45 (iter 3) → "un poco más chico" → 1.7/1.4 (iter 4, commit 7bb7895) → 1.6/1.35 (iter 5) → "-0.1 más" → **perfil 1.5 / frente 1.25** (iter 6, valor actual, commit 896eaef, ajustable). Diagnóstico: verifiqué en el HTML del grid que el `scale()` SÍ se aplicaba (no era bug del override) → el problema es que la foto es muy ancha (1000×491, 2:1) y el anteojo ocupa menos del cuadro que en Sotion/Eslav, por eso necesita MÁS scale que el default del tipo "envolvente". + confirmar perfil/frente de la SBLK no-pol (asigné a ojo: `--` perfil, `---` frente — no pude abrir las fotos por límite de imágenes del harness; misma SBLK, solo el orden).

🟢 **FIX IMPORTANTE: el scale override (regla 15) ahora SÍ aplica en la galería de la PDP** (2026-06-05). El founder reportó que subir el scale del Esvep (1.15→1.6→2.0) "no surtía efecto, se veía igual". Diagnóstico: `getImageScale()` se aplicaba SOLO en las cards del grid (vía `to-product-card-data`), NUNCA en `components/product/product-gallery.tsx` (imagen principal + thumbs de la PDP renderizaban a tamaño natural). Fix: la galería ahora aplica `getImageScale(storage_path)` en la imagen principal (wrapper, para coexistir con el hover) y en cada thumb. Verificado `scale(2)` del Esvep en 6 superficies: todos-sol, deportivos, polarizados, rusty, hombre, **PDP**. El lightbox queda natural a propósito (zoom de inspección). Ver MISTAKES (mismo patrón que el bug de precio OOS: regla transversal que se olvidó una superficie). Commit pendiente.
- **Esvep scale iter 6**: perfil 1.5 / frente 1.25 (1.6→2.0→1.8→1.7→1.6→"-0.1 más"→1.5. Commit 896eaef, valor actual ajustable. Foto 1000×491 2:1, anteojo chico en el frame).
- **Dileri scale iter 3**: perfil 1.15 / frente 1.0 (iter 2 era 1.25/1.1; founder "-0.1 en todos los grids" 2026-06-05, commit 896eaef).

🟡 **PRUEBA: actualización masiva de precios Vulk Sol vía lista de precios del proveedor — análisis hecho, SIN aplicar (2026-08-03)**. Founder pasó `LISTA 1 DE PRECIOS VULK SOL 20-07-2026.xlsx` (Marca/Código/Artículo/Precio Sugerido Final) pidiendo "una prueba a ver cómo sale" antes de decidir automatizarlo. Proceso: leí el Excel (1506 filas con código, sección SOL only — RECETA/ANTIPARRA vienen vacías en este archivo, ACCESORIOS con estuches al final) + consulté Supabase (`product_variants.sku` es el campo que matchea el "Código" de la lista) para las 98 variantes de marca `vulk` activas. **Cruce por SKU exacto**: 64/98 matchean contra esta lista; las 34 restantes son TODAS de categoría `anteojos-de-receta` (Be Again, Clems, Dieven Unisex, Katleen Receta, Kirt, My Crew, Stray, Strewn, The Trial Optics, Tour 81, Vartis) — comportamiento esperado, no un gap, porque esta lista es solo la línea SOL. Verifiqué a mano que cada match de SKU corresponde al mismo producto/color (similitud de texto baja en varios casos por diferencia de formato DB vs Excel, pero el código de color coincide en el 100% de los casos revisados, incluido el caso más dudoso "Vulk Nova" Dorado C1/C2/C3 ↔ "NOVA C1/C2/C3"). **Resultado de precios**: la gran mayoría (14 de 18 modelos) sube ~11-12% parejo, Vulk The Sil sube 18.7%; **excepción: Vulk Biller** — 4 de sus 6 variantes (SKU 125181/125187/125188/125189) BAJAN de precio (-4.9% a -21.6%) mientras el resto del catálogo sube, no identifiqué si es real (liquidación del modelo) o error de la lista del proveedor. También el código `125180` aparece DUPLICADO en el Excel con 2 nombres de artículo distintos pero mismo precio (no genera conflicto, pero es un dato sucio de la lista, no nuestro). **No ejecuté ningún UPDATE** — es cambio de precio real de e-commerce en producción, y el founder lo pidió explícitamente como prueba antes de decidir. Reporté el detalle completo (tabla por modelo, las 2 anomalías) y quedé esperando 2 decisiones del founder: (1) si aplica el precio sugerido de Biller tal cual o lo excluye/chequea antes; (2) confirmación para ejecutar el UPDATE de las 64 variantes. **Próximo paso EXACTO**: cuando el founder responda, ejecutar el UPDATE de `price_cents = precio_sugerido*100` sobre las 64 (o 60, si excluye Biller) variantes por `sku`, vía MCP de Supabase, y verificar en PDP/grid que el precio nuevo se refleje. Si esto sale bien, el founder evalúa si conviene automatizar este cruce para las próximas listas (Vulk Receta, Rusty, etc.) en vez de repetirlo a mano.

🟢 **Scrape de las 14 páginas de la tienda oficial Vulk en Mercado Libre — título + precio de ~629 filas (2026-08-03)**. Founder pidió extraer título y precio de todos los modelos de `listado.mercadolibre.com.ar/tienda/vulk/vulk` sin entrar a cada ficha. Bright Data MCP figuraba como "requiere autorización" y el founder dijo que ya estaba autorizado, pero `ToolSearch` no encontró ningún tool registrado con ese nombre — no llegó a estar disponible en la sesión. El Browser pane sandboxeado (sin sesión logueada) pegó contra el muro de login de ML (`account-verification`, confirmado también con `WebFetch` — no es un problema del tool, es bot-detection de ML sin cookies reales). Se resolvió usando `claude-in-chrome` (el Chrome REAL del founder, ya logueado) — ahí la tienda cargó sin fricción. Paginación: la URL soporta offset directo `tienda/vulk/vulk_Desde_<N>_NoIndex_True` (N = 49, 97, 145... +48 cada página; confirmado con el footer de paginación llegando a "1 7 8 9 10 11 12 13 14" = 14 páginas totales, coincide con lo que dijo el founder). Se navegó por URL en vez de clickear "Siguiente" (más rápido, sin depender de refs de DOM que cambian por página). Entregado `vulk_ml_prices.md` (scratchpad, mandado directo al founder por `SendUserFile`) con las 629 filas — no se guardó en el repo. **Nota de calidad del dato**: hay bastante duplicación real (mismo modelo con distintas variantes de color listadas por separado, marcado "(2)"/"(3)" en el archivo) — no es error de scrape, es cómo ML lista variantes de storefront. **Próximo paso EXACTO**: ninguno de este sub-paso — sigue en la misma sesión, founder pidió cruzar estos precios contra `LISTA 1 DE PRECIOS VULK SOL 20-07-2026.xlsx` (precio sugerido del proveedor) para detectar modelos que ML esté vendiendo por debajo del sugerido (ver entry siguiente).

🟢 **Cruce de los 629 precios scrapeados de ML (entry anterior) contra el precio sugerido del proveedor — 19 modelos vendiéndose por debajo (2026-08-03)**. Founder pidió señalar cualquier modelo de la tienda oficial Vulk en ML cuyo precio actual esté por debajo del "Precio Sugerido Final" de `LISTA 1 DE PRECIOS VULK SOL 20-07-2026.xlsx`, sin importar cuán chica sea la diferencia. **Matching sin SKU** (el listado scrapeado no trae SKU, solo título — a diferencia del cruce de la entry de más arriba que sí matcheaba por `product_variants.sku`): se armó en Python (`openpyxl`) una extracción de "frase de modelo" (nombre distintivo, ej. "the trial", "le groupie", "booping") descartando ruido (colores, códigos de lente tipo S10/C1/DRT02, "polarizado", "original", etc.) tanto del título de ML como del "Artículo" del Excel, y agrupando el Excel por modelo (varios colores = mismo modelo, precios ligeramente distintos por color). **Criterio "bulletproof" usado**: solo se reportan casos donde el precio de ML está por debajo del **MÍNIMO** de TODAS las variantes de color conocidas de ese modelo en la lista del proveedor — así no importa qué color exacto sea la publicación de ML, ya es más barata que la más barata que tiene el proveedor. Con ese criterio conservador: **19 modelos** (28 publicaciones de ML) por debajo, de -0.5% a -31.5%. Los 2 más grandes (Buzz -31.5%, Boston -13.8%) se marcaron para que el founder los revise a mano — mismo patrón que el hallazgo de Vulk Biller de la entry anterior (podría ser liquidación real o un salto real de precio, no error de este cruce). Detalle completo entregado al founder en el chat (tabla por modelo + listado de publicaciones ML afectadas). **No se cruzó con el catálogo propio de Óptica Carballo ni se tocó ningún precio** — es información de mercado (lo que el proveedor sugiere vs. lo que su propia tienda oficial cobra), no una acción sobre nuestra web. **Próximo paso**: ninguno urgente, es información para el founder — puede servir como referencia de piso de precio competitivo o para negociar con el proveedor.

🟢 **NUEVO: video de producto con HyperFrames** (2026-06-06). Founder pidió "hacer algo con hyperframe" (`npx hyperframes render`, tool open-source de HeyGen: HTML→MP4 determinístico). Armado template **1:1 1080×1080** en `marketing/product-video/` (carpeta aislada, NO toca el app de Next): intro de marca → perfil → frente (swap) → ficha técnica → precio/CTA. Data-driven vía `data.js` (cambiás JSON + 2 fotos = otro modelo). Demo renderizado con **Rusty Esvep** (datos reales del seed 58, fotos del bucket). Identidad: monocromo + ámbar `hsl(36 45% 52%)`, **Inter + Playfair Display**. Decisiones clave: (1) fuentes literales mapeadas por HyperFrames (NO `var()`, NO Google Fonts) para render determinístico sin red; (2) FFmpeg/FFprobe vía `ffmpeg-static`/`ffprobe-static` + symlinks `bin/` (no instala nada en el sistema; postinstall automático); (3) saqué "3 cuotas sin interés" del frame de precio porque NO está en BUSINESS_POLICIES (regla dura: no prometer lo no confirmado). Output: `renders/*.mp4` (~810 KB, 11s, gitignoreado). Comandos: `npm install` → `npm run check` → `npm run render`. Ver `marketing/product-video/README.md` + DECISIONS ADR. **⬜ Pendiente founder**: (a) ¿OK el video? ¿commiteo `marketing/`? (b) ¿querés versión vertical 9:16 para reels? (c) ¿lo usamos como base para más modelos?
...
- ✅ **Rusty Dileri** (sol, cuadrado femenino, 2 var) — APLICADO + verificado (founder subió fotos al final de la sesión).
...
🟢 **Producto Rusty Dileri (sol, cuadrado femenino liviano) — APLICADO** (2026-06-04). Founder subió las fotos → verifiqué CDN (5×200; el casing `POl` minúscula del seed matcheó) → apliqué seed `55_rusty_dileri.sql` vía MCP + verificado runtime (PDP 200, en grid sol + mujer + /polarizados). 2 variantes simples: SBLK/S10 POL 127042 $78.869,25 stock 3 (primary, **POL**); SIENNA/G.GREEN 127043 $73.661,17 stock 3 (NO pol). 1 de 2 pol → `lens_treatment ["uv400"]`. gender=female, 31,8g, cat 3, medidas 140/52×53/15/135, frente G-Flex + patillas Flex Temple. Scale: perfil **1.25** / frente **1.10** (founder 2026-06-04, 2 iteraciones de +0.05 desde 1.15/1.0). Verificación MCP: `variantes=2, pol=1, stock=6, imgs=5, vars_sin_foto=0`. **⬜ Pendiente founder**: ¿quedó bien el nuevo scale? + ¿primary SBLK POL o preferís SIENNA (más colorida)?
