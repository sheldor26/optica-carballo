...
Plan B aplicado: endpoint público `/users/{id}` de ML devolvió 403 PA_UNAUTHORIZED. Creado endpoint admin `/api/admin/ml-me` v1 (commit `7474f76`) → también devolvió `not_found` porque `mlFetch` mapea 404 sin más contexto. Refactor v2 (commit `119f30f`): bypass `mlFetch`, fetch raw a `/users/me` + `integration_summary` (externalUserId guardado, status, tokenExpiresAt, lastErrorMessage, token_preview de 8 chars). Próximo paso exacto: founder abre `https://opticacarballo.com.ar/api/admin/ml-me` tras deploy del commit `119f30f` y pasa el JSON completo con `response.status` + `response.body` + `integration_summary` para diagnosticar si el 404 es por: token revocado, cuenta suspendida, path equivocado, o discrepancia entre user_id guardado vs cuenta real autorizada.
