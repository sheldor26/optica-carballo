...
- **Infra P0 a construir (requiere OK founder, regla 3)**: (1) `mdx-components.tsx` (componentes para embeber productos/CTAs/FAQ en MDX, vía pipeline scale regla 15), (2) cablear FAQPage en la page, (3) extender `ArticleJsonLd`→`MedicalWebPage`+`lastReviewed`+`about`. P1: sitemap (backlog), campos frontmatter nuevos, componente TOC. **⬜ Esperando founder: ¿arranco la infra P0?**
...
- **⬜ FALTA antes de escribir**: (b) **founder ratifica** los 12 puntos del brief técnico; (c) **regente firma** los 7 clínicos (🩺) + confirma umbral "miopía alta" (−6.00 D ref) e inicio presbicia (40-45). Con (b)+(c) → `content-writer-medical` escribe la pillar Astigmatismo (en `draft: true`).
