...
🟢 **TRUST SIGNALS PDP COMPACTADOS (2026-06-14)**. Founder pidió comprimir el alto del bloque de 6 garantías (`components/product/product-trust-signals.tsx`). Propuso carrusel auto-avanza de 3 visibles; **conversion-optimizer (prioridad) + ui-motion-designer desaconsejaron** el carrusel: ocultar/rotar trust signals daña CRO (funcionan por presencia simultánea) y 2 son links legales de Defensa del Consumidor (devolución + arrepentimiento) → no se esconden. El problema real era alto vertical. **Founder eligió "Compactar"** (AskUserQuestion): las 6 quedan visibles en layout horizontal de 1 línea (ícono + título, sin descripción), `px-4 py-3` + gaps reducidos → ~mitad de alto. Sigue server component (0 JS nuevo). Docstring viejo ("4 signals/2x2") corregido. typecheck + lint OK. Visible tras ISR. **Próximo**: redeploy para ver; o seguir.
