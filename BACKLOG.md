...
- [ ] **Cache / rendering: el sitio responde `no-store` en todo** (auditoría 2026-06-09). Cada request regenera la página entera (más lento bajo carga, peor LCP). Causa: se lee la sesión del usuario (Supabase, cookies) en el árbol de render → Next marca todo dinámico. Fix = cambio arquitectónico delicado (mover la lectura de auth a un borde / PPR / segmentar estático las páginas de catálogo) — puede romper login/carrito/checkout, hacer en sesión dedicada con pruebas. NO es quick win.
