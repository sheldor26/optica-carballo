...
- [ ] **Vigilar LCP mobile tras el cambio de grilla a 1 col / 4 col** (2026-06-15). Las grillas de categoría pasaron a 1 col en mobile → la 1ra imagen se sirve a 100vw (antes 50vw). La card NO tiene `priority` (lazy). Mirar LCP mobile p75 en Vercel Speed Insights post-deploy; si sube de ~2.5s, evaluar `priority`/`fetchPriority=high` SOLO en la 1ra card de cada grilla (con dato real, no a ciegas — recomendación de nextjs-performance).
...
- [ ] **Cross-link sol↔receta pendiente para Vulk Dieven y Rusty Peating** (detectado 2026-07-14). Ambos modelos tienen versión de SOL activa en ML (Dieven: varios MLAs gris polarizado/rosa translúcido; Peating: MBLK/S10 + SBLK/DRT03) pero solo la versión RECETA fue cargada al catálogo. El cross-link es automático por convención de slug (`vulk-dieven` ↔ `vulk-dieven-receta`) — apenas se cargue el sol con el slug correcto, engancha solo, sin código nuevo.
