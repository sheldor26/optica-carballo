...
**Estado**: ✅ Cerrado — causa raíz real confirmada y arreglada (ver debajo; distinta de la hipótesis inicial de la dirección del remitente).
...
**Causa raíz (confirmada vía logs de Vercel)**: `buildSender()` armaba la dirección de la óptica desde variables de entorno (`NEXT_PUBLIC_BUSINESS_ADDRESS_*`) SIN el fallback hardcodeado que sí tiene `lib/site/business.ts` para el mismo dato. El log del intento real mostró el body exacto mandado a MiCorreo: `originAddress: {streetName: null, city: null, provinceCode: null, postalCode: "3342"}` — las env vars de calle/localidad/región nunca se cargaron en Vercel (solo el código postal, vía una env var distinta que sí está seteada). El resto del sitio se ve bien igual porque `business.ts` sí tiene fallback; `buildSender()` no lo tenía. Un remitente sin calle/ciudad/provincia es lo que tira el 500 sin mensaje del lado de MiCorreo.

**Primer fix (necesario pero NO suficiente)**: agregado el mismo fallback hardcodeado que ya usa `getBusinessInfo()` en `lib/site/business.ts` (dirección real: "Av. Lavalle 2686", "Gob. Virasoro", "Corrientes", "3342") a `buildSender()`. El founder reintentó y el 500 siguió IDÉNTICO — el log confirmó que la dirección ya llegaba completa, así que no era la única causa.

**Causa raíz REAL, aislada empíricamente contra prod (con `extOrderId` de diagnóstico, mismo patrón idempotente/sin-costo aprobado en junio)**: para envíos a **SUCURSAL** (`deliveryType: "S"`), MiCorreo devuelve 500 sin mensaje si `shipping.address` se manda como un OBJETO — sin importar si sus campos están en `null` o llenos con datos reales. Solo funciona si `address` es `null` literal (o la key está ausente). Nuestro código (`EMPTY_ADDRESS`, un objeto con 7 campos en `null`) mandaba justo la forma que rompe la API. Confirmado con 5+ variantes de payload contra la API real: los envíos a **domicilio** (`D`) siempre funcionaron bien (nunca fue la causa); ningún envío a **sucursal** funcionó hasta cambiar `address` a `null`. Este es el primer envío a sucursal que se probó contra prod — en junio solo se validó domicilio (dato que ya estaba en `CURRENT_STATE.md` como pendiente, la pista correcta desde el principio).

**Fix real**: `lib/correo/import.ts` — se elimina `EMPTY_ADDRESS`, `shippingAddress` ahora es `null` por default (tipo `CorreoApiAddress | null` en `lib/correo/types.ts`). Verificado contra prod con el payload EXACTO del pedido real (agency R0300, peso 750g, mismo valor declarado) → `200 {createdAt: "..."}`.
