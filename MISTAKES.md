...
**Estado**: 🟡 Fix aplicado, esperando confirmación del founder en el próximo reintento real
...
**Fix**: agregado el mismo fallback hardcodeado que ya usa `getBusinessInfo()` en `lib/site/business.ts` (misma dirección real: "Av. Lavalle 2686", "Gob. Virasoro", "Corrientes", "3342") directamente en `buildSender()` (`lib/correo/import.ts`). No se inventó fallback de teléfono — ese campo no tiene precedente hardcodeado en ningún lado del código (ni siquiera `business.ts` lo hace), y el teléfono del destinatario (obligatorio) sí llegaba bien en el payload, así que no es plausible causa del 500.
