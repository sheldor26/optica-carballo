...
/**
 * Página hija SEO de marca por género —
 * `/anteojos-de-sol/[brand]/hombre` y `/mujer` (también receta).
 *
 * Diferencias vs `BrandCatalogPage`:
 * - Breadcrumb 4 niveles.
 * - H1 con keyword target ("Anteojos de Sol Vulk Hombre").
 * - SIN BrandStorySection (la story va en la página parent /[brand]).
 * - Empty state apunta a la página parent para que el usuario explore
 *   el resto del catálogo de la marca.
 */
export function BrandGenderCatalogPage({
  category,
  brand,
  products,
  target,
}: Props) {
  const targetLabel = target === 'hombre' ? 'Hombre' : 'Mujer';
  const targetLower = target === 'hombre' ? 'hombre' : 'mujer';
  const pageUrl = `${SITE_URL}/${category.slug}/${brand.slug}/${target}`;
    ...
    <main className="container py-8 md:py-12">
      <BreadcrumbJsonLd
        items={[
          ...
          { name: targetLabel, url: pageUrl },
        ]}
      />
      <CatalogJsonLd
        brandName={`${brand.name} ${targetLabel}`}
      ...
      <RevealOnScroll as="article" className="mb-10 max-w-3xl">
        ...
        <h1 className="text-balance font-serif text-4xl font-medium leading-[1.05] tracking-[-0.02em] md:text-5xl lg:text-6xl">
          ...
          <span className="italic font-normal">{targetLabel}</span>
        </h1>
        <p className="text-muted-foreground mt-4 text-balance text-base md:text-lg">
          {items.length > 0
            ? `${items.length} ${items.length === 1 ? 'modelo' : 'modelos'} de ${brand.name} para ${targetLower}. Incluye unisex.`
            : `Por ahora no tenemos modelos específicos de ${brand.name} para ${targetLower}.`}
        </p>
      </RevealOnScroll>
      ...
      {items.length === 0 ? (
        ...
        <section
          aria-label={`Productos ${brand.name} ${targetLabel} en ${category.name.toLowerCase()}`}
    ...
    <div className="mx-auto mt-12 max-w-md text-center md:mt-16">
      <div className="from-muted/30 to-background relative isolate overflow-hidden rounded-2xl bg-gradient-to-b p-10 md:p-14">
        ...
        <p className="text-muted-foreground mx-auto mt-3 max-w-sm text-balance text-sm">
          Aunque no tenemos modelos específicamente filtrados, el resto del
