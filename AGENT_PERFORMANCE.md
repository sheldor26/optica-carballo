...
- **Casos de éxito (2026-06-11, simulador de visión)**: validación del modelo físico ANTES de codear, formato "validá/corregí punto por punto". Detectó 3 errores que el typecheck jamás vería: σ transpuesta (astigmatismo girado 90°), falta de zona muerta 0.25D (emétrope habría visto borroso), y propuso modelo de acomodación unificado A\*=clamp(SE+D,0,U) más simple y correcto que mis fórmulas ad-hoc. Bonus no pedidos de altísimo valor: gaussian blur apaga las luces nocturnas (separar en blend screen), ghost image como EL efecto reconocible del astígmata, binocular = mejor ojo (mi propuesta "peor ojo" era deshonesta en la dirección que conviene vender — lo marcó él). Disclaimer YMYL usable textual. Mejor invocación hasta ahora.
- **Casos donde requirió corrección**: -
- **Notas**: Encaja perfecto en el workflow YMYL: optical-expert valida → founder ratifica → regente firma lo clínico. Brief guardado en `content/briefs/defectos-refractivos.md` como fuente de verdad para content-writer-medical. Confirmado 2026-06-11: el trigger "validar ANTES de publicar" rinde máximo aplicado a MODELOS/fórmulas en fase de diseño, no solo a texto (ver LEARNINGS 2026-06-11).
...
- **Casos de éxito (2026-06-11, simulador de visión)**: ruta nueva consultada ANTES de crearla (trigger automático). Slug `/simulador-de-vision` decidido con data real de los CSVs (descartó `/simulador-de-miopia` por encierro mono-condición y `/como-ves-sin-anteojos` por no matchear fraseología "[condición] como ven" ~480/mes); title/meta exactos para `buildInfoPageMetadata`, H2s mapeados a keywords, WebApplication+FAQPage sin MedicalWebPage, internal links verificados contra guías EXISTENTES. Esta vez el I/O funcionó (leyó CSVs y pages reales) y hasta detectó proactivamente el riesgo de copiar el `force-dynamic` del lector a la página nueva. Sin confabulación detectada (verifiqué los slugs de guías contra `content/guias/` igual, por la regla de abajo).
