...
**Qué funcionó**: en vez de seguir generando hipótesis y pidiéndole al founder que reintente cada una en el sitio real (lento: cada ciclo depende de él, y cada click de "Generar envío" es una acción real), armé un script standalone que le pega directo a `/shipping/import` de MiCorreo en PROD con `extOrderId` FALSOS (prefijo `DIAG-`, mismo patrón ya aprobado en junio: queda en "preimposición" en el panel, sin costo si no se despacha) probando 9 variantes del payload en paralelo (repro exacto, distintos campos rellenos/vacíos, un control con `deliveryType: D` que sabíamos que andaba). El control confirmó que domicilio seguía OK (descartaba una rotura general de credenciales/auth) y la comparación sistemática de variantes aisló el campo exacto en minutos: `shipping.address` como objeto (aunque todos sus campos fueran `null`) rompía sucursal; `address: null` literal andaba. Verifiqué el fix final con el payload EXACTO del pedido real bloqueado antes de pedirle al founder que reintentara — cuando reintentó, ya tenía confianza alta de que iba a andar.

**Por qué importa**: pedirle al founder reintentar en el sitio real cada hipótesis es costoso (le interrumpe el flujo, y cada intento fallido es ruido). Cuando una integración externa devuelve un error sin detalle y ya se agotó lo que dicen los logs propios, tiene más leverage aislar el campo exacto con llamadas directas y baratas a la API (usando el mecanismo de datos-de-prueba ya establecido, si existe) que seguir iterando sobre el código a ciegas y gastando los reintentos del founder.

**Cómo replicar**: (1) confirmar que el mecanismo de prueba sin costo existe (acá: `extOrderId` fake queda en preimposición, no se cobra si no se despacha — patrón ya validado con el founder en junio); (2) pedir el OK explícito antes de pegarle a una API externa de producción con credenciales reales, aunque sea de bajo riesgo (el sandbox de Claude Code bloquea esto por default, correctamente); (3) armar variantes que aíslen UN campo por vez + un control conocido-bueno; (4) recién después de un 200 confirmado con el payload EXACTO del caso real, pedirle al founder el reintento definitivo a través del sitio.

**Hallazgo específico de MiCorreo (útil si se vuelve a tocar `lib/correo/import.ts`)**: para `deliveryType: "S"` (sucursal), `shipping.address` debe ir `null` literal — un objeto con los 7 campos en `null` (lo que hacía nuestro código con `EMPTY_ADDRESS`) da 500 sin mensaje. No documentado en el manual oficial v2025-01-14.

## 2026-08-03 — Después de scrapear muchas páginas seguidas de ML en la misma sesión, el sitio empieza a colgarse — mitigar con pestaña nueva + esperas largas, no reintentar en loop corto

**Contexto**: después de recorrer las 14 páginas de la tienda Vulk + varias búsquedas más para conseguir links directos de productos puntuales (~20 navegaciones a ML en la misma sesión de `claude-in-chrome`), el sitio empezó a quedarse en el spinner de carga indefinidamente — `get_page_text` y `find` fallaban con "Page still loading" incluso después de 45+ segundos, en varias URLs distintas (no era un problema de una página puntual).

**Qué funcionó**: (1) abrir una **pestaña nueva** (`tabs_create_mcp`) en vez de seguir reintentando en la misma — a veces destrababa de inmediato; (2) cuando ni eso alcanzaba, esperar tramos largos (8-10s) en vez de reintentar cada 2-3s — el contenido terminaba apareciendo solo, sin necesidad de recargar. Nunca hizo falta cerrar sesión ni resolver un captcha — parece ser throttling de carga/hidratación del lado del cliente (SPA pesado), no un bloqueo real de acceso.

**Por qué importa**: si el founder pide de nuevo scrapear varias páginas seguidas de ML (otro proveedor, otra tienda, benchmarking de precios), conviene anticipar que después de ~15-20 requests en la misma sesión el sitio se pone lento, y no interpretar eso como que el approach dejó de funcionar — con paciencia (esperas largas, pestaña nueva si hace falta) se recupera solo.

**Cómo replicar**: si `get_page_text`/`find` fallan repetidamente con "still loading" en una tarea de scraping de ML tras muchas navegaciones seguidas, antes de cambiar de approach: (1) esperar 8-10s (no menos); (2) si persiste, abrir pestaña nueva y renavegar la misma URL; (3) recién si eso tampoco destraba en 2-3 intentos, asumir que hay un problema real y avisar al usuario en vez de seguir reintentando en loop.

## 2026-08-03 — Scrapear un listado de Mercado Libre (tienda/storefront) sin API: usar el Chrome real del founder, no el browser sandboxeado, y la URL soporta offset de paginación directo

**Contexto**: founder pidió el título + precio de las 647 publicaciones de la tienda oficial Vulk en ML (`listado.mercadolibre.com.ar/tienda/vulk/vulk`), sin entrar a cada ficha.

**Qué NO funcionó**: (1) Bright Data MCP — listado como servidor que "requiere autorización"; el founder dijo que ya estaba autorizado pero `ToolSearch` no encontró ningún tool con ese nombre — el servidor no llegó a estar realmente disponible en la sesión pese a la instrucción del founder (verificar con `ToolSearch` antes de asumir que un MCP está listo, no confiar solo en lo que dice el usuario). (2) El Browser pane sandboxeado (`mcp__Claude_Browser__*`) y `WebFetch` — ambos redirigen a un muro `account-verification` de ML apenas se pide una página de tienda/storefront sin sesión logueada real (bot-detection, no error del tool). Confirmado con ambos por separado para descartar que fuera un bug puntual de una herramienta.

**Qué funcionó**: `claude-in-chrome` (el Chrome real del founder, con su sesión ML ya logueada) cargó la tienda sin fricción — mismo dominio, misma URL, la única diferencia es la sesión/cookies reales del navegador del usuario. Para paginar sin depender de clickear "Siguiente" (refs de DOM que cambian y tardan en cargar), la URL del storefront de ML soporta offset directo: `tienda/<slug>/<slug>_Desde_<N>_NoIndex_True` donde N arranca en 49 y sube de 48 en 48 (49, 97, 145...). El footer de paginación (`Anterior 1 ... N Siguiente`) confirma cuántas páginas totales hay realmente — no asumir a partir de `resultados totales / 48`, porque algunas páginas muestran menos ítems visualmente (variantes agrupadas bajo "Disponible en X colores" cuentan como 1 sola card pero ocupan varios de los 48 slots del offset).

**Por qué importa**: sirve de plantilla para cualquier pedido futuro de "traeme los precios de la tienda de ML de \<marca\>" (competidores, proveedores, benchmarking) sin necesitar Bright Data ni un MCP de scraping dedicado.

**Cómo replicar**: 1) intentar `claude-in-chrome` directo si el founder tiene sesión en ML (más confiable que browser sandboxeado); 2) navegar a la URL base, esperar a que cargue (spinner → contenido, puede tardar 3-8s, no asumir listo tras 1 wait corto); 3) `get_page_text` para extraer título+precio de la lista (formato: título, marca, rating opcional, `$` precio lista, `$` precio con descuento opcional + `%OFF`); 4) para la página siguiente, navegar directo a `_Desde_<N>_NoIndex_True` en vez de clickear, incrementando de 48 en 48; 5) parar cuando la página ya no tenga link "Siguiente" en el footer de paginación.

## 2026-08-03 — Cruce de lista de precios de proveedor (Excel) contra el catálogo: SKU vive en `product_variants.sku`, matchea 1:1 el "Código" del proveedor

**Contexto**: founder pasó `LISTA 1 DE PRECIOS VULK SOL 20-07-2026.xlsx` (columnas Marca/Código/Artículo/Precio Sugerido Final) pidiendo cruzarla contra el catálogo para actualizar precios, como prueba antes de decidir si automatizarlo.

**Qué funcionó**: el "Código" del Excel del proveedor es exactamente `product_variants.sku` en Supabase (no hace falta mapeo ni tabla intermedia) — un join directo por `sku` alcanza. Para verificar que el match no sea un falso positivo por coincidencia numérica, comparar el `attributes->>'model_code'` (o `reference_code`) de la variante contra el "Artículo" del Excel — no por similitud de string completa (da bajo por diferencias de formato, ej. DB guarda "Dorado - C1" y el proveedor "ANT NOVA C1 P (BCT)"), sino por el código de color/modelo que ambos comparten (ej. "C1", "MBLK/S10 POL") — ese fragmento sí coincide siempre que el match sea real.

**Por qué importa**: permite validar en minutos un cruce de cientos de filas sin abrir cada ficha a mano, y sirve de plantilla para las próximas listas de precios (Vulk Receta, Rusty, etc.) — mismo query, cambiando el `brand.slug` y el archivo.

**Cómo replicar**: 1) leer el xlsx con `openpyxl` filtrando filas sin código (headers/separadores de sección); 2) traer de Supabase `sku` + `price_cents` + `attributes->>'model_code'` de todas las variantes de esa marca; 3) join por `sku` exacto (como string, algunos SKU son alfanuméricos ej. `KATLEEN-MDEMI`); 4) los que no matchean, chequear su `category.slug` antes de asumir error — si la lista es de una sola línea (ej. solo "SOL"), lo esperable es que los de `anteojos-de-receta` queden sin match; 5) señalar cualquier variante cuyo precio se mueva en dirección contraria al resto (ej. la mayoría sube ~12% y una sola línea baja) como anomalía a confirmar antes de aplicar, no aplicarla de una.
