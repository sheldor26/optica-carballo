...
|-------|-------------------|-----------|--------------|---------|--------|
| Rusty: Bruice RECETA (aviador doble puente unisex, 2 var) | 1 | 2 (stock 4) | 5 (HTTP 200) | 1 | ✅ Aplicado MCP + PDP live (seed 94, 2026-08-25). **RECETA** aviador doble puente UNISEX, G-Flex frente y patillas, bisagras metálicas, 23 g. MBLK 957000 (negro mate, stock 4, primary) y CRY 957001 (transparente cristal, stock 0), $84.932 c/u, item multi-variación MLA1476793113 con `variation_code` real. **⚠️ La publicación de ML dice "bluecut" y es FALSO**: el founder lo desmintió, los cristales son de demostración. Va como callout `warning` arriba de todo, junto con que un armazón de receta no lleva UV400 ni polarizado. Verificado en producción: 0 apariciones de UV400/bluecut/categoría 3. **NO** se cargan `lens_material`, `lens_treatment` ni `lens_category` (convención receta, seed 84); sí `lens_compatibility` y `hinge_system`. **Slug `rusty-bruice-receta` obligatorio y exacto**: el cross-link sol↔receta lo arma `fetchCompanionModality()` por convención de nombre — verificado en las dos direcciones. **SEO**: branded, NO `anteojos aviador` (590) que ya es de Rusty The Take receta, el otro aviador del cluster. La placa MEDIDAS fw23 del fabricante dice 56-18-140, confirmación independiente del puente 18. 4 fotos en 92% con scale 1.00, sin overrides. |
| Rusty: Bruice SOL (aviador doble puente unisex, 4 var, 2 polarizadas) | 1 | 4 (stock 5) | 9 (HTTP 200) | 1 | ✅ Aplicado MCP + PDP live (seed 93, 2026-08-25). Aviador doble puente UNISEX, G-Flex, policarbonato UV400 cat 3, **23 g — el más liviano del cluster**. **2 colorways**: MBLK/ORANGE SKU 957006 (negro mate / naranja, primary, MLA1904009956, stock 2) y MDEMI HD-GG47 SKU 968190 (carey mate con patillas negras / verde degradé, stock 3, **publicación propia MLA2035140957**, creada el mismo día — separada de la de la naranja porque la cuenta está en el modelo User Products, donde el título lo arma ML y el de la naranja está congelado por sus 60 ventas nombrando un solo color). $84.354 las dos. **⚠️ NINGUNA POLARIZA** pese al meta genérico del fabricante: las colorways polarizadas del Bruice llevan POL en el código (957005, 957004) y tienen SKU aparte → excluido de `/anteojos-de-sol/polarizados`, verificado en producción. **⚠️ El puente es 18, no 16**: se cargó 16 y se corrigió el mismo día — el grabado del armazón estaba gastado y el 8 parecía un 6; lo destapó mirar la placa vieja de ML antes de reemplazarla. Placa regenerada como `medidas-18mm.jpg` (nombre nuevo por el cache de 31 días). **⚠️ Sin claim de "terapia del sueño"** (lente cat 3, la evidencia de blue-blocking habla de lente clara). `gender=unisex` del `GENDER` de la publicación de ML. **SEO+catalog-loader** (norma; primaria branded `anteojos de sol rusty bruice`; el valor real es que `/anteojos-de-sol/aviador` pasa a 4 productos). Fotos de `pnpm placas` (2000×1333) subidas con `pnpm fotos:subir`, una placa de medidas compartida (mismas medidas). **Sin scale override**: las 4 fotos en 92% con 1.00. Las 6 fotos de MLA1904009956 se reemplazaron por el set azul con `pnpm ml:fotos` (item sigue active, 60 ventas). La MDEMI se publicó aparte (MLA2035140957, activa, 3 unidades, 6 placas): pozos de stock verificados independientes (3 y 2) y la de 60 ventas intacta. **2026-08-25, 2ª tanda**: entraron las 2 POLARIZADAS en stock 0 — MBLK/SG91 POL 957005 (negro mate / gris degradé) y STEELBLUE/S10 POL 957004 (azul acero translúcido / gris oscuro), **$96.251** (el polarizado vale más), las dos colgadas del item multi-variación MLA1897188728 con `variation_code` real (184579332651 / 184579332649). Con esto el Bruice entra a `/anteojos-de-sol/polarizados`, donde la card muestra sólo el subconjunto polarizado: "Polarizado · Sin stock · 2 colores". Copy reescrito a 4 colorways. 8 fotos en 92% con scale 1.00, sin overrides. |
| Rusty: Yeah SOL (aviador doble puente unisex, 3 var) | 1 | 3 (stock 9) | 7 (HTTP 200) | 1 | ✅ Aplicado MCP + PDP live (seed 92, 2026-08-04). Fila agregada 2026-08-25 — el seed estaba aplicado pero el producto no figuraba acá. Aviador doble puente UNISEX, full ACETATO BIO-BASED (diferenciador vs el resto del cluster, que es G-Flex), policarbonato cat 3, 32,9 g. **Estructura MIXTA de variantes, primer precedente del catálogo**: C1 1194370 + C2 1194371 comparten MLA2819011214 con `variation_code` distinto; C3 1194372 tiene MLA2819024290 con `variation_code` NULL. $133.965 c/u. **2/3 polarizadas → NO se afirma "Polarizado" en title/H1.** ⚠️ Los 2 links de ML del founder tenían las etiquetas de color cruzadas; se desambiguó con `attribute_combinations` de la API. Scale 1.2/1.0 provisional. |
| Rusty: PRO 30 RECETA (cuadrado hombre, 1 var) | 1 | 1 (stock 3) | 3 (HTTP 200) | 1 | ✅ Aplicado MCP + PDP live (seed 90, 2026-08-01). **RECETA** cuadrado HOMBRE, G-Flex + alma de metal en patilla + terminales de goma, 22,9g, garantía 1 año. Variante única LIGHT GREY (gris transparente/terminales azules) SKU 126411 MLA1939580597, $95.399, stock 3. ⚠️ Duplicado en ML (MLA3709808184) NO usado. **NO bluecut**. **SEO+catalog-loader** (norma; primaria `anteojos transparentes hombre` 260 — carril libre vs Spell/Invig; cross-link ↔Spell/Zinz/Peating). Fotos en `Rusty-PRO30-receta/` (bucket con mayúsculas). Scale 1.1/1.0. |
| Rusty: Peating Carey RECETA (cuadrado unisex carey, 2 var) | 1 | 2 (stock 4) | 5 (HTTP 200) | 1 | ✅ Aplicado MCP + PDP live (seed 84, 2026-07-14). Cuadrado unisex carey, G-Flex, 18,9g, bisagras metálicas. SDEMI brillo 129251 (primary) / MDEMI mate 129254, $79.790 c/u. NO bluecut. SEO primaria `anteojos carey` (carril libre). Cross-link → rusty-peating (sol) visible. |
| Rusty: Peating SOL (cuadrado unisex, 2 var polarizadas) | 1 | 2 (stock 3) | 5 (HTTP 200) | 1 | ✅ Aplicado MCP + PDP live (seed 85, 2026-07-14). Cuadrado unisex negro, G-Flex, 18,9g, AMBAS polarizadas. MBLK/S10 960100 (primary) / SBLK/DRT03 960103, $82.127/$81.798. name "Rusty Peating" (sin forma, carril de Zinz). Cross-link → rusty-peating-receta visible. |
| Vulk: Dieven Unisex RECETA (rectangular unisex, 3 var) | 1 | 3 (stock 6) | 7 (HTTP 200) | 1 | ✅ Aplicado MCP + PDP live (seed 86, 2026-07-14). Rectangular bordes anchos unisex, G-Flex, 28,5g, bisagras plásticas reforzadas. MBLK 116080 (primary) / SBLK 116083 / L.ROSE 116090, $75.590 c/u. Primer rectangular Vulk receta. NO bluecut. Cross-link → vulk-dieven (sol) visible. |
| Vulk: Dieven SOL (rectangular unisex, 3 var, 2 polarizadas) | 1 | 3 (stock 17) | 7 (HTTP 200) | 1 | ✅ Aplicado MCP + PDP live (seed 87, 2026-07-14). Rectangular unisex, mismo armazón que receta (forma resuelta vs "Cuadrado" del título ML). MBLK/S10 GREY POL 116085 (primary, pol) / SBLK/SG91 POL 116089 (pol) / ROSE/BROWN-GREEN 116087 (polarización no confirmada, sin afirmar). name "Vulk Dieven". Cross-link → vulk-dieven-receta visible. |
| Vulk: Kirt RECETA (redondo unisex metal, 2 var) | 1 | 2 (stock 4) | 5 (HTTP 200) | 1 | ✅ Aplicado MCP + PDP live (seed 83, 2026-07-07). **RECETA** redondo UNISEX metal, liviano 17,5g, medium, patillas Monel + bisagras integradas. **2 MLAs separados** (var_code NULL): LG dorado 128870 MLA3572693800 (primary); MDB cobre 128871 MLA3572693798. $74.990 c/u, stock 2. SKUs del founder. `frame_shape="redondo"`. Medidas 138/42×45/22/140. **NO bluecut**. **SEO+catalog-loader** (norma; primaria `anteojos de metal` 210 — diferencia de My Crew G-Flex; cross-link ↔Ther). Fotos 900×442, scale 1.1/1.0. ⬜ Founder reverifica scale en grid. |
| Rusty: Invig RECETA (rectangular hombre metal, 3 var) | 1 | 3 (stock 6) | 7 (HTTP 200) | 1 | ✅ Aplicado MCP + PDP live (seed 82, 2026-07-07). **RECETA** rectangular HOMBRE metal, ultra liviano 14,7g, bisagra acero inox. **3 MLAs separados** (var_code NULL): C2 negro mate 840071 MLA1878532277 (primary); C1 marrón 840070 MLA1878532279; C3 gris oscuro 840072 MLA1878519921. $72.999 c/u, stock 2. SKUs del founder. `frame_shape="rectangular"`. Medidas 144/54×41/19/140. **NO bluecut**. **SEO+catalog-loader** (norma; primaria `anteojos rusty hombre` 390, único rectangular masculino; cross-link ↔Ther/R-CY02/Woxi). ⬜ Founder: subir 7 fotos a `rusty-invig-receta/` (vacío) + confirmar colores C1 BLUE/C3 PLATEADO vs ML marrón/gris + hero C2. Scale 1.1/1.0 provisional al subir. |
| Vulk: Strewn RECETA (cuadrado femenino transparente, 3 var) | 1 | 3 (stock 6) | 7 (HTTP 200) | 1 | ✅ Aplicado MCP + PDP live (seed 81, 2026-07-07). **RECETA** cuadrado FEMENINO small, G-Flex + bisagras flexo, lentes demo, 17,8g. **3 MLAs separados** (var_code NULL): MBLK negro mate MLA1878507183 (primary); CRY transparente MLA1878518671; M.ROSE rosa transp. MLA1878507185. $80.190 c/u, stock 2. **SKU reales** (founder 2026-07-07): MBLK 113140 / CRY 113145 / M.ROSE 113143 (UPDATE por item_id). `frame_shape="cuadrado"`. Medidas 134/50×45/18/145. **SEO+catalog-loader** (norma; primaria `anteojos transparentes mujer` 390, NO forma → anti-canib. vs Katleen que es el carril cuadrados-mujer; cross-link Strewn↔Katleen). Fotos bucket `vulk-strewn-receta/` (founder renombró la carpeta mid-carga). Scale **2 familias**: MBLK/MROSE 1.1/1.0 (900×442); CRY 1.2/1.05 (5365×3577). ⬜ Founder: 3 SKUs reales + reverificar scale CRY en grid. |
...
| Rusty: Ther Optics RECETA (redondo metal, 3 var) | 1 | 3 (stock 126) | 7 (HTTP 200) | 1 | ✅ Aplicado MCP (seed 75, 2026-06-15). **Categoría receta**. REDONDO de METAL unisex liviano 14,5g, patillas metal+terminales acetato, bisagra acero inox flex, lentes demo (mono/bi/multifocal). 3 colores mate items simples todos $74.990: M020 cobre 128172 stock 122 (primary), M036 violeta 128171 stock 2, M033 bordó 128173 stock 2. Link era catálogo MLAU → MLA reales por API search. `frame_shape="round"`, `frame_material="metal"`. Medidas 135/49×47/23/145. **SEO+catalog-loader** (norma; primaria `anteojos redondos`+carril metal; anti-canib. vs Misty receta acetato). Scale 1.0 (900×442). 7 fotos HTTP 200. |
| Rusty: Zinz (cuadrado unisex sol, 2 var polarizadas) | 1 | 2 (stock 14) | 5 (bucket) | 1 | ✅ Aplicado MCP (seed 79, 2026-06-30). **SOL** cuadrado unisex polarizado (hermano del receta). G-Flex frente+patillas + bisagras flex customizadas, policarbonato polarizada UV400 cat3, **25,7g**. **Multivariante** (1 MLA, var_id): 669K-SBLK/DRT23 POL 126807 $78.869,25 stock 10 (primary, gris transparente); MBLK/S10 POL 126805 stock 4 (negro brillo). **SKU los pasó el founder.** 2/2 pol → afirma "Polarizado" + /polarizados. `frame_shape="cuadrado"`. Medidas 137/51×42/20/145. **SEO+catalog-loader** (norma; primaria forma `lentes/anteojos de sol cuadrados`; anti-canib. vs cluster Rusty sol + vs Zinz receta). Scale 1.1/1.0 (669K 900×442 + MBLK 1200×589). 5 fotos en bucket. ⬜ reverificar grilla/scale post-ISR (regla 15). ⬜ recommended_face_shapes (BACKLOG). |
| Rusty: Zinz Optics RECETA (cuadrado unisex, 3 var) | 1 | 3 (stock 4) | 7 (bucket) | 1 | ✅ Aplicado MCP (seed 78, 2026-06-30). **Categoría receta**. CUADRADO unisex, frente+patillas G-Flex + bisagras metálicas flex, lentes demo, **25,7g**. 3 items simples: SBLK OPTICAL 126801 $79.790 stock 2 (primary, negro brillo); 669K-SBLK OPTICAL 126803 $82.745,69 stock 1 (gris oscuro/patillas negro brillo); MBLK OPTICAL 126800 $82.745,69 stock 1 (negro mate). **SKU 669K-SBLK+MBLK los pasó el founder** (no estaban en ML). `frame_shape="cuadrado"`. Medidas 137/51×42/20/145. **SEO+catalog-loader** (norma; primaria `anteojos/lentes cuadrados`; anti-canib. vs Spell receta cuadrado masculino + Katleen receta cuadrado femenino → Zinz = forma neutra unisex). Scale 1.1/1.0 (1200×589). 7 fotos en bucket. ⬜ reverificar grilla/scale post-ISR (regla 15). |
| Rusty: The Take (aviador doble puente sol unisex, 1 var) | 1 | 1 (stock 3) | 3 (HTTP 200) | 1 | ✅ Aplicado MCP (seed 77, 2026-06-29). **SOL** aviador doble puente unisex POLARIZADO. Frente G-Flex + bisagras flex, patillas ACETATO, policarbonato polarizada UV400 cat3, 18g. 1 var: MBLK/S10 POL 129234 $85.917 stock 3. `frame_shape="aviador"`. 1/1 pol → afirma "Polarizado" + /polarizados. Medidas 136/51×46/16/145. **SEO+catalog-loader** (norma; primaria forma aviador; anti-canib. vs Vulk The Trial sol + The Take receta). Scale 1.0. 3 fotos HTTP 200. Hermano del receta. |
| Rusty: The Take Optics RECETA (aviador unisex, 2 var) | 1 | 2 (stock 6) | 5 (HTTP 200) | 1 | ✅ Aplicado MCP (seed 76, 2026-06-29). **Categoría receta**. Aviador unisex, G-Flex (frente+patillas) + bisagras flexo, lentes demo, 18g. 2 items simples $79.790 stock 3: MDEMI-MBLK OPTICAL 129230 (primary, carey mate/negro mate); L.ROSE OPTICAL 129233 (caramelo). model_code OPTICAL. Link catálogo MLAU → MLA por API. Medidas 136/51×46/16/145. **SEO+catalog-loader** (norma; primaria `anteojos recetados`, anti-canib. vs The Trial receta aviador doble puente). Scale 1.0 (900×442). 5 fotos HTTP 200. ⚠️ existe The Take sol aparte (129234) no cargado. |
| Rusty: Woxi Optics RECETA (rectangular pequeño, 3 var) | 1 | 3 (stock 6) | 7 (HTTP 200) | 1 | ✅ Aplicado MCP (seed 74, 2026-06-15). **Categoría receta**. Rectangular PEQUEÑO liviano, G-Flex, **SOLO monofocal** (talle chico). 3 colores items simples, todos $87.890 stock 2: MBLK 111191 (primary), SBLK 111190, SBLKCL-SBLK 111194 (negro brillo/cristal). model_code OPTICAL. Link era catálogo MLAU → MLA reales por API search. Medidas 138/52×37/18/145. **SEO+catalog-loader** (norma; primaria `anteojos recetados`, anti-canib. vs R-CY 02 rectangular). Scale 1.15/1.0 PROVISIONAL. weight omitido → BACKLOG. 7 fotos HTTP 200. |
| Vulk: Bennie 51 (redondo sol unisex, 3 var) | 1 | 3 (stock 12) | 7 (HTTP 200) | 1 | ✅ Aplicado MCP + fotos subidas (seed 73, 2026-06-14). **SOL REDONDO** unisex small, G-Flex + patillas metal flex/goma, policarbonato UV400 cat 3, 18,9g. **3 MLAs**. 1 POL → /polarizados. S10 POL 123600 $95.306 stock 3 (primary, gris pol); G.GREY 123609 $86.573 stock 2 (gris degradé→lens_color); GREEN 123760 $86.573 stock 7 (verde+AR interno→callout). `frame_shape="round"`. Medidas 135/51×37/18/140. **HONESTIDAD** 1/3 pol → title "Redondos". **SEO+catalog-loader** (norma; primaria marca Vulk + redondo; anti-canib. vs Blinded redondo). Scale **1.0 confirmado visual** (fotos 900×442 más anchas que el card → llenan; redondo small NO necesitó 1.15). 7 fotos HTTP 200. |
| Vulk: The Trial Optics RECETA (aviador doble puente unisex, 2 var) | 1 | 2 (stock 0; ambas paused, sincronizan) | 5 (HTTP 200) | 1 | ✅ Aplicado MCP (seed 72, 2026-06-14). **Categoría receta** (hermano del sol). Aviador doble puente unisex, G-Flex + patillas Monel/acetato hecho a mano, lentes DEMO, 19,5g, large. Convención receta (solo lens_compatibility, sin lens_*; model_code OPTICAL; name "Optics"). 2 MLAs paused stock 0: 670-056 125721 $83.745 (primary, celeste translúcido + patillas metal); MBLK-046 125722 $83.736 (negro mate). `frame_shape="aviator"`. Medidas 147/50×49/15/150. **SEO+catalog-loader** (norma; primaria `anteojos recetados`+carril aviador). Scale 1.0 (mismo encuadre 900×442 que el sol). 5 fotos HTTP 200. Cross-link sol↔receta. |
| Vulk: The Trial (aviador doble puente sol unisex, 4 var) | 1 | 4 (stock 9; MDEMI en 0, pedido) | 9 (HTTP 200) | 1 | ✅ Aplicado MCP + fotos subidas (seed 71, 2026-06-14). **SOL AVIADOR DOBLE PUENTE** unisex, G-Flex + patillas Monel/acetato hecho a mano, policarbonato UV400 cat 3, 19,5g, large. **3 MLAs + 1 sin MLA**. 2 POL → /polarizados. SBLK/S10 POL 125728 $86.082 stock 3 (primary); MBLK 053/S15 125725 $78.751 stock 3 (ANTIFOG, no pol); SBLK/ORANGE 125727 $74.243 stock 3 (lente naranja, no pol); MDEMI/UPG15 POL 968279 **sin MLA** stock 0 (carey verde G15, pedido; **⚠️ precio provisional = S10**, actualizar al alta en ML). `frame_shape="aviator"`. ANTIFOG → descripción/callout (no enum). Medidas 147/50×49/15/150 (foto). **HONESTIDAD** 2/4 pol → title "Unisex". **SEO+catalog-loader** (norma). Scale **1.0 confirmado visual** (fotos 900×442 más anchas que el card → ya llenan; 1.20 cropeaba, como Raven). 9 fotos HTTP 200. ⬜ Precio MDEMI provisional (BACKLOG). |
| Vulk: Raven (wayfarer sol unisex, 3 var) | 1 | 3 (stock 8) | 7 (HTTP 200) | 1 | ✅ Aplicado MCP + fotos subidas (seed 70, 2026-06-14). **SOL WAYFARER** unisex (moda), G-Flex, policarbonato revo UV400 cat 3, 26g, medium. **3 MLAs separados**. 2 POL → /polarizados. SBLK/S10 POL 109142 $98.948 stock 5 (primary); MBLK/S10 POL 109148 $98.948 stock 2; MBLK/REVO BLUE 109149 $94.751 stock 1 (no pol, espejada azul+AR interior). `frame_shape="wayfarer"`. Medidas 140/55×46/15/142 (foto). **HONESTIDAD** 2/3 pol → title "Unisex" no "Polarizados". **SEO+catalog-loader** (norma). Scale **1.0 confirmado visual** (fotos 900×442 más anchas que el card → ya llenan; 1.15 cropeaba). 7 fotos HTTP 200. ⬜ Grilla/PDP en vivo tras ISR. |
| Rusty: And Now (envolvente deportivo sol, 3 var) | 1 | 3 (stock 19) | 4 (HTTP 200) | 1 | ✅ Aplicado MCP + PDP live (seed 69, 2026-06-13). **SOL ENVOLVENTE/deportivo** unisex, G-Flex, policarbonato UV400 cat 3. **3 MLAs separados**. 2 POL → /polarizados. SBLK/S10 POL 16118 $93.631 stock 8 (primary); MBLK/S10 POL 16122 $93.631 stock 6; REVO BLUE 1110 $89.630 stock 5 (no pol, espejada+AR interior). `frame_shape="envolvente"`+line deportiva (→/deportivos). **SOLO PERFIL** (1 img/variante). Medidas 140/64×39/18/108 (varilla corta⚠️). Scale 1.1. **SEO+catalog-loader** (norma). ⬜ Peso omitido (BACKLOG). |
| Rusty: Blinded (redondo unisex sol, 2 var) | 1 | 2 (stock 8; MDEMI en 0, reingresa) | 5 (HTTP 200) | 1 | ✅ Seed 68 APLICADO + CERRADO (2026-06-13, founder subió fotos). **SOL REDONDO** unisex liviano 22,7g, G-Flex, antirreflejo interior (las 2). **2 MLAs separados**. NINGUNA polarizada. MBLK/S10 129103 $73.661 stock 8 (primary); MDEMI-MBLK/BROWN 129105 $73.957 stock 0 (sin stock, REINGRESA — no discontinuada). `frame_shape="round"`. Medidas 130/48×43/22/145. **SEO+catalog-loader** (norma). Scale 1.15/1.0 (confirmado visual). ⬜ SKU 129105 aún asumido (founder escribió 29105, no corrigió). ⬜ Peso omitido (BACKLOG). |
